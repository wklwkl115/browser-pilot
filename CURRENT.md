...
- MCP 标准化 + 渐进式披露已完成 Phase -1 → Phase 8（共 9 个阶段全部落地）：TypeBox MCP 参数校验（Phase 0）、工具 annotations + `./mcp` library export（Phase 1）、DistillerDefinition + outputSchemas spike（Phase 2）、structuredContent egress（Phase 3）、browser-result:// resource store + resources/read/list（Phase 4）、nextActions MCP adapter 转换（Phase 5）、ingress handle resolution（Phase 6）、protocol middleware outer ring（Phase 7）、browser_artifact capability-gated retirement（Phase 8）。
- MCP 合同 6 项验收残留（A–F）已全部收口：A 统一 structured-envelope schema、B resource/ingress etag 新鲜度（含 sha256 ingress 完整性）、C Layer-1 section 子资源、D on_initialize/on_message 钩子接通、E list_changed 有意缺席 contract、F token 经济基线（确定性 fixtures，Layer-0<raw + 非回归）。验收期另修 structuredContent outputSchema 一致性、补 check:mcp-e2e 真 stdio fixture。已记边界：网络→sqli 单请求句柄（per-entry 子资源）延后、on_message 仅覆盖未注册方法、token 指标为 3-fixture guardrail。仅余 Phase 9 stretch（server dynamic list + MCP prompts，RFC 状态）未做。
