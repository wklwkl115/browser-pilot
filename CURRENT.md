...
- 当前新增执行合同：MCP 标准化 + 渐进式披露，正式 source of truth 为 `docs/mcp-standardization-progressive-disclosure-plan.md`；`.plan/mcp-standardization-progressive-disclosure.md` 仅保留为评审输入指针。下一步只允许从 Phase 0 开始：补 MCP 入口 TypeBox 等价参数校验与 conformance 地基；不得先做 structuredContent/resources/typed handle 或把顶层参数契约迁到 Zod。
