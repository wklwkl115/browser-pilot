...
- **Browser-tools skeptical-eval 修复线**（2026-06-05，验证中）：执行合同 `docs/browser-tools-eval-fixes-execution-plan.md` 已按 S1-S8 落地实现。首份真 agent 怀疑型评估（n=1，5 个真实站点）的混合结论已反映到 README/skill/coverage map：`causal` 与 `action.type` 优先；`action.click` 用于必须生效或 synthetic 不稳的点击；`action.scroll` 默认改为 bounded/probe-only，`collect:true` 仅用于虚拟列表实体收集；`templates` 继续只承诺 ARIA-grounded lists/tables；`diff.summary` 提供 value/name/state/focus salience，raw diff arrays 保留。实现要点：click/type 默认去掉 before/after 全量 readStructure，改目标语义状态 probe，完整 entity diff opt-in `diff:true`；click/type/scroll 使用共享 action deadline；scroll 去默认每步全扫并修摘要；URL query redaction 覆盖 PII-looking 值和通用 human-query 参数；observe artifact 镜像 live envelope 顶层 `diff/treeDiff/templates/snapshotProjection/causal`；baseline 过期错误带 recovery；bridge execute timeoutMs 透传到扩展脚本执行。边界保持：不新增公开工具、不改 native protocol、脱敏无 per-site、templating/diff 保持 ARIA/kind-grounded、纯核保纯。最终门禁仍以 `npm run check` 和可用时 `smoke:browser:abml-action-gap` 为准；meta 复评需后续新鲜 agent 运行。
