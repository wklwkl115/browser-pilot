/**
 * MCP protocol middleware (hook-style, FastMCP-inspired naming).
 *
...
export const timingLogHook: LoggingHook = (ctx, durationMs, result, details) => {
	...
	console.error(`[pi-browser-mcp] ${ctx.method}${ctx.toolName ? ` ${ctx.toolName}` : ""} ${result} +${durationMs}ms${detail}`);
};
