...
## Recently completed
- Performance audit 当前执行已收口：低风险批次已落地，后续 eval 子代理又关闭了 0.3 armed-recorder cheap-state、1.1 非默认 scan superset、1.7 stream signal ref、2.2 默认等待缩短等无足够证据项；同时落地 2.4 CLI-only details 省略、3.1 artifact 读取建议去重、3.2 `browser_tabs list` compact+top-level bridge（保留 `includeBridgePerTab:true` 兼容）。唯一仍保留的明确 future gate 是 3.3 scan focus entity-ref shape change，需要 targeted blind eval 和迁移合同；详情见 `docs/performance-overhead-audit.md` 与 `CHANGELOG.md`。
