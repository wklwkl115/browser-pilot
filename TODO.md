# TODO
当前 TODO 入口已更新：
- 当前激活主线：ABML 机制臂 M2b semantic ref anchor 合同/gate，见 `docs/abml-mechanism-m2b-semantic-ref-anchor-execution-plan.md`。
- 当前状态与执行队列：`CURRENT.md`。
- 历史完成归档：`ARCHIVE.md`。
- 后续路线与建议：`ROADMAP.md`。
- 文档结构规范：`docs/document-structure.md`；索引同步脚本：`npm run docs:sync-indexes`。
当前执行口径：
- 当前无并行大型架构主线；状态以 `CURRENT.md` 为准。M2b 当前只做合同/gate，不直接改 ref minting。
