...
## Recently completed
- Performance audit 当前低风险批次已落地：bridge whitespace minify、AX box-model 并发、observe network/hook seq 并发、default scan 复用首个 scan payload 给 ABML read、redaction first-hit、CLI render parse-once、network/intercept buffer caps、`browser_execute` 去掉普通路径 200ms 固定等待、CLI command registry memoize 与实际 bin 顶层 help 轻量动态导入、nested validation 移除 zod 依赖、daemon protocol-only version compatibility、offscreen 端口并发 probe、resource/ref store cap + amortized prune、scan summary 跨预算档位复用预计算并补高熵 byte/shape golden、scan summary 剩余 per-rung loop collapse、CJK budget `String.length` guard、`fitEnvelopeBudget`/`fitSummaryBudget` serialize-once safe subset、extension readiness event wait + no-extension negative cache、artifact text/search/sample 减少重复读；剩余明确队列是 0.3 armed-recorder cheap-state 证据、1.1 非默认 scan 模式 superset follow-up、1.7 stream signal ref 合同/eval-first follow-up、2.2 默认等待缩短证据、2.4 contract-first 项和 Tier 3 blind-eval-first 输出瘦身；详情见 `docs/performance-overhead-audit.md` 与 `CHANGELOG.md`。
