...
首个 ready client 初始化默认 browser selection；后续其他 client 的普通 `tabs_update` 不得抢占显式选择，只有同一 extension instance 的 reconnect 可以接替其旧 socket。Public `browserId` 由 `extensionInstanceId` 的 SHA-256 前缀生成，不再使用每个 WebSocket 的随机 id。Extension 为每个 Chrome tab 在 `chrome.storage.session` 保存随机 logical identity，所有 `ext_ready` / `tabs_update` / `tabs.list` payload 都携带 `tabIdentity`；`tabs.onReplaced` 把 identity 从 removed tab 迁到 added tab，真实 tab close 删除它。Daemon 用 stable browser id + `tabIdentity` 派生 `tabHandle`，因此同一 browser runtime 内 daemon replacement、MV3 worker restart、offscreen/socket reconnect 与 in-place tab replacement 都保持原 `targetRef`。Incoming valid `tabIdentity` 是 reconnect 的权威身份：若完整 extension reload 清空 session storage 并产生新 identity，router 立即失效旧 handle，不能暂时沿用到下次 daemon restart。完整 extension reload、browser runtime 重启或 tab close 是明确失效边界。带 `targetRef` 的内部 operation arm/finish 与实际命令始终使用该 tab session 的 owning client，不能回退到 default client。

Bridge status 将已连接但 build id 与当前 unpacked build 不一致的扩展标为 `readiness:"extension-stale"`。CLI `connect --wait` 不得把它当 ready；event-driven waiter 被 stale peer 唤醒后会继续等待剩余 deadline。多个浏览器实例共享端口时，current-build peer 自动替换 stale default，后到的 stale peer 不能反向抢占；只有 deadline 内没有 current peer 时才返回 `CLI_EXTENSION_STALE`、expected/reported build 和 exact `management.reload` recovery。普通 browser command 拒绝 stale runtime，只有该 self-reload recovery 允许通过。`doctor --check` 同时检查 daemon contract 与 extension build，不能由 `DAEMON_CONTRACT_MATCH` 掩盖旧扩展。Reload 完成后调用方必须重新 `tabs list` 获取新 handle。`connectKind` 只描述当前 daemon 进程内最近一次 `ext_ready`：新 daemon 的首个握手是 `cold`，普通 CLI `connect` 复用不会产生新握手，因此不会改写该字段；是否复用了 daemon 由 `startedDaemon` 判断。
