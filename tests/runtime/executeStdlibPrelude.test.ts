...
function element(
	...
	text: string,
	bounds: FakeRect,
	contains: (node: unknown) => boolean = () => false,
): FakeElement {
	return {
		nodeType: 1,
		tagName,
...
test("browserPilot refs never resolve an element from stale coordinates", () => {
	...
	const registry = {
		[ref]: { ok: true, fresh: true, descriptor: { refId: ref, locators: [{ by: "point", x: 90, y: 60 }] } },
	};
	vm.runInContext(
		`${stdlibPrelude(registry, { target: ref })}\nglobalThis.__bound = browserPilot.refs.target;`,
...
test("browserPilot.resolve disambiguates repeated selectors with captured geometry", () => {
	...
	const registry = {
		[ref]: {
			...
			descriptor: {
				refId: ref,
				locators: [{ by: "css", value: ".duplicate" }],
...
test("browserPilot.resolve rebinds a rerendered control by semantic identity", () => {
	...
	context.document.querySelectorAll = (selector: string) =>
		selector === "#stale-selector" ? [] : selector.includes("[aria-label]") ? [replacement] : [];
	...
	const registry = {
		[ref]: {
			...
			descriptor: {
				refId: ref,
				locators: [
					{ by: "css", value: "#stale-selector" },
					{ by: "textAnchor", value: "Run smoke", role: "button", exact: false },
				],
...
test("browserPilot.resolve refreshes semantic candidates after an in-script rerender", () => {
	...
	const registry = {
		[ref]: {
			...
			descriptor: {
				refId: ref,
				locators: [{ by: "textAnchor", value: "Run smoke", role: "button", exact: true }],
				semantic: { role: "button", name: "Run smoke" },
			},
		},
	};
	...
	vm.runInContext(
		`${stdlibPrelude(registry)}\nglobalThis.__before = browserPilot.resolve(${JSON.stringify(ref)}).el?.id; globalThis.__replace(); globalThis.__after = browserPilot.resolve(${JSON.stringify(ref)}).el?.id;`,
...
test("shared ref runtime preserves the canvas region role", () => {
	...
	vm.runInContext(
		`globalThis.__result = (${PAGE_REF_RUNTIME_SOURCE}).point(globalThis.__canvas, { semantic: { role: "region" } }, false);`,
		vm.createContext(sandbox),
	);
...
test("browserPilot.resolve treats native img and AX image roles as the same semantic entity", () => {
	...
	const registry = {
		[ref]: {
			...
			descriptor: {
				refId: ref,
				locators: [{ by: "textAnchor", value: "Preview", role: "image", exact: true }],
				semantic: { role: "image", name: "Preview" },
			},
		},
	};
	vm.runInContext(
...
test("browserPilot.resolve rejects semantic mismatches and ambiguous replacements", () => {
	for (const candidates of [
		...
		[
			element("BUTTON", "save-1", "Save", rect(40, 30, 120, 40)),
			element("BUTTON", "save-2", "Save", rect(200, 30, 120, 40)),
		],
	]) {
		const { context } = createContext();
		context.document.querySelectorAll = () => candidates;
		context.document.elementFromPoint = (x: number, y: number) =>
		...
		const registry = {
			[ref]: {
				...
				descriptor: {
					refId: ref,
					locators: [{ by: "textAnchor", value: name, role: "button", exact: false }],
					semantic: { role: "button", name },
				},
			},
		};
		vm.runInContext(
...
test("browserPilot stdlib exposes its current helper namespace", () => {
	...
	assert.deepEqual(Array.from((context.__namespace as string[]) ?? []), [
		"refs",
		"resolve",
		"box",
		"setValue",
