...
test("page identity equality ignores URL facts and classifies every re-anchor boundary", () => {
	assert.equal(
		samePageIdentity(identity(), identity({ url: "https://example.test/spa", documentId: "diagnostic-only" })),
		true,
	);
...
test("extension epochs advance on document commits and replacement, preserve SPA, and reuse proven BFCache lineage", () => {
	...
	const documentA = recordBrowserPilotDocumentCommit({
		tabId: 7,
		frameId: 0,
		documentId: "doc-a",
		url: "https://example.test/a",
	});
	const documentB = recordBrowserPilotDocumentCommit({
		tabId: 7,
		frameId: 0,
		documentId: "doc-b",
	...
	const restoredA = recordBrowserPilotDocumentCommit({
		tabId: 7,
		frameId: 0,
		documentId: "doc-a",
		url: "https://example.test/a",
...
test("a live fingerprint that disagrees with the tab epoch makes identity unproven", () => {
	...
	assert.equal(
		currentPageIdentity(server as never, { tabId: 7 }, { pageEpoch: "page-old", url: "https://example.test/old" }),
		undefined,
	);
	assert.deepEqual(
		currentPageIdentity(server as never, { tabId: 7 }, { pageEpoch: "page-new", url: "https://example.test/spa" }),
		{
			browserSessionId: "session-1",
			tabId: 7,
			targetGeneration: 2,
