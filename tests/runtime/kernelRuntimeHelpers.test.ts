...
import {
	...
	dedupeEntities,
} from "../../src/kernels/abml/entity.ts";
import {
	axBackendNodeId,
	axName,
	axNodeId,
...
} from "../../src/kernels/abml/relations.ts";
import {
	displayEntityText,
	groupEntities,
...
} from "../../src/kernels/abml/grouping.ts";
import { buildCollectionModels } from "../../src/kernels/abml/collections.ts";
import {
	firstSafeSemanticText,
	isItemLikePreview,
	safeContainerLabelText,
	sanitizeSemanticText,
} from "../../src/kernels/abml/semanticText.ts";
...
test("partial AX helper returns bounded successful enrichment diagnostics", async () => {
	...
	const result = await readPartialAxTree(server, {
...
test("partial AX helper fails closed for missing backend, unsupported, error, timeout, and empty responses", async () => {
	...
	for (const [message, reason] of [
		["'Accessibility.getPartialAXTree' wasn't found", "unsupported"],
		["CDP crashed", "error"],
		["bridge timeout after 1500ms", "error"],
	] as const) {
	...
	const empty = await readPartialAxTree(createCdpServer({ "Accessibility.getPartialAXTree": { nodes: [] } }), {
		tabId: 7,
		backendNodeId: 42,
		maxNodes: 5,
	});
...
test("partial AX helper truncates over-budget nodes and reports degraded diagnostics", async () => {
	...
	const result = await readPartialAxTree(
		createCdpServer({ "Accessibility.getPartialAXTree": { result: { nodes } } }),
		{ tabId: 7, backendNodeId: 100, maxNodes: 3, fetchRelatives: true },
	);
	assert.equal(result.nodes.length, 3);
	assert.deepEqual(
		result.nodes.map((node) => node.backendDOMNodeId),
		[100, 101, 102],
	);
...
test("full AX reader projects fresh snapshot geometry, paint order, and relations", async () => {
	const nodes = [
		axNode(42, "button", "Save changes", {
			properties: [{ name: "controls", value: { relatedNodes: [{ backendDOMNodeId: 77 }] } }],
		}),
	...
	assert.equal(
		first.anchors.some((anchor) => anchor.type === "controls" && anchor.targetKey === "b:77"),
		true,
	);
	assert.deepEqual(first.snapshotGeometryEntries?.[0], {
		backendNodeId: 42,
		bounds: { x: 10, y: 20, w: 80, h: 30 },
		attrs: { id: "save", class: "primary" },
	});
	assert.deepEqual(
		first.paintOrderEntries?.map((entry) => [entry.backendNodeId, entry.paintOrder]),
		[
			[42, 2],
			[77, 1],
		],
...
test("full AX reader absorbs rendered text fragments before entity and ref creation", async () => {
	...
	const server = createCdpServer({
		"Accessibility.getFullAXTree": {
			nodes: [...fragments, axNode(42, "button", "Save"), axNode(77, "button", "Cancel")],
		},
		"DOMSnapshot.captureSnapshot": domSnapshot(),
	});
	...
	assert.deepEqual(
		result.entities.map((item) => item.entity.name),
		["Save", "Cancel"],
	);
	const merged = mergeAxIntoDomEntities([], result.entities).entities;
	assert.equal(
		merged.every((item) => resolveRefUriDetailed(item.ref).ok),
		true,
	);
});
...
test("full AX reader redacts password values from AX-only entities and refs", async () => {
	const server = createCdpServer({
		"Accessibility.getFullAXTree": {
			nodes: [axNode(556, "textbox", "Password", { value: { type: "string", value: "secret" } })],
		},
		"DOMSnapshot.captureSnapshot": {
			strings: ["type", "password"],
			documents: [
				{ nodes: { backendNodeId: [556], attributes: [[0, 1]] }, layout: { nodeIndex: [], bounds: [] } },
			],
		},
	});
...
test("full AX reader consumes only the main DOMSnapshot document", async () => {
	...
	const server = createCdpServer({
		"Accessibility.getFullAXTree": {
			nodes: [
				axNode(42, "button", "Save"),
				axNode(999, "textbox", "Child password", { value: { type: "string", value: "secret" } }),
			],
		},
	...
	assert.deepEqual(
		result.snapshotGeometryEntries?.map((entry) => entry.backendNodeId),
		[42, 77],
	);
	assert.deepEqual(
		result.paintOrderEntries?.map((entry) => entry.backendNodeId),
		[42, 77],
	);
...
test("full AX reader keeps snapshot document geometry while projecting viewport state", async () => {
	...
	(document.layout as Record<string, unknown>).bounds = [
		[10, 120, 80, 30],
		[120, 400, 40, 30],
	];
	...
	assert.deepEqual(
		result.snapshotGeometryEntries?.map((entry) => entry.bounds.y),
		[120, 400],
	);
	assert.deepEqual(
		result.entities.map((item) => item.entity.geometry?.box?.y),
		[20, 300],
	);
	assert.deepEqual(
		result.entities.map((item) => item.entity.state.inViewport),
		[true, false],
	);
});
...
test("runtime fusion refreshes refs only from exact backend identity and redacts password values", () => {
	const context = {
		browserSessionId: "session-1",
		tabId: 7,
		observationId: "obs-ref-refresh",
		capturedAt: Date.now(),
		url: "https://example.test/",
	};
	const dom = buildDomEntityFromScanActionable(
		{
			tag: "button",
			selector: "#fusion",
			label: "DOM label",
	...
	const ax = buildAxEntityFromNode({ role: "button", name: "Accessible label", backendDOMNodeId: 555 }, context, {
	...
	assert.deepEqual(resolved.ref.descriptor.locators.slice(0, 2), [
		{ by: "backendNodeId", value: 555 },
		{ by: "css", value: "#fusion" },
	]);
	assert.equal(resolved.ref.descriptor.semantic?.name, "Accessible label");

	const passwordDom = buildDomEntityFromScanActionable(
		{
			tag: "input",
			role: "textbox",
			selector: "#password",
	...
	const passwordAx = buildAxEntityFromNode(
		{ role: "textbox", name: "Password", value: "secret", backendDOMNodeId: 556 },
		context,
	);
	const passwordFused = mergeAxIntoDomEntities([{ ...passwordDom.entity, ref: passwordRef }], [passwordAx])
		.entities[0]!;
...
test("full AX reader preserves fragmented snapshot projection semantics", async () => {
	...
	assert.deepEqual(result.paintOrderEntries, [
		{ backendNodeId: 42, paintOrder: 1, bounds: { x: 10, y: 20, w: 30, h: 10 } },
	]);
});
...
test("full AX reader degrades from paint snapshot and falls back to bounded box geometry", async () => {
	...
	const paintFallbackServer = createCdpServer({
		...
		"DOMSnapshot.captureSnapshot": () =>
			++snapshotCalls === 1 ? bridgeFailure("includePaintOrder unsupported") : domSnapshot(false),
	});
	...
	const boxFallback = await readAxEntities(boxFallbackServer, {
		tabId: 7,
		observationId: "obs-box-fallback",
		scrollY: 100,
	});
...
test("ABML grouping helpers normalize descriptors and suppress nested non-control groups", () => {
	...
	const duplicateEntities = [
		...Array.from({ length: 4 }, (_, index) =>
			entity(`bp-ref://a/${index}`, {
				role: "listitem",
				kind: "element",
				name: index ? `A${index}` : "Approve",
				hints: { containerRole: "list", containerName: "Results", containerKey: "b:1" },
			}),
		),
		...Array.from({ length: 4 }, (_, index) =>
			entity(`bp-ref://b/${index}`, {
				role: "listitem",
				kind: "element",
				name: index ? `B${index}` : "Approve",
				hints: { containerRole: "list", containerName: "Results", containerKey: "b:2" },
			}),
		),
	];
	...
	assert.equal(
		buildCollectionModels({
			...
			snapshotProjection: buildSnapshotProjection(duplicateEntities),
		}).length,
		2,
	);
...
test("ABML causal projection preserves complete redacted deltas", () => {
	const records = Array.from({ length: 15 }, (_, index) => ({
		...
		request: { method: "GET", url: `https://example.test/${"x".repeat(300)}?index=${index}` },
	...
	const actionScoped = buildTriggeredRelations(
		{
			sinceSeq: 0,
			requests: [
	...
	assert.deepEqual(
		actionScoped.map((relation) => [relation.targetRef, relation.confidence]),
		[["bp-ref://network/after", "medium"]],
	);
	const events = buildCausalEvents(
		records.map((record) => ({ ...record, type: "console", data: { message: "x".repeat(300) } })),
		0,
	);
...
test("ABML semantic text rejects unsafe names and item-like previews for container labels", () => {
	const pricingCard =
		"Kimi K2 Turbo model billing per 1M tokens input $0.60 output $2.50 cache write $0.15 cache read $0.05 context 128k";
...
test("ABML collections and snapshot projection handle empty, repeated, and boundary-sized trees", () => {
			...
			templates: [
				{
					...
					changed: {
						...
						instances: [
							{
								key: "name:Order 2",
								beforeRef: "bp-ref://row/1",
								afterRef: "bp-ref://row/1",
								anchor: "name",
								confidence: "high",
								name: "Order 2",
								fieldCount: 1,
								fields: [{ field: "name", before: "Order 2", after: "Order 2 updated" }],
							},
						],
					},
					reordered: {
						changed: true,
						commonCount: 2,
						beforeSample: ["bp-ref://row/1", "bp-ref://row/2"],
						afterSample: ["bp-ref://row/2", "bp-ref://row/1"],
					},
				},
			],
		},
	...
	const projectedRows = projection.templates.find(
		(template) => template.container === "grid" && template.containerName === "Orders" && template.count === 25,
	)!;
	...
	assert.equal(
		buildCollectionModels({ entities: rows, snapshotProjection: buildSnapshotProjection(rows) }).length,
		1,
	);

	const duplicatedPositions = Array.from({ length: 24 }, (_, index) =>
		entity(`bp-ref://fragment/${index}`, {
			kind: "element",
			role: "div",
	...
	const positionalCollection = buildCollectionModels({
		entities: duplicatedPositions,
		snapshotProjection: buildSnapshotProjection(duplicatedPositions),
	})[0]!;
...
test("ABML collections absorb malformed scan evidence and pagination edges", () => {
	const models = buildCollectionModels({
		entities: [
			entity("bp-ref://list/container", {
				role: "list",
				kind: "region",
				name: "Results",
				hints: { listContainer: true, itemCount: "4" as unknown as number },
			}),
		],
		scanEvidence: {
	...
	assert.deepEqual(
		independent.map((item) => item.paginationControl),
		[undefined, undefined],
	);
	const labelled = buildCollectionModels({
		...
		scanEvidence: {
			listHints: [
				{
					itemCount: 6,
					containerLabel: "Orders",
					selector: "#orders > li",
					firstItemPreview: '<svg><path d="M0 0" /></svg>',
				},
			],
		},
	});
	...
	assert.equal(
		buildRegionEntityFromListHint(
			{ selector: "#pricing", firstItemPreview: pricingPreview },
			{ observationId: "obs-list", capturedAt: 1 },
			1,
		).entity.name,
		"list-1",
	);
	assert.equal(
		buildRegionEntityFromListHint(
			{ selector: "#providers", firstItemPreview: "全部供应商 30" },
			{ observationId: "obs-list", capturedAt: 1 },
			3,
		).entity.name,
		"全部供应商 30",
	...
	assert.equal(
		buildRegionEntityFromListHint(
			{ selector: "#icons", firstItemPreview: '<path d="M0 0" />' },
			{ observationId: "obs-list", capturedAt: 1 },
			2,
		).entity.name,
		"list-2",
	...
	assert.equal(
		buildRegionEntityFromListHint(
			{ selector: "#icons", containerLabel: "Toolbar", firstItemPreview: '<path d="M0 0" />' },
			{ observationId: "obs-list", capturedAt: 1 },
			2,
		).entity.name,
		"Toolbar",
	...
	assert.deepEqual(
		duplicateCollections.map((collection) => collection.containerName),
		["Cards (archived cards)", "Cards (featured cards)"],
	);
	assert.deepEqual(
		duplicateCollections.map((collection) => collection.containerNameSource),
		["disambiguated", "disambiguated"],
	);
	const duplicateListRegions = buildScanEntities(
		pageWorldScanBundle({
			structure: {
				listHints: [
					{
						itemCount: 2,
						containerLabel: "Cards",
						selector: "#featured-cards > li",
						firstItemPreview: "Alpha",
					},
					{
						itemCount: 2,
						containerLabel: "Cards",
						selector: "#archived-cards > li",
						firstItemPreview: "Beta",
					},
				],
			},
		}),
		{ entityContext: { observationId: "obs-dup", capturedAt: 10 } },
	).listEntities;
	assert.deepEqual(
		duplicateListRegions.map((region) => region.name),
		["Cards (featured cards)", "Cards (archived cards)"],
	);
	const duplicateSelectorActions = buildScanEntities(
		pageWorldScanBundle({
			structure: {
				actionables: [
					{
						...
						point: { x: 10, y: 10 },
						entityRefs: { domAction: "bp-ref://element/item-1" },
					},
					{
						index: 1,
						selector: ".item",
						...
						actionConfidence: "medium",
						point: { x: 20, y: 20 },
						entityRefs: { domAction: "bp-ref://element/item-2" },
					},
				],
			},
		}),
		{ entityContext: { observationId: "obs-actions", capturedAt: 10 } },
	).entities;
	...
	assert.deepEqual(
		unsafeDuplicate.map((collection) => collection.containerName),
		["Cards (1)", "Cards (2)"],
	);
	assert.deepEqual(
		unsafeDuplicate.map((collection) => collection.containerNameSource),
		["disambiguated", "disambiguated"],
	);
	...
	assert.deepEqual(
		observeDuplicateCollections.map((collection) => collection.containerName),
		["筛选 (flex) (1)", "筛选 (flex) (2)"],
	);
	assert.equal(
		new Set(observeDuplicateCollections.map((collection) => collection.containerName)).size,
		observeDuplicateCollections.length,
	);
	assert.deepEqual(
		observeDuplicateCollections.map((collection) => collection.containerNameSource),
		["disambiguated", "disambiguated"],
	);
});
...
test("ABML entity builders handle malformed inputs, fallback roles, refs, and dedupe keys", () => {
	...
	const action = buildDomEntityFromScanActionable(
		{
			tag: "a",
			index: "2",
			text: "Open",
			value: "href",
			rect: { x: 1.2, y: 2.8, width: 10.2, height: 20.1 },
			current: "page",
			hitOk: false,
			occluderSelector: "#modal",
			controlsSelectors: ["#panel", "", 1],
			ownsSelectors: ["#owned"],
			expandedTargetSelectors: ["#expanded"],
			backendNodeIdBootstrap: { from: "snapshot" },
			inputKind: "search",
		},
		context,
	);
	...
	const noisy = buildDomEntityFromScanActionable(
		{
			tag: "button",
			role: "button",
			action: '<path d="M0 0 L1 1" />',
			label: "",
			text: "",
			displayLabel: "Open menu",
		},
		context,
	);
	assert.equal(noisy.entity.name, "Open menu");
	assert.deepEqual(
		noisy.entity.locators?.filter((locator) => locator.by === "textAnchor"),
		[{ by: "textAnchor", value: "Open menu", role: "button", exact: false }],
	);
	assert.equal(noisy.descriptor.semantic?.name, "Open menu");
	const unnamedIcon = buildDomEntityFromScanActionable(
		{ tag: "button", role: "button", action: '<svg><path d="M0 0" /></svg>', label: "", text: "" },
		context,
	);
	...
	const offscreenVision = buildVisionRegionFromCanvasActionable(
		{ rect: { x: 0, y: 500, w: 20, h: 10 }, point: { x: 10, y: 199 }, inViewport: false },
		context,
	);
...
test("ABML query priority oracle prefers user-facing semantics over selector-like and generated candidates", () => {
	const context = {
		observationId: "obs-query-priority",
		capturedAt: 2_000,
	...
	assert.deepEqual(
		button.entity.locators?.filter((locator) => locator.by === "textAnchor"),
		[{ by: "textAnchor", value: "Submit order", role: "button", exact: false }],
	);
	assert.equal(
		button.entity.locators?.some((locator) => locator.by === "textAnchor" && locator.value === longPreview),
		false,
	);
	assert.equal(
		button.entity.locators?.some(
			(locator) => locator.by === "textAnchor" && String(locator.value).includes("css-1abc23"),
		),
		false,
	);
	assert.equal(
		button.entity.locators?.some(
			(locator) => locator.by === "textAnchor" && String(locator.value).includes("<svg"),
		),
		false,
	);
	...
	const labelledList = buildRegionEntityFromListHint(
		{
			...
			firstItemPreview:
				"Invoice #1234 Total $99.00 Due tomorrow Owner Finance Department Status Pending Region West",
		},
		context,
		0,
	);
	...
	assert.deepEqual(
		labelledList.entity.locators?.filter((locator) => locator.by === "textAnchor"),
		[
			{
				by: "textAnchor",
				value: "Invoice #1234 Total $99.00 Due tomorrow Owner Finance Department Status Pending Region West",
				role: "list",
				exact: false,
			},
		],
	...
	assert.equal(
		firstSafeSemanticText(["button.css-1abc23 > svg > path", '<svg><path d="M0 0" /></svg>', "Open settings"]),
		"Open settings",
	);
});
...
test("ABML ref stability oracle ignores runtime locators when concise semantic anchors are available", () => {
	...
	assert.notEqual(
		first,
		stableRefIdForDescriptor({
			...base,
			owner: { ...base.owner, browserSessionId: "session-2" },
			semantic: anchoredSemantic,
			locators: [],
		}),
	);
	assert.notEqual(
		...
		stableRefIdForDescriptor({
			...base,
			documentEpoch: { ...base.documentEpoch, pageEpoch: "page-2" },
			semantic: anchoredSemantic,
			locators: [],
		}),
	);
...
test("ABML AX helpers cover malformed nodes, structure properties, relation anchors, and merge ambiguity", () => {
	...
	const built = buildAxEntityFromNode(
		node,
		{ observationId: "obs-ax", capturedAt: 10, url: "https://example.test/page", tabId: 2 },
		{ box: { x: 0, y: 0, w: 10, h: 20 }, point: { x: 5, y: 10 } },
	);
	...
	const dom = [
		entity("bp-ref://dom/1", { role: "button", name: "Save", geometry: { point: { x: 100, y: 100 } } }),
		entity("bp-ref://dom/2", { role: "button", name: "Save", geometry: { point: { x: 101, y: 101 } } }),
	];
	const ambiguousAx = buildAxEntityFromNode(
		{ role: "button", name: "Save" },
		{ observationId: "obs-ax", capturedAt: 10 },
	);
	...
	assert.deepEqual(ambiguousSemanticMerge.diagnostics.skipped, {
		ambiguousBackend: 0,
		ambiguousGeometry: 0,
		ambiguousSemantic: 1,
	...
	const ambiguousBackendMerge = mergeDomAndAxEntities(
		...
		[
			buildAxEntityFromNode(
				{ role: "button", name: "Save", backendDOMNodeId: 88 },
				{ observationId: "obs-ax", capturedAt: 10 },
			),
		],
	);
	...
	const backendMerged = mergeDomAndAxEntities(
		[
			entity("bp-ref://dom/backend", {
				role: "button",
				name: "Old",
				actionability: { actions: ["click"], confidence: "medium" },
				scope: { key: "#feed > article", position: 2, size: 10 },
				locators: [
					{ by: "backendNodeId", value: 99 },
					{ by: "css", value: "#save" },
				],
				hints: { selector: "#save" },
				state: { ...entity("x").state, pressed: false, current: "page" },
			}),
		],
		[
			buildAxEntityFromNode(
				{
					...
					properties: [
						...
						{ name: "current", value: { value: "step" } },
						{ name: "setsize", value: { value: 6 } },
					],
				},
				{ observationId: "obs-ax", capturedAt: 10 },
			),
	...
	assert.deepEqual(backendMerged.merged[0]!.locators, [
		{ by: "backendNodeId", value: 99 },
		{ by: "css", value: "#save" },
	]);
	...
	const stateNode = buildAxEntityFromNode(
		{
			role: "option",
			name: "Filter",
	...
	const unsafeNode = buildAxEntityFromNode(
		{ role: "button", name: '<svg><path d="M10 10" /></svg>', backendDOMNodeId: 102 },
		{ observationId: "obs-ax", capturedAt: 10 },
	);
	assert.equal(unsafeNode.entity.name, undefined);
	assert.equal(unsafeNode.entity.hints?.unsafeNameSkipped, true);
	const unsafeMerge = mergeDomAndAxEntities(
		[
			entity("bp-ref://dom/unsafe", {
				locators: [{ by: "backendNodeId", value: 102 }],
				hints: { selector: "#unsafe" },
			}),
		],
		[unsafeNode],
	);
...
test("ABML relations materialize fallbacks, paint-order occlusion, graph dedupe, and summaries", () => {
	const source = entity("bp-ref://control/source", {
		locators: [{ by: "backendNodeId", value: 1, targetId: "target-a" }],
		hints: {
			targetId: "target-a",
			selector: "#source",
			backendNodeId: 1,
			currentContainerKeys: ["b:404", "s:#nav"],
			controlsSelectors: ["#panel"],
			occluderSelector: "#overlay",
		},
		state: { ...entity("x").state, current: "page", occluded: true },
	});
	...
	const overlay = entity("bp-ref://region/overlay", {
		kind: "region",
		role: "dialog",
		hints: { selector: "#overlay" },
	});
	...
	assert.equal(
		materialized.entities[0]!.relations?.some(
			(relation) => relation.type === "currentIn" && relation.targetRef === "bp-ref://region/nav",
		),
		true,
	);
	assert.equal(
		materialized.entities[0]!.relations?.some(
			(relation) => relation.type === "controls" && relation.targetRef === "bp-ref://region/panel",
		),
		true,
	);
	const paintEntities = [
		entity("bp-ref://paint/lower", {
			locators: [{ by: "backendNodeId", value: 10 }],
			geometry: { box: { x: 0, y: 0, w: 100, h: 100 } },
		}),
		entity("bp-ref://paint/upper-near", {
			locators: [{ by: "backendNodeId", value: 11 }],
			geometry: { box: { x: 5, y: 5, w: 90, h: 90 } },
		}),
		entity("bp-ref://paint/upper-far", {
			locators: [{ by: "backendNodeId", value: 12 }],
			geometry: { box: { x: 500, y: 500, w: 10, h: 10 } },
		}),
		entity("bp-ref://paint/text", {
			kind: "text",
			role: "StaticText",
			locators: [{ by: "backendNodeId", value: 13 }],
		}),
	];
	...
	const relationRich = addEntityRelations(entity("bp-ref://control/rich"), [
		...
		...Array.from({ length: 10 }, (_, index) => ({
			type: "labelledBy" as const,
			targetRef: `bp-ref://label/${index}`,
			source: "ax" as const,
			confidence: "high" as const,
		})),
	]);
	assert.equal(relationRich.relations?.length, 12);
	assert.deepEqual(relationRich.relations?.[0], {
		type: "controls",
		targetRef: "bp-ref://target/a",
		source: "ax",
		confidence: "high",
		evidence: { ax: true },
	});
	const summary = buildRelationSummary([
		relationRich,
		entity("bp-ref://cell/1", {
			relations: [{ type: "cellOf", targetRef: "bp-ref://table/1", source: "ax", confidence: "high" }],
		}),
	]);
	assert.equal(summary.summary.controls, 1);
	assert.equal(summary.summary.tableCells, 1);
	assert.equal(
		summary.highlights.some((highlight) => highlight.type === "cellOf"),
		false,
	);
});

function projectionKey(containerRole: string, containerName: string, itemRole: string, declaredTotal: number): string {
	return [undefined, containerRole, containerName, itemRole, `total:${declaredTotal}`, undefined]
...
test("capture template helpers escape inline JSON and fail closed for missing placeholders", () => {
	...
	assert.equal(
		jsonForInlineScript("<tag>&line\u2028next\u2029>"),
		'"\\u003Ctag\\u003E\\u0026line\\u2028next\\u2029\\u003E"',
	);
...
test("scan script builder clamps options and injects scan helper blocks deterministically", () => {
	...
	assert.doesNotMatch(
		source,
		/content\.tree|tree\s*:\s*content|function walk\(|iframeNotes|includeIframes|growthProbe|collectVisibleRows|collectMediaCandidates|mediaCandidates|\binteractive\s*:|window\.scrollTo|\.scrollTop\s*=/,
	);
	assert.match(
		source,
		/selected\s*:\s*selectedState.*pressed\s*:\s*pressedAttr===?["']true["'].*expanded\s*:\s*expandedAttr===?["']true["']/,
	);
	assert.match(source, /documentRect\s*:\s*visible\.documentRect/);
	assert.doesNotMatch(
		source,
		/list_hints|canvas_regions|media_candidates|node_count|iframe_notes|controls_pairs|text_only/,
	);
	for (const helper of [
		"safePreviewOf",
		"conciseContainerLabel",
		"headingLabelNear",
		"containerLabelOf",
	...
	assert.equal(
		transformHelpers.every((name) => name === "__name"),
		true,
	);
...
function scanRoleOf(
	...
	return Function(
		"BrowserPilotDomAccessibilityApi",
		"__name",
		`${source.slice(start, end)}\nreturn roleOf;`,
	)(provider, (value: unknown) => value) as (el: {
		tagName: string;
		type?: string;
		multiple?: boolean;
		size?: number;
...
test("scan role provider covers representative implicit roles without widening no-role anchors", () => {
	const roleOf = scanRoleOf({
		getRole: (el: { attrs?: Record<string, string> }) => el.attrs?.["data-provider-role"] ?? null,
	});
	...
	assert.equal(
		roleOf(roleFixture("INPUT", { list: "choices", "data-provider-role": "combobox" }, { type: "text" })),
		"combobox",
	);
