...
test("tab session sync normalizes tabs, preserves partial fields, and disconnects stale entries", () => {
	...
	assert.equal(
		router.getTabs({ includeDisconnected: true }).find((tab) => tab.tabId === 7)?.disconnectedAt !== undefined,
		true,
	);
});
...
test("tab replacement preserves stable handles, advances target generation, and resolves nested or numeric stale targets", () => {
	...
	router.updateTabs(
		[
			{
				id: 7,
				url: "https://replace.test/",
				title: "Before",
				active: true,
				windowId: 1,
				pageEpoch: "page-before",
	...
	const replacements = router.applyTabReplacements(
		[
			{ from: 7, to: 9 },
			{ from: 0, to: 2 },
			{ from: 9, to: 9 },
	...
	router.updateTabs(
		[
			{
				id: 9,
				url: "https://replace.test/after",
				title: "After",
				active: true,
				windowId: 1,
				pageEpoch: "page-after",
...
test("Prerender2 same-tab activation advances target generation exactly once", () => {
	...
	assert.equal(
		router.applyTabReplacements([{ from: 11, to: 11, at: 101, kind: "prerender-activation" }], ws, 101).length,
		1,
	);
	assert.equal(
		router.applyTabReplacements([{ from: 11, to: 11, at: 101, kind: "prerender-activation" }], ws, 102).length,
		0,
	);
	router.updateTabs(
		[
			{
				id: 11,
				url: "https://prerender.test/target",
				title: "Target",
				active: true,
				pageEpoch: "page-target",
...
test("same-extension reconnect adopts identity and migrates session selection", () => {
	...
	router.updateTabs(
		[
			{
				id: 7,
				url: "https://reconnect.test/",
				title: "Page",
				active: true,
				windowId: 3,
				pageEpoch: "worker-1:page:1",
			},
		],
		previous,
	);
...
test("extension-owned tab identity changes invalidate the old handle immediately on reconnect", () => {
	...
	router.updateTabs(
		[{ id: 7, tabIdentity: "11111111111111111111111111111111", url: "https://reload.test/", active: true }],
		previous,
	);
	...
	router.updateTabs(
		[{ id: 7, tabIdentity: "22222222222222222222222222222222", url: "https://reload.test/", active: true }],
		reloaded,
	);
	...
	assert.throws(
		() => router.resolveTargetRef(oldHandle),
...
test("extension-owned tab identity preserves targetRef across daemon replacement", () => {
	...
	replacement.router.updateTabs(
		[{ id: 9, tabIdentity, url: "https://stable.test/after", active: true }],
		replacementSocket,
	);
