...
test("bridge-down when the server is not listening", () => {
	...
	assert.equal(
		deriveBridgeReadiness({ running: false, extensionConnected: true, connectedClients: 1 }),
		"bridge-down",
	);
});
...
test("a connected stale extension is not ready", () => {
	assert.equal(
		deriveBridgeReadiness({ running: true, extensionConnected: true, extensionStale: true }),
		"extension-stale",
	);
});
...
test("connecting when a socket is open but ext_ready has not arrived", () => {
	assert.equal(
		deriveBridgeReadiness({ running: true, extensionConnected: false, connectedClients: 1 }),
		"connecting",
	);
});
