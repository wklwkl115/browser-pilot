...
function newPending(): BrowserBridgePendingRequests {
	return new BrowserBridgePendingRequests(
		() => ({}),
		(target) => target,
	);
}
...
test("request cancellation preserves whether browser dispatch was acknowledged", async () => {
	...
	await assert.rejects(
		pr.send(ws, "never-send", { tabId: 7, timeoutMs: 5_000, signal: beforeSend.signal }),
...
test("resolved requests include bounded latency diagnostics without payload preview", async () => {
	const pr = newPending();
	const ws = fakeSocket();
	const promise = pr.send(
		ws,
		{ cmd: "network.list", url: "https://secret.test/?token=hidden" },
		{ tabId: 1, timeoutMs: 5_000 },
	);
	const id = lastId(ws);
	pr.ack(id, ws);
	await delay(2);
...
test("timeout diagnostics select queue depth by browser and tab", () => {
	...
	const diagnostics = buildBridgeTimeoutDiagnostics(snapshot, 7, 1_000, false, {
		browserId: "browser-b",
		tabId: 7,
