...
async function connectExtension(
	...
	while (
		!server
			...
			.clients.some(
				(client) =>
					client.extensionInstanceId === expectedInstanceId && client.workerBootId === expectedWorkerBootId,
			)
	) {
...
test("current extension readiness waits past a stale peer and promotes the current build", async (t) => {
	...
	await withServer(async (server) => {
		const stale = await connectExtension(server, [{ id: 7, url: "https://stale.test/", active: true }], {
			extensionInstanceId: "stale-instance",
			buildId: "stale-build",
		});
		...
		let settled = false;
		const ready = server.waitForExtensionReady(undefined, 1_000).then((value) => {
			settled = true;
			return value;
		});
		await new Promise((resolve) => setImmediate(resolve));
		...
		const current = await connectExtension(server, [{ id: 9, url: "https://current.test/", active: true }], {
			extensionInstanceId: "current-instance",
		});
...
test("BrowserBridgeServer advances to the next configured port when the requested port is busy", async () => {
	...
	try {
		...
		const health = await fetch(`http://${server.host}:${server.port}/health`).then(
...
test("BrowserBridgeServer command runtime port exposes snapshot, tabs and perception state", async () => {
	await withServer(async (server) => {
		...
		const observation = port.createObservationSnapshot({
		...
		assert.equal(
			port.listObservationSnapshots().some((item) => item.snapshotId === observation.snapshotId),
			true,
		);
		...
		ws.send(
			JSON.stringify({
				type: "tabs_update",
				tabs: [{ id: 8, url: "https://example.test/replaced", active: true }],
				replaced: [{ from: 7, to: 8 }],
			}),
		);
		await new Promise((resolve) => setImmediate(resolve));
...
test("BrowserBridgeServer ignores tab state before the extension handshake", async () => {
	await withServer(async (server) => {
		...
		ws.send(
			JSON.stringify({ type: "tabs_update", tabs: [{ id: 99, url: "https://untrusted.test/", active: true }] }),
		);
		await new Promise((resolve) => setImmediate(resolve));
		...
		ws.send(
			JSON.stringify({
				...
				bridge: {
					id: "bridge-1",
					extensionInstanceId: "instance-pre-ready",
					workerBootId: "worker-1",
					build: { buildId: readExpectedExtensionBuild().buildId },
				},
...
test("BrowserBridgeServer rejects a different browser instance after ownership is established", async () => {
	await withServer(async (server) => {
		const owner = await connectExtension(server, [{ id: 7, url: "https://owner.test/", active: true }], {
			extensionInstanceId: "owner-instance",
			workerBootId: "owner-worker",
		});
		const contender = await openExtension(server, [{ id: 9, url: "https://contender.test/", active: true }], {
			extensionInstanceId: "contender-instance",
			workerBootId: "contender-worker",
		});
		...
		assert.equal(
			server.getTabs().some((tab) => tab.url === "https://contender.test/"),
			false,
		);
		owner.close();
	});
...
test(
	"BrowserBridgeServer lets a new worker socket replace the same browser instance",
	{ timeout: 7_000 },
	async (t) => {
		await useExpectedExtensionBuild(t);
		await withServer(async (server) => {
			const first = await within(
				connectExtension(server, [{ id: 7, url: "https://before.test/", active: true }], {
					extensionInstanceId: "same-instance",
					workerBootId: "worker-1",
				}),
				"first extension connect",
			);
			const firstClosed = new Promise<number>((resolve) => first.once("close", resolve));
			const replacement = await within(
				connectExtension(server, [{ id: 8, url: "https://after.test/", active: true }], {
					extensionInstanceId: "same-instance",
					workerBootId: "worker-2",
				}),
				"replacement extension connect",
			);
			...
			assert.equal(
				server.getTabs().some((tab) => tab.url === "https://after.test/"),
				true,
			);
			const stale = await within(
				openExtension(server, [], {
					extensionInstanceId: "same-instance",
					workerBootId: "worker-old",
					buildId: "stale-build",
				}),
				"stale extension connect",
			);
