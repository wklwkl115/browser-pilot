...
type EvalPlan = {
	value?: JsonRecord;
	exception?: string;
	downloadEvent?: JsonRecord;
	downloadItem?: DownloadItem;
	fileChooser?: JsonRecord;
};
		...
		download(options: JsonRecord, callback: (id?: number) => void) {
			...
			const item = {
				...completedDownload(
					id,
					String(options.url),
					typeof options.filename === "string" ? `C:\\Downloads\\${options.filename}` : undefined,
				),
				danger: nextDownloadDanger,
			};
...
const cdpBridge = {
	async send(tabId: number, method: string, params: JsonRecord = {}, options: JsonRecord = {}) {
		...
		if (plan.exception)
			return { ok: true, data: { result: { exceptionDetails: { exception: { description: plan.exception } } } } };
...
test("transfer download completes direct URL and selector media paths", async () => {
	reset();
	const direct = await transfer.handleBrowserPilotTransferCommand("transfer.download", 7, {
	...
	const media = await transfer.handleBrowserPilotTransferCommand("transfer.download", 7, {
		selector: "img.hero",
		mode: "media",
		timeoutMs: 500,
	});
	...
	assert.deepEqual((media.data as JsonRecord).mimeMismatch, {
		expected: "image",
...
test("transfer click uses extracted HTTP URL without arming a page-event wait", async () => {
	...
	const result = await transfer.handleBrowserPilotTransferCommand("transfer.download", 7, {
		selector: "a.download",
		mode: "click",
		timeoutMs: 500,
	});
...
test("transfer download fails immediately when Chrome requires confirmation", async () => {
	...
	const result = await transfer.handleBrowserPilotTransferCommand("transfer.download", 7, {
		url: "https://example.test/uncommon.exe",
		timeoutMs: 500,
	});
...
test("transfer click matches the tab CDP event to a completed Chrome download", async () => {
	...
	evalPlans.push(
		...
		{
			value: { clicked: true, selector: "button.export", index: 0 },
			downloadItem: item,
			downloadEvent: { url: item.url, guid: "guid-240", suggestedFilename: "generated.csv" },
		},
	);
	const result = await transfer.handleBrowserPilotTransferCommand("transfer.download", 7, {
...
test("transfer download preserves selector errors and ambiguous URL-less page events", async () => {
	...
	const invalid = await transfer.handleBrowserPilotTransferCommand("transfer.download", 7, {
		selector: "[",
		timeoutMs: 500,
	});
	...
	evalPlans.push(
		{ value: { href: "" } },
		{
			...
			downloadEvent: { guid: "guid-250", suggestedFilename: "candidate.bin" },
		},
	);
	const ambiguous = await transfer.handleBrowserPilotTransferCommand("transfer.download", 7, {
...
test("transfer upload binds the chooser backend node and always disables interception", async () => {
	reset();
	evalPlans.push({
		value: { clicked: true, selector: "input[type=file]", index: 0 },
		fileChooser: { backendNodeId: 55, mode: "selectMultiple" },
	});
	const result = await transfer.handleBrowserPilotTransferCommand("transfer.upload", 7, {
		selector: "input[type=file]",
		files: ["C:\\fixtures\\a.txt", "C:\\fixtures\\b.txt"],
		timeoutMs: 500,
	});
	assert.equal(result.ok, true);
	...
	assert.deepEqual(
		cdpCalls
			.filter((call) => call.method === "Page.setInterceptFileChooserDialog")
