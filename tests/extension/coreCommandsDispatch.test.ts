...
const tabUpdatedListeners: Array<
	(tabId: number, changeInfo: Record<string, unknown>, tab: Record<string, unknown>) => void
> = [];
...
const runtimeMessageListeners: Array<
	(message: unknown, sender: Record<string, unknown>, sendResponse: (response: unknown) => void) => boolean
> = [];
...
async function debuggerCommandResult(_debuggee: Debuggee, method: string): Promise<unknown> {
	...
	if (method === "Page.addScriptToEvaluateOnNewDocument")
		return { identifier: `new-document-${++newDocumentScriptSeq}` };
...
const chromeStub = {
	runtime: {
		...
		async sendMessage(message: unknown) {
			...
			if (
				...
				(message as Record<string, unknown>).type === "browser-pilot-offscreen-send"
			)
	...
	offscreen: {
		async hasDocument() {
			return offscreenHasDocumentOverride ? await offscreenHasDocumentOverride() : true;
		},
	...
	tabs: {
		...
		async sendMessage(_tabId: number, message: unknown, options?: Record<string, unknown>) {
			...
			if (record.cmd === "browserPilot.contentFingerprint")
				return {
					ok: true,
					data: {
						changeSeq: 1,
		...
		onCreated: {
			addListener(listener: (tab: Record<string, unknown>) => void) {
				tabCreatedListeners.push(listener);
			},
			removeListener(listener: (tab: Record<string, unknown>) => void) {
				const index = tabCreatedListeners.indexOf(listener);
				if (index >= 0) tabCreatedListeners.splice(index, 1);
			},
		},
		onUpdated: {
			addListener(
				listener: (tabId: number, changeInfo: Record<string, unknown>, tab: Record<string, unknown>) => void,
			) {
		...
		onReplaced: {
			...
			removeListener(listener: (addedTabId: number, removedTabId: number) => void) {
				const index = tabReplacedListeners.indexOf(listener);
				if (index >= 0) tabReplacedListeners.splice(index, 1);
			},
		},
		onActivated: {
			...
			removeListener(listener: (info: { tabId: number; windowId: number }) => void) {
	...
	downloads: {
		onCreated: {
			addListener(listener: (item: Record<string, unknown>) => void) {
				downloadCreatedListeners.push(listener);
			},
			removeListener(listener: (item: Record<string, unknown>) => void) {
				const index = downloadCreatedListeners.indexOf(listener);
				if (index >= 0) downloadCreatedListeners.splice(index, 1);
			},
		},
		onChanged: {
			addListener(listener: (delta: Record<string, unknown>) => void) {
				downloadChangedListeners.push(listener);
			},
			removeListener(listener: (delta: Record<string, unknown>) => void) {
				const index = downloadChangedListeners.indexOf(listener);
				if (index >= 0) downloadChangedListeners.splice(index, 1);
			},
		},
	},
	webNavigation: Object.fromEntries(
		["onCommitted", "onCompleted", "onHistoryStateUpdated", "onErrorOccurred"].map((name) => [
			name,
			{
				addListener(listener: (details: Record<string, unknown> & { tabId?: number; url?: string }) => void) {
					navigationListeners.push(listener);
				},
				removeListener(
					listener: (details: Record<string, unknown> & { tabId?: number; url?: string }) => void,
				) {
...
test("core command direct and batch dispatch share supported command handlers", async () => {
	...
	const batch = await coreCommands.handleBatch(
		{ cmd: "batch", commands: [{ cmd: "tabs", method: "list" }] },
		sender(),
	);
...
test("core commands expose only extension reload and target fingerprint reads to the top frame", async () => {
	for (const command of ["cookies", "contentSettings"]) {
		...
		assert.equal(
			(await coreCommands.dispatchBrowserPilotBridgeCommand({ cmd: command }, sender())).error_code,
			"INVALID_RULE",
		);
	}
	assert.equal(
		(await coreCommands.dispatchBrowserPilotBridgeCommand({ cmd: "management", method: "reload" }, sender())).ok,
		true,
	);
	assert.equal(
		(await coreCommands.dispatchBrowserPilotBridgeCommand({ cmd: "management", method: "list" }, sender()))
			.error_code,
		"INVALID_RULE",
	);
	assert.equal(
		(await coreCommands.dispatchBrowserPilotBridgeCommand({ cmd: "content.fingerprint", tabId: 7 }, sender())).ok,
		true,
	);
...
test("core command batch cdp preserves result parameter substitution", async () => {
	const previousPersistentCdp = (globalThis as typeof globalThis & { BrowserPilotPersistentCdp?: unknown })
		.BrowserPilotPersistentCdp;
	const previousPersistentCdpBridge = (
		globalThis as typeof globalThis & { browserPilotPersistentCdpBridge?: unknown }
	...
	try {
		...
		assert.deepEqual(
			...
			[{ expression: "location.href" }, { argument: "location.href" }],
		);
		assert.deepEqual(
		...
		(globalThis as typeof globalThis & { BrowserPilotPersistentCdp?: unknown }).BrowserPilotPersistentCdp =
			previousPersistentCdp;
		(
			globalThis as typeof globalThis & { browserPilotPersistentCdpBridge?: unknown }
		).browserPilotPersistentCdpBridge = previousPersistentCdpBridge;
	}
});
	...
	assert.deepEqual(messages, [
		{
			type: "error",
			id: "empty-cmd",
			error: 'Message object must contain a non-empty "cmd" field',
			details: { codeType: "object", dispatchStarted: false, acked: false },
		},
		{
			type: "error",
			id: "bad-code",
			error: "Unsupported message code type: number",
			details: { codeType: "number", dispatchStarted: false, acked: false },
		},
	]);
});
...
test("extension websocket router rejects native command validation errors before ACK", async () => {
	...
	await router.handleBrowserPilotBridgeWsMessage(
		{ id: "native-invalid", tabId: 7, code: JSON.stringify({ cmd: "input.pointer" }) },
		socket,
	);
	await router.handleBrowserPilotBridgeWsMessage(
		{ id: "timeout-invalid", tabId: 7, timeoutMs: -1, code: { cmd: "tabs", method: "list" } },
		socket,
	);
...
test("extension websocket router returns a bounded error instead of overflowing the bridge", () => {
	...
	router.sendBrowserPilotBridgeWsCommandResult(
		socket,
		"large",
		{ cmd: "network.exportHar" },
		{
			ok: true,
...
test("offscreen transport preserves ext_ready, ACK, and result order", async () => {
	...
	try {
		const firstConnect = transport.handleBrowserPilotOffscreenMessage({
			type: "browser-pilot-offscreen-connected",
			port: 9333,
		});
		await recoveryStarted;
		const secondConnect = transport.handleBrowserPilotOffscreenMessage({
			type: "browser-pilot-offscreen-connected",
			port: 9334,
		});
		...
		await transport.handleBrowserPilotOffscreenMessage({
			type: "browser-pilot-offscreen-ws-message",
			port: 9333,
			data: { id: "ordered", code: { cmd: "tabs", method: "list" } },
		});
			...
			sentRuntimeMessages
				.filter(
					(message): message is Record<string, unknown> =>
						...
						typeof message === "object" &&
						(message as Record<string, unknown>).type === "browser-pilot-offscreen-send" &&
						(message as Record<string, unknown>).port === 9333,
				)
		...
		assert.deepEqual(
			envelopes().map((message) => message.type),
			["ext_ready", "ack", "result"],
		);
	} finally {
		releaseRecovery();
		await transport.handleBrowserPilotOffscreenMessage({
...
test("offscreen transport drops queued sends from a superseded connection", async () => {
	...
	const firstCheckReleased = new Promise<void>((resolve) => {
		...
		const ready = sentRuntimeMessages
			.filter(
				(message): message is Record<string, unknown> =>
					...
					typeof message === "object" &&
					(message as Record<string, unknown>).type === "browser-pilot-offscreen-send" &&
					(message as Record<string, unknown>).port === 9444,
			)
			.map((message) => JSON.parse(String(message.data)) as Record<string, unknown>)
...
test("extension runtime helpers keep error content verbatim while breaking cycles", () => {
	...
	assert.deepEqual(
		runtimeSupport.normalizeBridgeResponse(
			{ ok: false, error: { code: "NO_SESSION", message: "missing session", details: { tabId: 7 } } },
			"hook.read",
		),
		{
			ok: false,
	...
	assert.deepEqual(
		runtimeSupport.normalizePersistentBrowserPilotResponse({
			ok: false,
			error: { code: "CDP_TIMEOUT", message: "wait expired", details: { timeoutMs: 5 } },
		}),
		{
			ok: false,
...
test("extension runtime state serializes its shared storage blob and reports previous-boot state", async () => {
	...
	await stateStore.persist(
		...
		{ tabId: 7, sessionId: "lost", recoveryPolicy: "diagnosticOnly" },
	);
	...
	await Promise.all([
		...
		stateStore.persist(
			...
			"7:during-recovery",
			{ active: true },
			{ tabId: 7, sessionId: "during-recovery" },
		),
	]);
	assert.equal(storageWriteConcurrency.max, 1);
	storageWriteConcurrency = undefined;
...
test("tab sync coalesces concurrent events without letting an older snapshot overtake", async () => {
	...
	assert.deepEqual(
		sent.map((message) => (message.tabs as Array<Record<string, unknown>>)[0]?.title),
		["Old", "New"],
	);
});
...
test("persistent CDP evaluates one-off scripts directly, then compiles hot scripts and falls back from stale cache entries", async () => {
	...
	const first = await cdpCommands.browserPilotPersistentCdpBridge.send(
		71,
		"Runtime.evaluate",
		{ expression: "21 * 2", returnByValue: true },
		options,
	);
	const second = await cdpCommands.browserPilotPersistentCdpBridge.send(
		71,
		"Runtime.evaluate",
		{ expression: "21 * 2", returnByValue: true },
		options,
	...
	assert.deepEqual(
		debuggerCommands.map((call) => call.method),
		["Runtime.evaluate", "Runtime.compileScript", "Runtime.runScript"],
	);
	...
	const stale = await cdpCommands.browserPilotPersistentCdpBridge.send(
		71,
		"Runtime.evaluate",
		{ expression: "21 * 2", returnByValue: true },
		options,
	...
	assert.deepEqual(
		debuggerCommands.slice(-2).map((call) => call.method),
		["Runtime.runScript", "Runtime.evaluate"],
	);
...
test("persistent CDP coalesces concurrent compilation after a script becomes hot", async () => {
	...
	await cdpCommands.browserPilotPersistentCdpBridge.send(
		70,
		"Runtime.evaluate",
	...
	const results = await Promise.all(
		Array.from({ length: 3 }, () =>
			cdpCommands.browserPilotPersistentCdpBridge.send(
				70,
				"Runtime.evaluate",
				{ expression: "40 + 2", returnByValue: true },
				options,
...
test("new-document scripts are bounded and forgotten on debugger detach", async () => {
	...
	for (let index = 0; index < 32; index += 1) {
		const added = await cdpCommands.browserPilotPersistentCdpBridge.addNewDocumentScript(
			72,
			`globalThis.fixture${index} = true`,
			{ persistent: true, name: "new_document" },
		);
		assert.equal(added.ok, true);
	}
	const limited = await cdpCommands.browserPilotPersistentCdpBridge.addNewDocumentScript(
		72,
		"globalThis.overflow = true",
		{ persistent: true, name: "new_document" },
	);
	assert.equal(limited.ok, false);
	assert.equal((limited.error as { code?: string }).code, "SCRIPT_LIMIT");
	const oversized = await cdpCommands.browserPilotPersistentCdpBridge.addNewDocumentScript(
		73,
		"x".repeat(256 * 1024 + 1),
		{ persistent: true, name: "new_document" },
	);
...
test("persistent CDP coalesces concurrent tab attaches without eager domain round trips", async () => {
	...
	const [first, second] = await Promise.all([
		cdpCommands.browserPilotPersistentCdpBridge.send(
			...
			"Runtime.evaluate",
			{ expression: "1" },
			{ name: "first", persistent: true },
		),
		cdpCommands.browserPilotPersistentCdpBridge.send(
			76,
			"Runtime.evaluate",
			{ expression: "2" },
			{ name: "second", persistent: true },
		),
	]);
...
test("CDP domain arming runs independent domain round trips concurrently", async () => {
	...
	const record = {
		key: "wait:parallel-domains",
		waitId: "parallel-domains",
		kind: "wait",
		tabId: 81,
...
test("persistent CDP coalesces per-session focus setup for concurrent evaluation", async () => {
	...
	const [first, second] = await Promise.all([
		cdpCommands.browserPilotPersistentCdpBridge.send(
			...
			"Runtime.evaluate",
			{ expression: "1" },
			{ name: "focused", persistent: true, focusEmulation: true },
		),
		cdpCommands.browserPilotPersistentCdpBridge.send(
			78,
			"Runtime.evaluate",
...
test("persistent CDP keeps a concurrent temporary attachment until the final command settles", async () => {
	...
	const [first, second] = await Promise.all([
		cdpCommands.browserPilotPersistentCdpBridge.send(
			...
			"Runtime.evaluate",
			{ expression: "1" },
			{ name: "temporary-a", persistent: false },
		),
		cdpCommands.browserPilotPersistentCdpBridge.send(
			79,
			"Runtime.evaluate",
			{ expression: "2" },
			{ name: "temporary-b", persistent: false },
		),
	]);
...
test("persistent CDP enables Page lazily once for repeated frame-tree reads", async () => {
	...
	const second = await cdpCommands.browserPilotPersistentCdpBridge.frameTree(77, {
		name: "frames",
		persistent: true,
	});
	...
	assert.deepEqual(
		debuggerCommands.map((call) => call.method),
		["Page.enable", "Page.getFrameTree", "Page.getFrameTree"],
	);
});
...
test("persistent CDP re-enables a required domain after an explicit disable", async () => {
	...
	assert.deepEqual(
		debuggerCommands.map((call) => call.method),
		["Page.enable", "Page.disable", "Page.enable", "Page.getFrameTree"],
	);
});
...
test("persistent CDP send retries detached sessions and releases temporary attachments", async () => {
	...
	const retried = await cdpCommands.browserPilotPersistentCdpBridge.send(
		72,
		"Network.enable",
		{},
		{ name: "retry", persistent: true },
	);
	...
	resetCdpFixtures();
	const temporary = await cdpCommands.browserPilotPersistentCdpBridge.send(
		73,
		"Network.enable",
		{},
		{ name: "temporary", persistent: false },
	);
...
test("persistent CDP aliases the default logical session when a named tab debugger is already attached", async () => {
	resetCdpFixtures();
	const recorder = await cdpCommands.browserPilotPersistentCdpBridge.send(
		75,
		"Network.enable",
		{},
		{ name: "network-recorder", persistent: true },
	);
	assert.equal(recorder.ok, true);
	failAlreadyAttachedOnce = true;
	const reload = await cdpCommands.browserPilotPersistentCdpBridge.send(
		75,
		"Page.reload",
		{},
	...
	assert.equal(
		debuggerCommands.some((call) => call.method === "Page.reload"),
		true,
	);
});
...
test("persistent CDP send preserves explicit child-session routing", async () => {
	resetCdpFixtures();
	const result = await cdpCommands.browserPilotPersistentCdpBridge.send(
		74,
		"Runtime.evaluate",
		{ expression: "42" },
		{ name: "child", persistent: true, sessionId: "child-session-1" },
	);
...
test("frame command dispatch normalizes list, evaluate, and new-document script operations", async () => {
	const globalCdp = globalThis as typeof globalThis & {
	...
	const bridge = {
		async frameTree(...args: unknown[]) {
			...
			return {
				ok: true,
				data: { frameTree: { frameId: "main" }, frames: [{ frameId: "main" }, { frameId: "child" }] },
			};
		},
	...
	try {
		const listed = await frameCommands.handleBrowserPilotFrameCommand("frame.list", 7, {
			cmd: "frame.list",
			options: { name: "frames" },
		});
		const evaluated = await frameCommands.handleBrowserPilotFrameCommand("frame.evaluate", 7, {
			cmd: "frame.evaluate",
			frameId: "child",
			expression: "21 * 2",
			awaitPromise: false,
		...
		const added = await frameCommands.handleBrowserPilotFrameCommand("frame.addNewDocumentScript", 7, {
			cmd: "frame.addNewDocumentScript",
			source: "globalThis.ready = true",
			runImmediately: true,
			includeCommandLineAPI: true,
			timeout_ms: 500,
		});
		const removed = await frameCommands.handleBrowserPilotFrameCommand("frame.removeNewDocumentScript", 7, {
			cmd: "frame.removeNewDocumentScript",
			identifier: "script-1",
			timeoutMs: 600,
		});
		...
		assert.deepEqual(listed, {
			...
			data: {
				tabId: 7,
				frameTree: { frameId: "main" },
				frames: [{ frameId: "main" }, { frameId: "child" }],
				count: 2,
			},
		});
		...
		assert.deepEqual(
			...
			["frameTree", "evaluateInFrame", "addNewDocumentScript", "removeNewDocumentScript"],
		);
		...
		assert.equal(
			(await frameCommands.handleBrowserPilotFrameCommand("frame.evaluate", 7, { cmd: "frame.evaluate" }))
				.error_code,
			"INVALID_RULE",
		);
		assert.equal(
			(await frameCommands.handleBrowserPilotFrameCommand("frame.unknown", 7, { cmd: "frame.unknown" }))
				.error_code,
			"INVALID_RULE",
		);
	} finally {
