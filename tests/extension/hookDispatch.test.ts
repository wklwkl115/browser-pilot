...
const cdpBridge = {
	async send(_tabId: number, method: string, params: JsonRecord) {
		...
		const value = Array.isArray(configured)
			? (configured.shift() ?? { ok: true, data: { command } })
			: (configured ?? { ok: true, data: { command } });
...
test("hook dispatch exposes static targets and tab-scoped session inventory", async () => {
	...
	assert.deepEqual(
		(targetData.targets as Array<JsonRecord>).map((item) => item.id),
		["console", "error", "networkApi", "websocket", "storage", "crypto", "dom", "domSinks", "cookies"],
	);
	...
	assert.deepEqual(
		(sessionData.sessions as Array<JsonRecord>).map((item) => item.session_id),
		["session-7"],
	);
});
...
test("hook dispatch forwards page commands with canonical session arguments", async () => {
	...
	await hook.handleBrowserPilotHookCommand("hook.collect", 7, {
	...
	assert.deepEqual(
		dispatches.map((entry) => entry.command),
		["hook.status", "hook.collect", "hook.clear_buffer", "hook.pause", "hook.resume"],
	);
	assert.deepEqual(dispatches[0]?.args, { session_id: "session-7" });
	assert.deepEqual(dispatches[1]?.args, {
		session_id: "session-7",
		since_seq: 4,
		limit: 12,
		event_types: ["console.*"],
...
test("hook dispatch keeps evaluate and unknown-command boundaries stable", async () => {
	const evaluated = await hook.handleBrowserPilotHookCommand("hook.evaluate", 7, {
		expression: "1 + 1",
		awaitPromise: true,
		timeoutMs: 500,
	});
...
test("hook install_targets expands explicit targets and records reusable session metadata", async () => {
	...
	pageResponses.set("hook.install", {
		ok: true,
		data: { session_id: "installed-7", state: "INSTALLED", idempotent: true, dispatcher_version: "1.0" },
	});
	const installed = await hook.handleBrowserPilotHookCommand("hook.install_targets", 7, {
		sessionId: "installed-7",
		targets: ["console", "ws"],
	});
	...
	assert.deepEqual(
		(data.expanded_targets as Array<JsonRecord>).map((item) => item.id),
		["console", "websocket"],
	);
	assert.deepEqual(dispatches.at(-1), {
		command: "hook.install",
		args: { session_id: "installed-7", targets: { console: true, websocket: true }, force: false },
	});
