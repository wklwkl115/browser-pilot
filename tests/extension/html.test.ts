...
test("html.get applies a bounded default and rejects attempts to exceed it", async () => {
	...
	const oversized = await html.handleBrowserPilotHtml(7, {
		cmd: "html.get",
		maxBytes: html.BROWSER_PILOT_HTML_MAX_BYTES + 1,
	});
	assert.equal(oversized.ok, false);
	assert.equal(
		validateBridgeCommand({ cmd: "html.get", tabId: 7, maxBytes: html.BROWSER_PILOT_HTML_MAX_BYTES + 1 }).ok,
		false,
	);
});
