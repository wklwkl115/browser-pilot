...
test("ws.open rejects unused headers and oversized inbound frames fail closed without parsing or storage", async () => {
	const rejected = validateBridgeCommand({
		cmd: "ws.open",
		tabId: 7,
	...
	assert.equal(
		"headers" in
			model.normalizeWsOpenConfig({ url: "wss://example.test/socket", headers: { authorization: "secret" } }),
		false,
	);

	const opening = wsRuntime.handleBrowserPilotWsCommand("ws.open", 7, {
		cmd: "ws.open",
		url: "wss://example.test/socket",
	});
	...
	assert.equal(
		transcript.some((entry) => entry.event === "message"),
		false,
	);
...
test("ws session supports send, wait, replay, collect, and explicit close", async () => {
	...
	const sent = await wsRuntime.handleBrowserPilotWsCommand("ws.send", 7, {
		cmd: "ws.send",
		sessionId: "flow",
		text: "hello",
	});
	...
	const immediate = await wsRuntime.handleBrowserPilotWsCommand("ws.wait", 7, {
		cmd: "ws.wait",
		sessionId: "flow",
		contains: "ready",
	});
	...
	const replay = wsRuntime.handleBrowserPilotWsCommand("ws.replay", 7, {
		cmd: "ws.replay",
		sessionId: "flow",
		steps: [{ text: "ping", contains: "pong", timeoutMs: 500 }],
	});
	...
	const collected = await wsRuntime.handleBrowserPilotWsCommand("ws.collect", 7, {
		cmd: "ws.collect",
		sessionId: "flow",
		limit: 20,
	});
	assert.ok(Number((collected.data as Record<string, unknown>).count) >= 5);
	const closed = await wsRuntime.handleBrowserPilotWsCommand("ws.close", 7, {
		cmd: "ws.close",
		sessionId: "flow",
		code: 1000,
		reason: "done",
