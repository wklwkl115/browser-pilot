...
const { browserPilotNetworkHandleRecorderCdpEvent } =
	await import("../../src/bridge/extension/service_worker/network_events.ts");
const { browserPilotNetworkRecorders, createNetworkRecorder, networkRecorderKey, normalizeNetworkRecorderConfig } =
	await import("../../src/bridge/extension/service_worker/network_model.ts");
const { getNetworkRecorderBody, makeHarEntry, networkWaitMatches, waitNetworkRecorder } =
	await import("../../src/bridge/extension/service_worker/network.ts");
...
test("network recorder events preserve the request-response-finish projection", () => {
	...
	const deps = {
		wakeNetworkWaits: (_recorder: typeof recorder, eventType: string) => {
			wakes.push(eventType);
		},
		maybeCaptureNetworkBody: async (_recorder: typeof recorder, record: NetworkRecord) => {
			bodyCaptures.push(record);
		},
	};
	...
	browserPilotNetworkHandleRecorderCdpEvent(
		...
		"Network.responseReceived",
		{
			requestId: "req-1",
			type: "Fetch",
			response: { status: 201, statusText: "Created", mimeType: "application/json", encodedDataLength: 9 },
		},
		deps,
	);
	browserPilotNetworkHandleRecorderCdpEvent(
		...
		{ requestId: "req-1", dataLength: 8, encodedDataLength: 9 },
		deps,
	);
	browserPilotNetworkHandleRecorderCdpEvent(
		recorder,
		...
		{ requestId: "req-1", encodedDataLength: 12 },
		deps,
	);
	...
	assert.deepEqual(
		{
			request: recorder.counters.request,
			response: recorder.counters.response,
			data: recorder.counters.data,
			finished: recorder.counters.finished,
		},
		{ request: 1, response: 1, data: 1, finished: 1 },
	);
...
test("network recorder events keep websocket, SSE, page, and failure streams bounded", () => {
	...
	browserPilotNetworkHandleRecorderCdpEvent(
		...
		{ requestId: "ws-1", url: "wss://example.test/socket" },
		deps,
	);
	browserPilotNetworkHandleRecorderCdpEvent(
		recorder,
		...
		{ requestId: "ws-1", response: { opcode: 1, payloadData: "first" } },
		deps,
	);
	browserPilotNetworkHandleRecorderCdpEvent(
		recorder,
		{},
		"Network.webSocketFrameReceived",
		{ requestId: "ws-1", response: { opcode: 1, payloadData: "second" } },
		deps,
	);
	browserPilotNetworkHandleRecorderCdpEvent(
		recorder,
		...
		{ requestId: "sse-1", eventName: "first", data: "12345" },
		deps,
	);
	browserPilotNetworkHandleRecorderCdpEvent(
		recorder,
		{},
		"Network.eventSourceMessageReceived",
		{ requestId: "sse-1", eventName: "second", data: "67890" },
		deps,
	);
	browserPilotNetworkHandleRecorderCdpEvent(
		recorder,
		...
		{ requestId: "req-fail", errorText: "blocked", canceled: true, blockedReason: "inspector" },
		deps,
	);
	browserPilotNetworkHandleRecorderCdpEvent(
		recorder,
		{},
		"Page.lifecycleEvent",
		{ frameId: "frame-1", name: "load", loaderId: "loader-1" },
		deps,
	);
	...
	assert.deepEqual(
		ws?.wsFrames.map((frame) => frame.payloadData),
		["seco"],
	);
	assert.deepEqual(
		sse?.sseEvents.map((event) => [event.eventName, event.data]),
		[["second", "6789"]],
	);
	assert.equal(failed?.phase, "failed");
	assert.equal(failed?.canceled, true);
	assert.deepEqual(
		(recorder.lifecycleEvents as Array<{ name?: unknown }>).map((event) => event.name),
		["load"],
	);
...
test("HAR projection preserves request, response, body, and recorder metadata", () => {
	...
	assert.deepEqual(
		...
		{ status: 201, redirectURL: "/next", bodySize: 20, serverIPAddress: "127.0.0.1", connection: "12" },
	);
	assert.deepEqual(
		{
			...
			seq: entry._seq,
			type: entry._type,
			redirects: entry._redirects,
			wsFrames: entry._wsFrames,
			sseEvents: entry._sseEvents,
			bodyRef: entry._bodyRef,
		},
		{
			requestId: "request-1",
			seq: 3,
...
test("HAR projection keeps stable defaults when response and body data are absent", () => {
	...
	assert.deepEqual(
		{ method: entry.request.method, url: entry.request.url, bodySize: entry.request.bodySize },
		{ method: "GET", url: "", bodySize: 0 },
	);
	assert.deepEqual(
		{ status: entry.response.status, statusText: entry.response.statusText, bodySize: entry.response.bodySize },
		{ status: 0, statusText: "", bodySize: -1 },
	);
	assert.deepEqual(entry.response.content, {
		size: -1,
		mimeType: "",
		compression: 0,
		_bodyAvailability: "not_requested",
		_bodyUnavailableReason: null,
	});
...
test("network wait matching preserves filters, body, frame, idle, count, and event aliases", () => {
	...
	recorder.bodyStore.set("body-wait", {
		bodyRef: "body-wait",
		requestId: "request-wait",
		tabId: 7,
		sessionId: "session-1",
	...
	const matches = (
		...
		eventType: string,
		candidate: NetworkRecord | null = record,
	) => networkWaitMatches(recorder, { condition, criteria, idleMs: 500, count: 1 }, eventType, candidate);
...
test("network waits resolve immediate phase, websocket, count, and idle matches", async () => {
	...
	try {
		...
		const idle = await waitNetworkRecorder(7, {
			cmd: "network.wait",
			condition: "idle",
			idleMs: 500,
			timeoutMs: 0,
		...
		assert.deepEqual(
			data.map((item) => item.event),
			["response", "websocket", "count", "idle"],
		);
...
test("network body lookup shares request resolution and preserves truncation diagnostics", async () => {
	...
	recorder.bodyStore.set("body-stored", {
		bodyRef: "body-stored",
		requestId: "request-body",
		tabId: 7,
		sessionId: "default",
	...
	browserPilotNetworkRecorders.set(networkRecorderKey(7, "default"), recorder);
	try {
		const found = await getNetworkRecorderBody(7, {
			cmd: "network.body",
