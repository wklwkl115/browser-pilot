...
function installPersistentCdp(evalValues: unknown[], options: { bindingError?: string } = {}): CdpCall[] {
	...
	runtimeGlobal.browserPilotPersistentCdpBridge = {
		async send(...args: unknown[]) {
			...
			if (callOptions.name === "selector_binding" && options.bindingError)
				return { ok: false, error: { message: options.bindingError } };
...
test("selector wait rejects missing, framed, unsupported, and invalid selectors", async () => {
	...
	assert.equal(
		(await waitSelector.waitForSelector(7, { selector: "#target", frameId: "child" })).error_code,
		"CROSS_ORIGIN_IFRAME",
	);
	assert.equal(
		(await waitSelector.waitForSelector(7, { selector: "#target", state: "shadow" })).error_code,
		"INVALID_RULE",
	);
...
test("selector wait preserves immediate match and immediate timeout projections", async () => {
	...
	const matched = await waitSelector.waitForSelector(7, {
		selector: "#ready",
		timeoutMs: 0,
		waitId: "immediate-match",
	});
	...
	const timeout = await waitSelector.waitForSelector(7, {
		selector: "#missing",
		timeoutMs: 0,
		waitId: "immediate-timeout",
	});
...
test("selector wait reacts to Runtime binding events and cleans observer state", async () => {
	const calls = installPersistentCdp([
		{ ok: true },
		{ matched: false },
		{ matched: false },
		{ matched: true, selector: "#bound" },
	]);
	const resultPromise = waitSelector.waitForSelector(7, {
		selector: "#bound",
		timeoutMs: 500,
		pollMs: 500,
		waitId: "binding-wait",
	...
	await waitUntil(
		() => debuggerEventListeners.length > 0 && calls.some((call) => call.options.name === "selector_binding"),
	);
	...
	for (const listener of [...debuggerEventListeners])
		listener({ tabId: 7 }, "Runtime.bindingCalled", {
			name: bindingName,
			payload: JSON.stringify({ reason: "binding", mutationTick: 2 }),
		});
	...
	assert.equal(
		terminal?.cdpEvents?.some((event) => (event as Record<string, unknown>).kind === "selector_binding"),
		true,
	);
});

test("selector wait falls back to polling when binding installation fails", async () => {
	installPersistentCdp(
		[{ ok: true }, { matched: false }, { matched: false }, { matched: true, selector: "#polled" }],
		{ bindingError: "binding unavailable" },
	);
	const result = await waitSelector.waitForSelector(7, {
		selector: "#polled",
		timeoutMs: 500,
		pollMs: 10,
		waitId: "poll-wait",
	...
	assert.equal(
		(terminal?.diagnostics as Array<Record<string, unknown>>).some(
			(entry) => entry.warning === "runtime_binding_observer_unavailable_poll_fallback_active",
		),
		true,
	);
});
...
test("selector wait cancellation and timeout both release registered resources", async () => {
	...
	const cancelled = await waitSelector.waitForSelector(7, {
		selector: "#cancel",
		timeoutMs: 500,
		pollMs: 10,
		waitId: "cancel-wait",
	...
	installPersistentCdp([{ ok: true }, { matched: false }], { bindingError: "binding unavailable" });
	const timedOut = await waitSelector.waitForSelector(7, {
		selector: "#timeout",
		timeoutMs: 50,
		pollMs: 10,
		waitId: "timeout-wait",
