...
} from "../../src/bridge/extension/service_worker/types.ts";
...
function populatedRecord(config: JsonRecord = {}) {
	...
	record.request = {
		method: "POST",
		url: "https://api.example.test/users?q=1",
...
test("network model normalizes recorder config and preserves every recorder filter decision", () => {
	...
	assert.deepEqual(
		{
			...
			capture: [
				...
				config.captureRequestPostData,
				config.includeWebSocketFrames,
				config.includeSse,
			],
			lists: [
	...
	const decision = (overrides: Partial<NetworkRecorderConfig>, phase = "response") =>
		model.makeNetworkRecorderFilter({
			...config,
			includeUrls: [],
			excludeUrls: [],
			resourceTypes: [],
			...
			statuses: [],
			...overrides,
		} as NetworkRecorderConfig)(record, phase);
	...
	assert.deepEqual(decision({ resourceTypes: ["all"], statuses: [404] }, "request"), {
		match: false,
		reason: "status",
	});
	...
	assert.deepEqual(
		model.makeNetworkRecorderFilter({
			...
			methods: [],
			statuses: [404],
		} as NetworkRecorderConfig)(noStatus, "request"),
		{ match: true, reason: "matched" },
	);
	assert.deepEqual(
		decision({
			includeUrls: ["api.example.test"],
			excludeUrls: ["logout"],
			resourceTypes: ["Fetch"],
			methods: ["POST"],
			statuses: [201],
...
test("network model classifies MIME/body errors and stores bounded text and base64 bodies", () => {
	...
	assert.deepEqual(model.networkBodyMimeDecision({ ...owner.config, bodyMimeAllow: ["all"] }, record), {
		match: true,
		reason: "all",
	});
	assert.deepEqual(model.networkBodyMimeDecision({ ...owner.config, bodyMimeAllow: ["json"] }, record), {
		match: true,
		reason: "mime_allowed",
		mimeType: "application/json",
	});
	assert.deepEqual(
		model.networkBodyMimeDecision({ ...owner.config, bodyMimeAllow: ["text"] }, { ...record, response: null }),
		{ match: false, availability: "not_requested", reason: "mime_missing", mimeType: "" },
	);
	assert.deepEqual(
		model.networkBodyMimeDecision(
			{ ...owner.config, bodyMimeAllow: ["json"] },
			{ ...record, response: { mimeType: "application/pdf" } },
		),
		{ match: false, availability: "binary", reason: "binary_mime", mimeType: "application/pdf" },
	);
	assert.deepEqual(
		model.networkBodyMimeDecision(
			{ ...owner.config, bodyMimeAllow: ["json"] },
			{ ...record, response: { mimeType: "application/yaml" } },
		),
		{ match: false, availability: "not_requested", reason: "mime_not_allowed", mimeType: "application/yaml" },
	);
	assert.deepEqual(model.classifyNetworkBodyError(new Error("No resource with given identifier")), {
		availability: "expired",
		reason: "cdp_body_expired",
	});
	assert.deepEqual(model.classifyNetworkBodyError(new Error("request timed out")), {
		availability: "cdp_failed",
		reason: "cdp_timeout",
	});
	assert.deepEqual(model.classifyNetworkBodyError(new Error("debugger detached")), {
		availability: "cdp_failed",
		reason: "cdp_get_response_body_failed",
	});

	const notifications: string[] = [];
	model.setNetworkWaitNotifier((_recorder: ReturnType<typeof recorder>, eventType: string) =>
...
test("network model list, frame, summary, pruning, and diagnostics helpers stay bounded", () => {
	...
	const matching = {
		sinceSeq: Number(record.seq) - 1,
		requestId: record.requestId,
		urlContains: "/users",
		urlPattern: "api\\.example\\.test",
		method: "post",
		type: "fetch",
		mime: "json",
		status: 201,
	...
	for (const [key, value] of Object.entries({
		...
		status: 404,
		includeUrls: ["missing"],
		excludeUrls: ["api.example.test"],
	})) {
		assert.equal(model.networkRecordMatchesList(record, { ...matching, [key]: value }), false, key);
	}
	assert.equal(
		model.networkWsFrameMatches(record.wsFrames[0], { method: "received", opcode: 1, payloadContains: "socket" }),
		true,
	);
	assert.equal(model.networkWsFrameMatches(record.wsFrames[0], { opcode: 2 }), false);
	assert.equal(
		model.networkSseEventMatches(record.sseEvents[0], {
			eventName: "update",
			eventId: "7",
			dataContains: "stream",
		}),
	...
	assert.deepEqual(
		{
			...
			type: summary.type,
			postDataBytes: summary.postDataBytes,
			redirects: summary.redirects,
			wsFrameCount: summary.wsFrameCount,
			sseEventCount: summary.sseEventCount,
		},
		{ status: 201, type: "Fetch", postDataBytes: 7, redirects: 0, wsFrameCount: 1, sseEventCount: 1 },
	);
