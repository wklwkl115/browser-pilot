...
const chromeStub = {
	tabs: {
		onCreated: {
			...
			removeListener(listener: CreatedListener) {
...
const cdpBridge = {
	async send(_tabId: number, method: string, params: Record<string, unknown>, options?: Record<string, unknown>) {
		...
		if (method === "Runtime.evaluate")
			return { ok: true, data: { result: { result: { value: { ok: true, data: "cdp-result" } } } } };
...
test("exec dispatch rejects missing tabs before ACK and handles navigation shortcuts", async () => {
	...
	assert.deepEqual(messages(navigate).at(-1), {
		type: "result",
		id: "navigate",
		result: { navigated: true, url: "/next" },
	});

	const blocked = socket();
	await handleWsExec({ id: "blocked", tabId: 7, code: "location = 'javascript:alert(1)'" }, blocked);
	assert.match(
		String((messages(blocked).at(-1)?.error as Record<string, unknown>).message),
		/Blocked navigation protocol/,
	);
...
test("exec dispatch returns MAIN-world results and collected new-tab metadata", async () => {
	...
	const value = socket();
	await handleWsExec(
		{ id: "main", tabId: 7, code: "window.open('https://new.example/'); 21 * 2", timeoutMs: 3_000 },
		value,
	);
	...
	assert.deepEqual(result?.diagnostics, {
		execute: {
			...
			totalMs: (result?.diagnostics as { execute: { totalMs: number } }).execute.totalMs,
		},
	});
...
test("exec dispatch routes background tabs directly through persistent CDP", async () => {
	...
	assert.deepEqual(cdpCalls[0]?.options, {
		name: "default",
		persistent: true,
		focusEmulation: true,
		timeoutMs: 2_000,
