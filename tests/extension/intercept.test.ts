...
test("intercept owns install, rules, paused request settlement, collection, and cleanup", async () => {
	const installed = await intercept.handleBrowserPilotInterceptCommand("intercept.install", 7, {
		cmd: "intercept.install",
		stages: ["request"],
	});
	...
	await intercept.handleBrowserPilotInterceptCommand("intercept.pause", 7, {
		cmd: "intercept.pause",
		autoApply: false,
		params: { requestId: "request-fulfill", request: { url: "https://example.test/other", method: "GET" } },
	});
	const fulfilled = await intercept.handleBrowserPilotInterceptCommand("intercept.fulfill", 7, {
		cmd: "intercept.fulfill",
		requestId: "request-fulfill",
		responseCode: 201,
		body: "created",
	...
	await intercept.handleBrowserPilotInterceptCommand("intercept.pause", 7, {
		cmd: "intercept.pause",
		autoApply: false,
		params: { requestId: "request-fail", request: { url: "https://example.test/fail" } },
	});
	const failed = await intercept.handleBrowserPilotInterceptCommand("intercept.fail", 7, {
		cmd: "intercept.fail",
	...
	const collected = await intercept.handleBrowserPilotInterceptCommand("intercept.collect", 7, {
		cmd: "intercept.collect",
		limit: 20,
	});
	assert.ok(Number((collected.data as Record<string, unknown>).count) >= 5);
	const removed = await intercept.handleBrowserPilotInterceptCommand("intercept.removeRule", 7, {
		cmd: "intercept.removeRule",
		ruleId: "rewrite-api",
	});
	assert.equal((removed.data as Record<string, unknown>).removed, true);

	const uninstalled = await intercept.handleBrowserPilotInterceptCommand("intercept.uninstall", 7, {
