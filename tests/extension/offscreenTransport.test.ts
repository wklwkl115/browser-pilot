...
test("offscreen transport closes a bridge socket when forwarding to the service worker fails", async () => {
		...
		chrome: {
			runtime: {
				async sendMessage(message: Record<string, unknown>) {
					...
					if (message.type === "browser-pilot-offscreen-connected")
	...
	try {
		...
		for (let index = 0; index < 10 && !FakeWebSocket.instances.length; index += 1)
		...
		assert.equal(
			messages.some(
				(message) =>
					...
					(message.data as Record<string, unknown>)?.reason === "service-worker-unreachable",
			),
			true,
		);
	} finally {
