...
test("extension manifest preserves page security and native dialog semantics", () => {
	const manifest = JSON.parse(
		readFileSync(path.join(root, "src", "bridge", "extension", "static", "manifest.json"), "utf8"),
	) as {
		permissions?: string[];
	...
	for (const permission of ["cookies", "management", "contentSettings"])
	...
	assert.equal(
		manifest.content_scripts?.some((script) => script.all_frames === true),
		false,
	);
	assert.equal(
		manifest.content_scripts?.flatMap((script) => script.js ?? []).some((file) => /dialog/i.test(file)),
		false,
	);
});
