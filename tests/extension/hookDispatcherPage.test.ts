...
const dispatcherSource = await readFile(
	new URL("../../src/bridge/extension/page_scripts/hook_dispatcher.ts", import.meta.url),
	"utf8",
);
const dispatcherScript = new vm.Script(
	(
		await transform(dispatcherSource, {
			format: "iife",
			loader: "ts",
...
test("page hook restores exact crypto method identities on uninstall", async () => {
	...
	const eventTypes = eventsFrom(
		page.api.collect({ session_id: "crypto-session", event_types: ["crypto."], limit: 20 }),
