...
assert(protocolSandbox.self.PiNativeProtocol?.validateCommand?.({ cmd: "input.ref", tabId: 1, action: "click", target: { backendNodeId: 91 } })?.ok === true, "protocol validator must accept input.ref click commands");
assert(protocolSandbox.self.PiNativeProtocol?.validateCommand?.({ cmd: "input.ref", tabId: 1, action: "click", target: { backendNodeId: 91, targetId: "child-target" } })?.ok === true, "protocol validator must accept target-scoped input.ref click commands");
assert(schema.commands?.["input.ref"]?.domain === "input" && schema.commands?.["input.ref"]?.accessMode === "write" && schema.commands?.["input.ref"]?.tabScoped === true, "input.ref must remain a tab-scoped input-domain write command");
...
assert(errorCodesSource.includes('"BACKEND_NODE_STALE"') && errorCodesSource.includes('"OOPIF_SESSION_UNSUPPORTED"') && errorCodesSource.includes('"INVALID_REF_TARGET"'), "native error codes must include input.ref fail-closed diagnostics");
assert(errorCodesSource.includes('"TARGET_ATTACH_FAILED"'), "native error codes must include persistent CDP child target diagnostics");
