...
assert.equal(pkg.scripts?.["verify:bridge:dist"], "node tests/contracts/protocol/check-bridge-build.mjs", "package must expose a read-only bridge dist verification command");
...
assert.match(buildManifest.buildId, /^[0-9a-f]{64}$/, "build manifest must expose a sha256 buildId");
assert.equal(buildManifest.buildIdAlgorithm, "sha256:fingerprint-inputs-with-placeholder", "build manifest must document the non-self-referential buildId algorithm");
assert.equal(buildManifest.buildIdPlaceholder, "__PI_BROWSER_BRIDGE_BUILD_ID_PLACEHOLDER__", "build manifest must record the buildId placeholder");
assert.deepEqual(buildManifest.inputs, [
	"bridge/pi_browser_bridge/dist/content.js",
	"bridge/pi_browser_bridge/dist/disable_dialogs.js",
	"bridge/pi_browser_bridge/dist/hook_dispatcher.js",
	"bridge/pi_browser_bridge/dist/offscreen.js",
...
].sort(), "build manifest must record the fixed browser-loaded fingerprint inputs");
...
const commandModules = ["network_model", "network_events", "network", "intercept", "hook", "evidence", "frame", "layer", "html", "screenshot", "transfer", "bridge_info", "core_commands", "exec", "input", "ws_model", "ws"];
...
assert(coreCommandsSource.includes("pi.contentFingerprint") && coreCommandsSource.includes("drainDirty") && coreCommandsSource.includes("chrome.scripting.executeScript") && coreCommandsSource.includes("__piBrowserFingerprintFallbackState__"), "content.fingerprint must keep a content-script-first executeScript fallback for effect fingerprints and dirty roots");
...
assert(runtimeSource.includes("from \"./network\"") && runtimeSource.includes("handleNetworkRecorderCommand") && runtimeSource.includes("from \"./hook\"") && runtimeSource.includes("handlePiBrowserHookCommand") && runtimeSource.includes("from \"./layer\"") && runtimeSource.includes("handlePiBrowserLayerCommand"), "TODO 198 runtime must call command modules through ESM imports");
...
for (const legacyHandler of ["handleNetworkRecorderCommand", "handlePiBrowserHookCommand", "handlePiBrowserEvidenceCommand", "handlePiBrowserFrameCommand", "handlePiBrowserTransferCommand", "handlePiBrowserInputCommand", "handlePiBrowserHtml", "captureScreenshotWithRetry"]) {
...
const serviceWorkerBundleBudget = 424_000; // ABML layer probe + OOPIF flat-session routing, ratcheted 2026-06-14.
assert(Buffer.byteLength(bundle, "utf8") < serviceWorkerBundleBudget, "dist service-worker bundle must stay below the whitespace-minified size budget");
...
assert(bundle.includes(buildManifest.buildId), "dist service-worker bundle must carry the substituted buildId");
assert(!bundle.includes(buildManifest.buildIdPlaceholder), "shipped bundles must not retain the buildId placeholder");
assert(/runtimeSwitched:\s*(?:true|!0)/.test(bundle) && /mode:\s*"production"/.test(bundle), "dist service-worker bundle must expose production runtime build metadata");
...
for (const required of ["new WebSocket", "browser-pilot-offscreen-ws-message", "browser-pilot-offscreen-send", "function probeAndConnectWS"]) {
	assert(offscreenBundle.includes(required), `offscreen bundle must own durable transport behavior: ${required}`);
}
assert(offscreenBundle.includes("PI_BROWSER_BRIDGE_PORT_RANGE_END") || offscreenBundle.includes("18784"), "offscreen bundle must own durable transport behavior: PI_BROWSER_BRIDGE_PORT_RANGE_END");
...

{
	...
	for (const rel of buildManifest.inputs) {
		const absolute = path.join(root, rel);
		let bytes = readFileSync(absolute);
		if (rel.endsWith(".js")) bytes = Buffer.from(bytes.toString("utf8").replaceAll(buildManifest.buildId, buildManifest.buildIdPlaceholder), "utf8");
		hash.update(rel.replace(/\\/g, "/"));
		hash.update("\0");
		hash.update(bytes);
		hash.update("\0");
	}
	assert.equal(hash.digest("hex"), buildManifest.buildId, "reverse-substitution hash must reproduce the shipped buildId");
}
...
try {
	...
	assert.equal(globalThis.__PI_BROWSER_EXPERIMENTAL_BUILD__?.buildId, buildManifest.buildId, "imported service-worker bundle must expose the dist buildId");
