...
assert(!exec.includes(".stack") && !exec.includes("stack:"), "page-scripts exec: default error results must not expose JavaScript stack traces");
const execSandbox = {};
...
const returnedErrorExec = execSandbox.buildExecScript("return new Error('returned boom')", "return {ok:false}");
const returnedErrorResult = await vm.runInNewContext(returnedErrorExec, { console, setTimeout, clearTimeout, WeakSet, Map, Set, Date, RegExp, Error, NodeList: class NodeList {}, HTMLCollection: class HTMLCollection {} }, { filename: "generated-exec-returned-error.js" });
assert.equal(returnedErrorResult.ok, true, "page-scripts exec behavior: returned Error objects must stay successful script values");
assert.equal(returnedErrorResult.data.__type, "Error", "page-scripts exec behavior: returned Error objects must retain typed error metadata");
assert.equal(Object.hasOwn(returnedErrorResult.data, "stack"), false, "page-scripts exec behavior: returned Error objects must not expose stack traces");
const thrownErrorExec = execSandbox.buildExecScript("throw new Error('thrown boom')", "return { ok: false, error: { name: e.name || 'Error', message: e.message || String(e), details: e.details || {} } };");
const thrownErrorResult = await vm.runInNewContext(thrownErrorExec, { console, setTimeout, clearTimeout, WeakSet, Map, Set, Date, RegExp, Error, NodeList: class NodeList {}, HTMLCollection: class HTMLCollection {} }, { filename: "generated-exec-thrown-error.js" });
assert.equal(thrownErrorResult.ok, false, "page-scripts exec behavior: thrown errors must return structured failures");
assert.equal(Object.hasOwn(thrownErrorResult.error, "stack"), false, "page-scripts exec behavior: thrown errors must not expose stack traces");
