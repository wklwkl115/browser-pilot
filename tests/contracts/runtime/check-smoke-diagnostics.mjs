...
const interceptRequestMutateSmoke = read("tests/smoke/smoke-intercept-request-mutate.mjs");
...
assert(pkg.scripts?.["smoke:browser:intercept-request-mutate"]?.includes("smoke-intercept-request-mutate.mjs"), "package must expose intercept request-mutate runtime smoke");
...
assert(interceptRequestMutateSmoke.includes("intercept-request-mutate.html") && interceptRequestMutateSmoke.includes("intercept.continue") === false && interceptRequestMutateSmoke.includes("action: \"continue\"") && interceptRequestMutateSmoke.includes("request-mutate-patched") && interceptRequestMutateSmoke.includes("smoke-browser-intercept-request-mutate-results.json"), "intercept request-mutate smoke must use the local mutate fixture, validate continueRequest mutation before send, and write a dedicated artifact");
