...
`, "utf8");
	...
	const validDnsLabel = `corr-contract-${"a".repeat(49)}`;
	const validDnsTrigger = await runCallbackOast({ action: "trigger", sessionId: "contract-callback", mode: "dns", queryName: `${validDnsLabel}.oast.local`, queryType: "A", timeoutMs: 5_000 });
	assert.equal(validDnsTrigger.count, 1, "callback DNS trigger should accept 63-byte labels");
	assert.equal(validDnsTrigger.events[0].queryName, `${validDnsLabel}.oast.local`, "callback DNS trigger should preserve valid 63-byte labels");
	await assert.rejects(() => runCallbackOast({ action: "trigger", sessionId: "contract-callback", mode: "dns", queryName: `${"b".repeat(64)}.oast.local`, queryType: "A", timeoutMs: 5_000 }), /label 1 exceeds 63 bytes/i, "callback DNS trigger should reject labels longer than 63 bytes before building the packet");
	await assert.rejects(() => runCallbackOast({ action: "trigger", sessionId: "contract-callback", mode: "dns", queryName: `${"a".repeat(63)}.${"b".repeat(63)}.${"c".repeat(63)}.${"d".repeat(63)}`, queryType: "A", timeoutMs: 5_000 }), /name exceeds 255 bytes/i, "callback DNS trigger should reject DNS names whose wire length exceeds 255 bytes");
	...
	assert.equal(callbackSummary.eventCount, 5, "callback summary should expose event count");
	...
	assert.equal(callbackClear.cleared, 5, "callback clear should reset persisted events");
