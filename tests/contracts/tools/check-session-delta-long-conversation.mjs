...
try {
	...
	assert.throws(() => validateObserveParams("scan", { fresh: true, baselineSnapshotId: "snap-old" }), (error) => error?.code === "INVALID_RULE" && /fresh:true cannot be combined/.test(String(error.details?.reason)), "fresh:true rejects raw baselineSnapshotId");
	assert.throws(() => validateObserveParams("scan", { fresh: true, baselinePath: "some/path.json" }), (error) => error?.code === "INVALID_RULE" && /fresh:true cannot be combined/.test(String(error.details?.reason)), "fresh:true rejects raw baselinePath");
	...
	const normEnv = (env) => JSON.stringify(env).replace(/observe-scan-\d+\.json/g, "observe-scan-N.json").replace(/op-\d+/g, "op-N").replace(/\b\d{13}\b/g, "T").replace(/"bytes":\d+/g, '"bytes":N').replace(/"chars":\d+/g, '"chars":N');
	const serverA = createLongConversationServer();
	const serverB = createLongConversationServer();
	await runScanObservation(serverA, { mode: "scan", tabId: 7, maxChars: 6_000 }, { cwd }, "scan");
	await runScanObservation(serverB, { mode: "scan", tabId: 7, maxChars: 6_000 }, { cwd }, "scan");
	const envFalse = JSON.parse(textOf(await runScanObservation(serverA, { mode: "scan", tabId: 7, fresh: false, maxChars: 6_000 }, { cwd }, "scan")));
	const envAbsent = JSON.parse(textOf(await runScanObservation(serverB, { mode: "scan", tabId: 7, maxChars: 6_000 }, { cwd }, "scan")));
	assert.equal(envFalse.delta, "session", "fresh:false comparison must run in delta-established state");
	assert.equal(normEnv(envFalse), normEnv(envAbsent), "fresh:false is byte-identical to absent fresh in delta-established state");
