...
// expandedTarget, tableCells summary, currentIn) materialize from a real AX
...
try {
  ...
  const fixtureHtml = await readFile(path.join(root, "tests", "fixtures", "browser-workflows", "abml-relations.html"), "utf8");
  ...
  const familiesPresent = {
    ...
    tableCells: (summary.tableCells ?? 0) > 0,
  ...
  // summary covering every stable agent-visible family this fixture exercises.
