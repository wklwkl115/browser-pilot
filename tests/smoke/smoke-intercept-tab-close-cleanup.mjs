...
const resultPath = path.resolve(process.env.PI_BROWSER_SMOKE_RESULT_PATH || path.join(outDir, "smoke-browser-intercept-tab-close-cleanup-results.json"));
...
const profileDir = path.join(tempRoot, `intercept-tab-close-profile-${runId}`);
const extensionDir = path.join(tempRoot, `intercept-tab-close-extension-${runId}`);
let chrome;
let fixture;
let bridge;
let tabId;
...
try {
	...
	const bridgePort = await freePort(19001, 19040);
	const fixturePort = await freePort(9041, 9080);
	...
	const install = await bridge.sendCommand({ cmd: "intercept.install", tabId, sessionId: "smoke-tab-close", timeoutMs: 8_000 }, { tabId, timeoutMs: 10_000 });
	record("intercept.install", install.data?.active === true, { data: install.data });
	const addRule = await bridge.sendCommand({ cmd: "intercept.addRule", tabId, sessionId: "smoke-tab-close", action: "fulfill", matcher: { urlContains: "/api/intercept-data", stage: "request" }, patch: { responseCode: 200, responseHeaders: { "content-type": "application/json; charset=utf-8" }, body: JSON.stringify({ ok: true, source: "tab-close-rule", value: "tab-close-rule" }) }, timeoutMs: 8_000 }, { tabId, timeoutMs: 10_000 });
	record("intercept.addRule", typeof addRule.data?.rule?.ruleId === "string", { ruleId: addRule.data?.rule?.ruleId });
	await bridge.closeTab(tabId, 5_000).catch((error) => { throw error; });
	record("tab.close", true, { tabId });
	await waitUntil(async () => !bridge.getTabs().some((tab) => Number(tab.tabId) === Number(tabId)), 10_000);
	const statusAfterClose = await bridge.sendCommand({ cmd: "intercept.status", tabId, sessionId: "smoke-tab-close", timeoutMs: 8_000 }, { tabId, timeoutMs: 10_000 }).catch((error) => error);
	record("intercept.statusAfterClose", errorCodeOf(statusAfterClose) === "TAB_NOT_FOUND" || errorCodeOf(statusAfterClose) === "SESSION_NOT_FOUND" || /not connected|intercept session not found/i.test(errorMessageOf(statusAfterClose)), { errorCode: errorCodeOf(statusAfterClose), error: errorMessageOf(statusAfterClose) });
	const collectAfterClose = await bridge.sendCommand({ cmd: "intercept.collect", tabId, sessionId: "smoke-tab-close", timeoutMs: 8_000 }, { tabId, timeoutMs: 10_000 }).catch((error) => error);
	record("intercept.collectAfterClose", errorCodeOf(collectAfterClose) === "TAB_NOT_FOUND" || errorCodeOf(collectAfterClose) === "SESSION_NOT_FOUND" || /not connected|intercept session not found/i.test(errorMessageOf(collectAfterClose)), { errorCode: errorCodeOf(collectAfterClose), error: errorMessageOf(collectAfterClose) });
	tabId = undefined;
