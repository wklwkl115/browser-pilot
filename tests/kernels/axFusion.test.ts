...
test("indexed AX fusion keeps overflow boxes, radius-boundary points, and normalized semantic matches", () => {
	const overflow = mergeDomAndAxEntities(
		[
			domEntity("bp-ref://dom/overflow", {
				role: "region",
				name: "DOM region",
				geometry: { box: { x: 0, y: 0, w: 2_000, h: 2_000 } },
			}),
		],
	...
	const boundary = mergeDomAndAxEntities(
		[
			domEntity("bp-ref://dom/boundary", {
				role: "link",
				name: "Shared name",
				geometry: { point: { x: 24, y: 0 } },
			}),
		],
...
test("geometry enriches semantics without promoting AX identity while competing proposals stay unmatched", () => {
	const enriched = mergeDomAndAxEntities(
		[
			domEntity("bp-ref://dom/enriched", {
				role: "button",
				name: "Save",
				locators: [{ by: "css", value: "#save" }],
				geometry: { box: { x: 0, y: 0, w: 100, h: 30 } },
			}),
		],
...
test("DOM-only, AX-only, and fused modes preserve execution and password boundaries", () => {
	const dom = domEntity("bp-ref://dom/password", {
		role: "textbox",
		name: "Password",
		value: undefined,
		hints: { inputKind: "password" },
		locators: [{ by: "backendNodeId", value: 77 }],
	});
	const axWithoutBackend = buildAxEntityFromNode(
		{ role: "textbox", name: "Password", value: "secret" },
		{ observationId: "ax-fusion-test", capturedAt: 1 },
	);
	const axWithBackend = buildAxEntityFromNode(
		{ role: "textbox", name: "Password", value: "secret", backendDOMNodeId: 77 },
		{ observationId: "ax-fusion-test", capturedAt: 1 },
	);
...
test("indexed AX fusion stays bounded across backend, geometry, and semantic 8k datasets", () => {
	...
	assert.ok(
		...
		`expected indexed AX fusion run < ${maxElapsedMs}ms, got ${elapsedMs.toFixed(2)}ms`,
	);
});

test("indexed AX fusion keeps dense backend and unnamed semantic ambiguity buckets bounded", () => {
	const count = 10_000;
	const unnamedDom = Array.from({ length: count }, (_, index) =>
		domEntity(`bp-ref://element/${index}`, {
			kind: "element",
			role: "generic",
			name: undefined,
			geometry: undefined,
	...
	const duplicateBackendDom = Array.from({ length: count }, (_, index) =>
		domEntity(`bp-ref://backend/${index}`, {
			name: `Item ${index}`,
			locators: [{ by: "backendNodeId", value: 1 }],
		}),
	);
	const duplicateBackendAx = Array.from({ length: count }, (_, index) =>
		axEntity("button", `Item ${index}`, undefined, 1),
	);
	...
	assert.ok(
		elapsedMs < maxElapsedMs,
		`expected dense AX ambiguity runs < ${maxElapsedMs}ms, got ${elapsedMs.toFixed(2)}ms`,
	);
});
