...
import { PAGE_WORLD_SCAN_SCHEMA } from "../../src/kernels/abml/pageWorldScan.ts";
			...
			actionables: [
				{
					...
					focused: false,
					handlers: [],
					rect: { x: 1, y: 2, width: 90, height: 30 },
					documentRect: { x: 1, y: 202, width: 90, height: 30 },
					point: { x: 46, y: 17 },
					hitOk: true,
					hitTarget: null,
					priority: 100,
...
test("page-world scan bundle rejects unknown schemas and malformed fields", () => {
	...
	const unknownNested = {
		...valid,
		structure: { ...valid.structure, actionables: [{ selector: "#pay", unknownField: true }] },
	};
	const staleUnusedFields = { ...valid, structure: { ...valid.structure, rows: [] } };
	const staleGrowthProbe = {
		...valid,
		signals: { ...valid.signals, growthProbe: { supported: true, candidateCount: 1, elapsedMs: 10 } },
	};
	const internalAnnotation = {
		...valid,
		structure: {
			...valid.structure,
			actionables: [{ selector: "#pay", entityRefs: { domAction: "bp-ref://control/pay" } }],
		},
	};
