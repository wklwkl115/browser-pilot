		...
		pageWorldScanBundle({
			...
			structure: {
				actionables: [
					{
						selector: "#phone",
						rect: { x: 834, y: 265, width: 239, height: 48 },
						documentRect: { x: 834, y: 344, width: 239, height: 48 },
					},
				],
			},
		}),
...
test("bootstrapScanBackendNodeIds spatial index preserves ambiguity, overflow, and stale-selector semantics", () => {
	...
	assert.deepEqual(
		result.stats.records.map((record) => record.status),
		["ambiguous", "matched", "stale"],
	);
...
test("bootstrapScanBackendNodeIds stays bounded on large exact-match datasets", () => {
	...
	assert.ok(
		...
		`expected large exact-match bootstrap run < ${maxElapsedMs}ms after warmup, got ${elapsedMs.toFixed(2)}ms`,
	);
});
