...
test("identity reconciliation keeps global semantic controls stable across node and selector replacement", () => {
	const before = entity("bp-ref://control/old", {
		locators: [
			{ by: "backendNodeId", value: 1 },
			{ by: "css", value: "#old" },
		],
	});
	const after = entity("bp-ref://control/new", {
		locators: [
			{ by: "backendNodeId", value: 2 },
			{ by: "css", value: "#new" },
		],
		state: { ...before.state, focused: true },
	});
...
test("identity reconciliation follows unique names instead of repeated-item order", () => {
	const before = [
		entity("bp-ref://control/alice", {
			name: "Alice",
			scope: { key: "old-list", name: "Recipients", position: 1, size: 2 },
		}),
		entity("bp-ref://control/bob", {
			name: "Bob",
			scope: { key: "old-list", name: "Recipients", position: 2, size: 2 },
		}),
	];
	const after = [
		entity("bp-ref://control/new-bob", {
			name: "Bob",
			scope: { key: "new-list", name: "Recipients", position: 1, size: 2 },
		}),
		entity("bp-ref://control/new-alice", {
			name: "Alice",
			scope: { key: "new-list", name: "Recipients", position: 2, size: 2 },
		}),
	];
	const result = reconcileEntityIdentities(before, after);
	assert.deepEqual(
		result.entities.map((item) => item.ref),
		[before[1]!.ref, before[0]!.ref],
	);
...
test("identity reconciliation preserves refs through high-confidence template anchors", () => {
	...
	const after = names.map((name, index) =>
		entity(`bp-ref://control/new-${index}`, { name, hints, structure: { landmark: "main" } }),
	);
	const result = reconcileEntityIdentities(before, after);
	assert.deepEqual(
		result.entities.map((item) => item.ref),
...
test("identity reconciliation uses geometry only for one unnamed non-conflicting candidate", () => {
	...
	const nearby = entity("bp-ref://control/nearby", {
		name: undefined,
		geometry: { box: { x: 40, y: 20, w: 100, h: 40 } },
	});
	const far = entity("bp-ref://control/far", {
		name: undefined,
		geometry: { box: { x: 1_000, y: 1_000, w: 100, h: 40 } },
	});
...
test("identity reconciliation stays bounded across 8k semantic entities", () => {
	...
	const before = Array.from({ length: count }, (_, index) =>
		entity(`bp-ref://control/old-${index}`, { name: `Action ${index}` }),
	);
	const after = Array.from({ length: count }, (_, index) =>
		entity(`bp-ref://control/new-${index}`, { name: `Action ${index}` }),
	);
	...
	assert.ok(
		...
		`expected identity reconciliation < ${maxElapsedMs}ms, got ${elapsedMs.toFixed(2)}ms`,
	);
});
