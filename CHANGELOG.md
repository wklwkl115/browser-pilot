...
- 完成 TODO 228 Pre-navigation Hook Policy 设计：新增 `docs/pre-navigation-hook-policy.md`，冻结 document-start hook registry、metadata schema、install-before-navigate lifecycle、MV3 restart/watch 恢复与 no-script-persistence 边界；orchestration types 增加 future metadata/registry/registration 类型，`normalizeDesired` 在 TODO229 前显式拒绝 enabled `preNavigationHooks` 并递归拒绝 `script/code/source`；新增 `check:pre-nav-hook-policy` 静态契约并纳入 `npm run check`。Runtime 安装、验证、清理与 smoke 留给 TODO229。
