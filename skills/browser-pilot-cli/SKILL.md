...
4. Act with `execute` or the native `command` escape hatch.
   ...
   Use `browser-pilot execute --target-ref <targetRef> --script "document.title" --read-only --json` for a declared JavaScript query or `execute --program @program.json` for trusted CDP mouse/key/text/program frames. Supply exactly one input. `--read-only` is a caller guarantee: use it only when every supplied script/eval frame cannot mutate browser or page state; arbitrary JavaScript defaults to an unverified mutation. For a non-idempotent script mutation, also supply a stable `--intent-id` and a read-only `--postcondition` predicate in the same call, and keep the full payload unchanged across every recovery attempt:
   ...
   Scripted intents without `postcondition` are rejected. The predicate is evaluated immediately and again on DOM, input/change, focus, scroll, and resize signals, passes only on `true` or `{"verified":true}`, and produces `completed/script-postcondition-verified`. A false, lost, or timed-out predicate is ambiguous. Use `command --command '<inline-json>'` only when the normal command surface cannot express a required native operation; pass inline JSON only.
   ...
   Validated raw `browser_command` `cdp`, `persistent_cdp`, and `input.*` page writes use the same ABML semantic settlement path. Their `native-command-result` is dispatch evidence, not business proof; prefer `execute` with an explicit verifier whenever the normal surface can express the action.
...
5. Classify the returned operation before continuing.
   ...
   Read `dispatch`, `completion.source`, `business`, `semantic`, `signals`, `diagnostics`, `continuation`, `saved`, and `artifact_hints`. `dispatch.finishedAt` is transport/JavaScript settlement; `dispatch.settledAt`, when present, is the later semantic terminal commit. An action ACK means validation and the exact dispatch boundary succeeded, not that the business transaction succeeded; never equate acknowledgement or a visible effect with verified completion. `script-resolved` proves only that JavaScript evaluation returned and may complete an explicitly `--read-only` query; it is not business proof for an undeclared mutation. If ABML observes a semantic change without a declared expectation, the operation returns `effect_observed` with `business.status:"inconclusive"`, even when JavaScript returned a value. Require `script-postcondition-verified` for scripted business mutations and `program-verified` for non-navigation physical mutations. A stable first no-effect capture does not fail immediately; an unverified mutation remains event-driven until a later page revision or its deadline. A false explicit postcondition or final verifier is a declared terminal failure: Browser Pilot performs one fenced post-action capture and commits its exact revision, then returns `ambiguous` for acknowledged physical/page effects or the resolver's failure status without waiting for an unrelated future revision. Page writes are serialized as one arm/dispatch/semantic-settlement/commit transaction on the same owner/session/stable target; `operation.begin` confirms the persistent top-frame observer when a concrete tab exists, while targetless control-plane operations only arm lifecycle listeners. Do not blindly replay an acknowledged mutation; follow `observe`, `reacquire_target`, `inspect_diagnostics`, `verify_command_state`, or `inspect_artifact` guidance according to the returned continuation. After an acknowledged but unverified mutation, later `execute` or native-command writes remain blocked until a fresh canonical `observe` begins after settlement and verifies the same page identity; reusing the same `intentId` remains blocked even after observation. Mint a new intent only after read-only evidence proves the prior intent did not take effect.
