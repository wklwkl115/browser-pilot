...
- 当前主链路：`browser_tabs` 定位目标 → `browser_observe mode=scan|content|html|text|tabs` / `browser_screenshot` / `browser_frame` 观察 → `browser_execute` / `browser_command` / `browser_wait` 执行与等待 → `browser_network` / `browser_hook` / `browser_evidence` 取证 → `browser_artifact` 读取大证据；任务成功后可用 `browser_memory` 做本地 evidence-gated 经验沉淀与 recall。
...
- `browser_memory`：本地 `.pi/browser-memory/` 的 browser memory 工具；`action:"record"|"validate"` 只接受带 durable evidence 的本地经验沉淀，`record` 会写 entry + 派生 `index.json`，`recall` 只返回 bounded cards，`read` 通过 `id` 或 `browser-memory://...` 读取正文；本地 `scopeKind:"origin"|"task"|"project"` 均支持，不写 repo、不做 export/promote。
