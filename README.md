...
`npm run smoke:browser:isolated` 会复制临时扩展目录、patch dist bridge 端口、启动独占 Chrome profile，并写入 `.pi/browser-artifacts/smoke-browser-isolated-results.json`；用于本地常驻 agent 占用 18765 时的 runtime 验证，不修改用户当前扩展目录。Full isolated smoke 已覆盖 TODO 233 managed profile A/B 隔离，以及 TODO 237 登录态存在/不存在、pre-navigation/hook effect、logical target 与 redacted assertions artifact；`PI_BROWSER_PROFILE_CHROME` 可指定 runtime managed profile Chrome/Edge，默认 smoke/release gate 不启动 incognito。
