...
- `browser_observe`：观察层 canonical 工具。`mode=scan` 返回 GA-style 简化 DOM/text、Scan Manifest v2、actionables、forms/lists/text signals 与 `artifact_hints`；ABML envelope 顶层提供 `gist/outline/relations/inference/templates/snapshotProjection`，且可传 `baseline`（上一轮实体数组/scan summary/envelope/snapshotId）得到顶层 `diff`（appeared/disappeared/changed/focusedRef）、`treeDiff`（重复结构的 appeared/disappeared/changed/reordered）和 `form-dependency` 推理；`snapshotProjection` 会把当前结构模板与 template delta 持久化到 observe artifact；`mode=content` 返回正文 Markdown 并保留 durable navigation；`mode=html` 返回原始 HTML/text snapshot；`mode=text` 返回可见文本优先观察；`mode=tabs` 返回观察用 tab facts。不支持 `auto` 模式，不做 selector miss fallback。
...
- 现阶段 deeper internal integration 已把部分 ABML 证据显式上浮到既有公开工具结果中：`browser_observe mode=scan|text` 会在 details 中提供 `abml.integrated/entityCount/primaryEntityCount/listEntityCount/visualRegionCount/frameEntityCount`；`browser_observe mode=scan` 可用 `baseline` 生成顶层 `diff/treeDiff`，并让 `inference.intents` 出现 `form-dependency`；`templates/treeDiff/snapshotProjection` 用于大 list/table 的结构压缩、O(change) 复观测和 saved artifact 结构投影；`browser_frame` 结果会提供 `abmlIntegrated/frameCount/entityCount`。这仍是内部增强，不是新公开 tool surface。
