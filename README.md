...
`npm run release:local:smoke` 在上述验收基础上对当前解包扩展运行 full isolated smoke，并对 `previous/` 上一包运行最小 tabs/wait/execute 回滚 smoke；artifact 为 `.pi/browser-artifacts/release-acceptance/current-smoke-browser-isolated-results.json` 与 `rollback-smoke-browser-isolated-results.json`。当前包 full smoke artifact 上浮 `windowTabGroups`、`preNavigationHooks`、`profileIsolation`、`windowId/groupId/tabGroupsStatus`、profile A/B cookie/localStorage/sessionStorage 隔离与 cleanup 证据。失败摘要包含 pack 文件列表、build manifest、Chrome profile、bridge port、smoke artifact、orchestrationId、operationResults、window/tabGroups/preNavigation/profile diagnostics 与 orchestration artifact paths；需要保留失败临时 profile/扩展时加 `--keep-temp-on-failure` 或 `PI_BROWSER_SMOKE_KEEP_TEMP_ON_FAILURE=1`。
...
`npm run smoke:browser:isolated` 会复制临时扩展目录、patch dist bridge 端口、启动独占 Chrome profile，并写入 `.pi/browser-artifacts/smoke-browser-isolated-results.json`；用于本地常驻 agent 占用 18765 时的 runtime 验证，不修改用户当前扩展目录。Full isolated smoke 已覆盖 TODO 233 managed profile A/B 隔离；`PI_BROWSER_PROFILE_CHROME` 可指定 runtime managed profile Chrome/Edge，默认 smoke/release gate 不启动 incognito。

当前最终 smoke 覆盖 tabs/wait/scan/content/html/artifact/execute/pick/network/hook/evidence/frame/screenshot/download/upload，以及 `browser_orchestrate plan/apply/status/watch/delete`、cookie set/remove、network/hook 编排、watch 自愈、owned window 创建/cleanup、tabGroups success/degraded diagnostics、pre-navigation hooks、persistent state/adoption 与 managed profile A/B cookie/localStorage/sessionStorage 隔离。
