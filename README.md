...
- `browser_execute`：只执行 JavaScript（参数仅 `script` + 可选 `monitor` + 共享 tab 参数）；command-like JSON string 会显式报错并给出 `browser_command` recovery hint；常规自定义页面操作默认在这里用 JS 一次性完成。CLI 长脚本优先用 `pi-browser execute --script-file <path>` 避免 shell 引号问题（CLI-only，仍写入既有 `script` 参数）。没有 `action` 参数（曾试验的 `action.click/type/scroll` 臂已于 2026-06-05 整体回退）；执行后默认附带 cheap `effect` 事实，`monitor:true` 才追加更重的执行前后 scan diff；如果 fingerprint 信号不可用，effect 会显式给出 `signals:"partial"`，不会把未知渲染成 `mutations:0/settled:true`；dirty channel 可返回 `dirty`/`coverage:"overflow"`，脚本显式使用 `pi-ref://` 时还会在命中 dirty root 后给出 `targetObservedAt`/`targetRegionDirty`，提示刷新 ref。内部 `pi.*` stdlib 仅在脚本显式使用 `pi.` / `pi-ref://` 时注入并回显 `piRuntime:"1"`；2026-06-11 盲测未自然采用它，所以不作为公开操作建议。
...
物理输入最小闭环：先用 `browser_execute` JS 定位坐标 / `focus()`，再用 `browser_command` 发送 `{cmd:'input.pointer',gesture:'press',x,y}` 或 `{cmd:'input.keys',text:'...'}`，最后读 cheap `effect` / `browser_wait` / re-observe；清空可用 `input.keys` 的非打印键序列。需要语义 before/after 摘要时才给 JS 模式传 `monitor:true`。
