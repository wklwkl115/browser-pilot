...
- `browser_execute`：`script` 模式只执行 JavaScript；command-like JSON string 会显式报错并给出 `browser_command` recovery hint；常规自定义页面操作默认在这里用 JS 一次性完成。需要可靠页面动作时可用同一工具的 `action` 参数：`action.click` / `action.type` 走 ABML actionability + CDP trusted-event fallback + 共享 `timeoutMs` deadline + 目标状态 probe verify；完整 before/after entity diff 仅在 click/type 传 `diff:true` 时启用。`action.scroll` 默认只做 bounded scroll + cheap probe，虚拟列表实体收集需显式 `scroll.collect:true`。可选 `monitor:true` 只用于 `script` 模式，在保留 `data/target/newTabs/acknowledged` 顶层执行结果的同时追加执行前后 scan diff。
...
- `browser_observe`：观察层 canonical 工具。`mode=scan` 返回 GA-style 简化 DOM/text、Scan Manifest v2、actionables、forms/lists/text signals 与 `artifact_hints`；ABML envelope 顶层提供 `gist/outline/relations/inference/templates/snapshotProjection`，且可传短期 `baseline`（上一轮实体数组/scan summary/envelope/snapshotId）得到顶层 `diff`（appeared/disappeared/changed/focusedRef + additive `summary` salience）、`treeDiff`（重复结构的 appeared/disappeared/changed/reordered，排序优先 value/state/name 变化）和 `form-dependency` 推理；`snapshotProjection` 会把当前结构模板与 template delta 持久化到 observe artifact，scan artifact 同时镜像顶层 `diff/treeDiff/templates/snapshotProjection/causal` 以便 `browser_artifact jsonPath` 读取；`mode=content` 返回正文 Markdown 并保留 durable navigation；`mode=html` 返回原始 HTML/text snapshot；`mode=text` 返回可见文本优先观察；`mode=tabs` 返回观察用 tab facts。不支持 `auto` 模式，不做 selector miss fallback。
