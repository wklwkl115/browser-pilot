...
- Tab-scoped 工具自动化场景优先显式传 `targetRef`（来自 `tabHandle`）；省略时使用当前 browser session 内的 selected/latest fallback，手动切换会通过 activation 事件更新隐式目标；兼容 numeric `tabId` 会在同浏览器无歧义 replacement chain 内自动跟随，显式 stale target 会本地返回 `TAB_NOT_FOUND`，不会回落到其它 browser socket；结果会暴露 `target.tabHandle`、`target.browserSessionId`、`target.source`、`target.replacedFrom/replacedByTabId` 与 `selectionVersion` 便于诊断并发切 tab。
