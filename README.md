...
- 现阶段 deeper internal integration 已把部分 ABML 证据显式上浮到既有公开工具结果中：`browser_observe mode=scan|text` 会在 details 中提供 `abml.integrated/entityCount/primaryEntityCount/listEntityCount/visualRegionCount/frameEntityCount`；`browser_observe mode=scan` 可用短期 `baseline` 生成顶层 `diff/treeDiff`，其中 `diff.summary` 把 value/name/state/focus 变化排在 churn 前；`collections` 把长列表/虚拟列表/懒加载/分页的完整度与只读 continuation 证据作为感知数据输出；`templates/treeDiff/snapshotProjection` 用于大 list/table 的结构压缩、O(change) 复观测和 saved artifact 结构投影；observe artifact 顶层镜像 live envelope 的 `diff/treeDiff/templates/snapshotProjection/collections/causal`，也保留 `abml.*` 兼容路径；`browser_frame` 结果会提供 `abmlIntegrated/frameCount/entityCount`。这仍是内部增强，不是新公开 tool surface。`pi-ref://`、`pi-cont://collection/*` 与 observe baseline 都是短期证据句柄，过期或 `HANDLE_NOT_FOUND` 时重新 `browser_observe mode=scan`。
