...
| `browser_execute` | Precise JavaScript execution, or a structured program of physical input frames (mouse/key/text/drag) + eval, in a tab. | Need custom page action, state read, DOM mutation, focused interaction, trusted physical input (click/type/scroll/drag) via the `program` frames, or compact before/after monitor. | Do not use for native bridge commands, file upload, stable download handling, long waits, or bulk network replay. | JS result or per-frame program result, target/newTabs metadata, optional monitor diff, artifact, operation metadata. | `browser_wait` then re-observe with `browser_observe`. |
...
- `browser_execute` runs page JavaScript (`script`) or a structured input program (`program`: ordered mouse/key/text/drag/eval/wait frames dispatched as trusted CDP events). Physical page interaction is a mode of `browser_execute`, not a separate tool.
...
Web Security follow-up tools are always registered as part of the 21-tool surface. They remain a scoped follow-up group, not a separate capability profile or workflow mode.
