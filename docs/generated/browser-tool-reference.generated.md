...
| `browser_execute` | Browser Execute | core | `monitor`, `script`, `tabId`, `targetRef` | Execute JavaScript in a real browser tab. | outputPath/artifact; execute-<ts>.json | `src/commands/defineExecuteCommand.ts` |
...
| `browser_observe` | Browser Observe | core | `actionRef`, `baseline`, `baselinePath`, `baselineSnapshotId`, `fresh`, `htmlMode`, `includeIframes`, `includeLinks`, `intent`, `maxNodes`, `mode`, `params`, `selector`, `tabId`, `targetRef`, `url` | Observe browser tabs, page structure, readable content, or exact HTML via an explicit observation mode. | outputPath/artifact | `src/commands/defineObserveCommand.ts` |
