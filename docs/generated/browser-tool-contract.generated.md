...
| `browser_cookie_analyze` | Browser Cookie Analyze | security (default profile; set PI_BROWSER_TOOL_PROFILE=core to hide) | `allowPrivateTargets`, `bindBrowserSession`, `browserSessionId`, `claimMutations`, `claimReplay`, `cookie`, `cookies`, `detailLevel`, `jwt`, `jwts`, `maxChars`, `maxSecretCandidates`, `outputPath`, `secretCandidates`, `secrets`, `setCookie`, `setCookies`, `tabId`, `timeoutMs`, `url`, `values`, `wordlist`, `wordlistPath` | Analyze cookies/JWT/session values, verify signing or decryption candidates, generate claim-mutation tokens, validate claim replays, and store structured evidence. | outputPath/artifact; cookie-analyze-<ts>.json | `src/tools/webSecurity/register/registerCookieAnalyze.ts` |
| `browser_crawl` | Browser Crawl | security (default profile; set PI_BROWSER_TOOL_PROFILE=core to hide) | `action`, `activeGraphqlIntrospection`, `bindBrowserSession`, `browserSessionId`, `defaultScheme`, `detailLevel`, `extractJs`, `followRedirects`, `headers`, `includeFaviconHash`, `includeTlsCertificate`, `knownFiles`, `maxBodyBytes`, `maxChars`, `maxDepth`, `maxPages`, `maxRedirects`, `method`, `outputPath`, `paths`, `ports`, `sameOrigin`, `schemes`, `tabId`, `timeoutMs`, `url`, `urls` | Collect scoped Web metadata through fingerprint probing or recursive crawl with structured evidence artifacts. | outputPath/artifact; crawl-fingerprint-<ts>.json | `src/tools/webSecurity/register/registerCrawl.ts` |
...
| `browser_execute` | Browser Execute | always-on | `action`, `browserSessionId`, `detailLevel`, `maxChars`, `monitor`, `outputPath`, `script`, `tabId`, `timeoutMs` | Execute JavaScript in a real browser tab. | outputPath/artifact; execute-action-<ts>.json | `src/tools/registerExecuteTool.ts` |
...
| `browser_fuzz` | Browser Fuzz | security (default profile; set PI_BROWSER_TOOL_PROFILE=core to hide) | `appendSlash`, `baseDomain`, `baseUrl`, `baselineHost`, `baselineHosts`, `baselinePath`, `baselineStrategy`, `bindBrowserSession`, `body`, `bodyBase64`, `browserSessionId`, `contentTypeVariants`, `defaultScheme`, `detailLevel`, `extensions`, `filterBaseline`, `filterBodyBytes`, `filterStatus`, `followRedirects`, `headers`, `hosts`, `jsonValues`, `locations`, `matchStatus`, `maxBodyBytes`, `maxCandidates`, `maxCases`, `maxChars`, `maxDepth`, `maxRedirects`, `method`, `mode`, `mutations`, `operations`, `outputPath`, `paramNames`, `paths`, `rateLimitPerSecond`, `rawRequest`, `recursive`, `request`, `sniMode`, `sniName`, `tabId`, `template`, `timeoutMs`, `url`, `urls`, `values`, `wordlist`, `wordlistPath`, `words` | Run bounded path, vhost, or parameter fuzzing against explicit scoped HTTP targets with structured evidence. | outputPath/artifact; fuzz-vhosts-<ts>.json | `src/tools/webSecurity/register/registerFuzz.ts` |
...
| `browser_memory` | Browser Memory | always-on | `action`, `body`, `evidenceRefs`, `id`, `jsonPath`, `kind`, `limit`, `maxChars`, `mode`, `offset`, `query`, `scopeKey`, `scopeKind`, `title`, `triggers`, `uri`, `url` | Record, recall, read, or validate local browser memory entries. | outputPath/artifact; browser-memory-read.json-<ts>.json | `src/tools/registerMemoryTool.ts` |
...
| `browser_observe` | Browser Observe | always-on | `actionRef`, `baseline`, `browserSessionId`, `detailLevel`, `htmlMode`, `includeIframes`, `includeLinks`, `maxChars`, `maxNodes`, `mode`, `outputPath`, `params`, `selector`, `tabId`, `timeoutMs`, `url` | Observe browser tabs, page structure, readable content, or exact HTML via an explicit observation mode. | outputPath/artifact | `src/tools/registerObserveTool.ts` |
...
| `browser_sqli` | Browser SQLi | security (default profile; set PI_BROWSER_TOOL_PROFILE=core to hide) | `allowLauncherOverride`, `allowPrivateTargets`, `answers`, `banner`, `baseUrl`, `baselineRepeats`, `batch`, `bindBrowserSession`, `body`, `bodyBase64`, `booleanPayloadPairs`, `browserSessionId`, `cookieMode`, `currentDb`, `currentUser`, `dbms`, `defaultScheme`, `detailLevel`, `engine`, `errorPayloads`, `extraArgs`, `extractCharset`, `extractExpression`, `extractMaxLength`, `flushSession`, `followRedirects`, `har`, `harEntryIndex`, `harMaxEntries`, `harPath`, `harUrlPattern`, `headers`, `isDba`, `level`, `locations`, `maxBodyBytes`, `maxCases`, `maxChars`, `maxRedirects`, `method`, `mutations`, `orderByMax`, `outputPath`, `paramNames`, `payloadMode`, `payloads`, `probeTypes`, `rateLimitPerSecond`, `rawRequest`, `request`, `requests`, `retries`, `risk`, `sequence`, `sqlmapArgs`, `sqlmapPath`, `stopOnFirstMatch`, `tabId`, `tamper`, `technique`, `threads`, `timePayloads`, `timeThresholdMs`, `timeoutMs`, `timeoutSeconds`, `unionColumnMax`, `unionEcho`, `unionPayloads`, `url` | Probe SQL injection or run bounded sqlmap automation from explicit scoped request templates with structured evidence. | outputPath/artifact; sqlmap-bridge-<ts>.json | `src/tools/webSecurity/register/registerSqli.ts` |
...
| `browser_template` | Browser Template | security (default profile; set PI_BROWSER_TOOL_PROFILE=core to hide) | `allowLauncherOverride`, `allowPrivateTargets`, `authors`, `bindBrowserSession`, `body`, `bodyBase64`, `browserSessionId`, `bulkSize`, `concurrency`, `cookieMode`, `defaultScheme`, `detailLevel`, `engine`, `excludeTags`, `extraArgs`, `followRedirects`, `har`, `harEntryIndex`, `harMaxEntries`, `harPath`, `harUrlPattern`, `headers`, `maxBodyBytes`, `maxChars`, `maxRedirects`, `maxRequests`, `maxTemplates`, `method`, `mutations`, `nucleiArgs`, `nucleiPath`, `outputPath`, `paths`, `rateLimitPerSecond`, `rawRequest`, `request`, `requests`, `retries`, `sequence`, `severities`, `tabId`, `tags`, `tech`, `templateIds`, `templatePath`, `templatePaths`, `templates`, `timeoutMs`, `timeoutSeconds`, `url`, `urls`, `variables`, `workflowPaths` | Run bounded built-in/custom HTTP template checks or mature nuclei template automation with structured evidence. | outputPath/artifact; nuclei-bridge-<ts>.json | `src/tools/webSecurity/register/registerTemplate.ts` |
...
| --- | --- | --- | --- | --- | --- |
| `ACTIONABILITY_TIMEOUT` | unknown | abml.actionability | yes | schema |  |
...
| `BACKEND_UNAVAILABLE` | unknown | abml.backend | yes | schema | `src/abml-core/verbs/click.ts`, `src/abml-core/verbs/frame.ts`, `src/abml-core/verbs/pierce.ts`, `src/abml-core/verbs/read.ts`, `src/abml-core/verbs/router.ts`, `src/abml-core/verbs/scroll.ts`, `src/abml-core/verbs/type.ts`, `src/abml/verbs/runtime.ts`, `src/abml/verbs/visionRuntime.ts`, `src/tools/registerExecuteTool.ts` |
...
| `CROSS_ORIGIN_BLOCKED` | unknown | abml.backend | no | schema | `src/abml/verbs/frameRuntime.ts` |
...
| `VERIFY_FAILED` | unknown | abml.verification | yes | schema |  |
| `VERIFY_INCONCLUSIVE` | unknown | abml.verification | yes | schema |  |
