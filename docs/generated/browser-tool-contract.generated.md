...
| `browser_observe` | Browser Observe | always-on | `baseline`, `browserSessionId`, `detailLevel`, `htmlMode`, `includeIframes`, `includeLinks`, `maxChars`, `maxNodes`, `mode`, `outputPath`, `params`, `selector`, `tabId`, `timeoutMs`, `url` | Observe browser tabs, page structure, readable content, or exact HTML via an explicit observation mode. | outputPath/artifact | `src/tools/registerObserveTool.ts` |
