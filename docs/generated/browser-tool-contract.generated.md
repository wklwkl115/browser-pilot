...
| `browser_execute` | Browser Execute | core | `monitor`, `script`, `tabId` | Execute JavaScript in a real browser tab. | outputPath/artifact; execute-<ts>.json | `src/tools/registerExecuteTool.ts` |
...
| `browser_observe` | Browser Observe | core | `actionRef`, `baseline`, `baselinePath`, `baselineSnapshotId`, `fresh`, `htmlMode`, `includeIframes`, `includeLinks`, `intent`, `maxNodes`, `mode`, `params`, `selector`, `tabId`, `url` | Observe browser tabs, page structure, readable content, or exact HTML via an explicit observation mode. | outputPath/artifact | `src/tools/registerObserveTool.ts` |
