...
| `browser_download` | Browser Download | always-on | `browserSessionId`, `conflictAction`, `detailLevel`, `expectMime`, `filename`, `index`, `maxChars`, `mode`, `outputPath`, `saveAs`, `selector`, `tabId`, `timeoutMs`, `url` | Download via selector click, media selector extraction, or direct HTTP(S) URL; returns download id/path/state. | outputPath/artifact | `src/tools/registerTransferTools.ts` |
...
| `browser_http_replay` | Browser HTTP Replay | security (default profile; set PI_BROWSER_TOOL_PROFILE=core to hide) | `baseUrl`, `bindBrowserSession`, `body`, `bodyBase64`, `browserSessionId`, `compareBaseline`, `continueOnError`, `cookieMode`, `csrfCookie`, `csrfHeader`, `defaultScheme`, `detailLevel`, `followRedirects`, `har`, `harEntryIndex`, `harMaxEntries`, `harPath`, `harUrlPattern`, `headers`, `maxBodyBytes`, `maxChars`, `maxRedirects`, `method`, `multipart`, `mutations`, `outputPath`, `rawRequest`, `reflectCsrf`, `request`, `requests`, `sequence`, `sequenceCookies`, `tabId`, `timeoutMs`, `url`, `variableScope`, `variables` | Replay captured/raw HTTP requests, mutate method/headers/body, and store bounded response evidence artifacts. | outputPath/artifact; http-replay-<ts>.json | `src/tools/webSecurity/register/registerHttpReplay.ts` |
| `browser_memory` | Browser Memory | always-on | `action`, `body`, `detailLevel`, `evidenceRefs`, `id`, `jsonPath`, `kind`, `limit`, `maxChars`, `mode`, `offset`, `query`, `redact`, `scopeKey`, `scopeKind`, `title`, `triggers`, `uri`, `url` | Record, recall, read, or validate local browser memory entries. | outputPath/artifact; browser-memory-read.json-<ts>.json | `src/tools/registerMemoryTool.ts` |
...
| `browser_observe` | Browser Observe | always-on | `actionRef`, `baseline`, `baselinePath`, `baselineSnapshotId`, `browserSessionId`, `detailLevel`, `htmlMode`, `includeIframes`, `includeLinks`, `maxChars`, `maxNodes`, `mode`, `outputPath`, `params`, `selector`, `tabId`, `timeoutMs`, `url` | Observe browser tabs, page structure, readable content, or exact HTML via an explicit observation mode. | outputPath/artifact | `src/tools/registerObserveTool.ts` |
