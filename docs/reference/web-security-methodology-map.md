...
| Headers, redirects, tech hints, known files | `browser_recon_probe` | `browser_crawl`, `browser_template_check` |
| Hidden routes, APIs, forms, JS endpoints | `browser_crawl` | `browser_fuzz_paths`, request replay |
| Path/file/extension discovery | `browser_fuzz_paths` | `browser_template_check` |
| Host routing / vhost suspicion | `browser_fuzz_vhosts` | recon probe per host |
| Need exact request | `browser_network start/list/get/body` | `browser_http_replay` |
| Replay or mutation | `browser_http_replay` | `browser_fuzz_params`, SQLi/OAST/template routes |
| Parameter behavior / parser difference | `browser_fuzz_params` | evidence/reporting |
| SQL error/time/boolean/UNION signal | `browser_sqli_probe` | `browser_sqlmap_bridge` |
| Exposure/config/CVE-shaped check | `browser_template_check` | `browser_nuclei_bridge` |
| Mature template pack needed | `browser_nuclei_bridge` | artifact read and manual verification |
| Cookie/JWT/JWE/session analysis | `browser_cookie_analyze` | claim replay via explicit replay target |
| Authz/IDOR request comparison | `browser_http_replay compareBaseline:true` | `browser_fuzz_params` |
