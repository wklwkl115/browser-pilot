# GenericAgent 到 Pi 浏览器资产同步

## 方向

- `bridge/pi_browser_bridge/` 是 Pi 原生桥的唯一运行入口。
- `bridge/tmwd_cdp_bridge/` 只作为迁移参考资产保留，不再由 Pi 工具加载。
- 从 GenericAgent 同步能力时，先迁移到原生命令域：`wait`、`network`、`hook`、`frame`、`html`、`screenshot`。

## 流程

1. 在 GenericAgent 资产中定位行为与边界条件。
...
- 不把 GenericAgent/TMWD 命名复制进 `bridge/pi_browser_bridge/`。
