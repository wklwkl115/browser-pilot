...
function isNativeBrowserCommand(cmd) {
  return /^(hook|wait|network|frame|html|screenshot)\./.test(String(cmd || ''));
}
...
async function handle(el) {
  try {
    ...
    } else if (cmd.startsWith('browser_pro.') || isNativeBrowserCommand(cmd)) {
      resp = await chrome.runtime.sendMessage(req);
    } else {
      resp = { ok: false, error_code: 'INVALID_RULE', error: 'unknown cmd: ' + cmd, details: { cmd } };
    }
