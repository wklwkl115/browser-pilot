...
var WaitCoordinator = class {
  ...
  diagnostics(tabId2) {
    ...
    return { activeWaits: Array.from(this.activeWaits.values()).filter((w) => !tabId2 || Number(w.tabId) === Number(tabId2)).map((w) => ({ wait_id: w.wait_id || w.waitId, request_id: w.request_id || w.requestId, kind: w.kind, epoch: w.epoch, diagnostics: w.diagnostics || {} })), eventSubscriptions: scopedEvents.length, epoch: this.epoch };
  }
};
...
async function waitForLoadState(tabId2, msg) {
  ...
  return await new Promise((resolve) => {
    ...
    record.timers.push(timeoutHandle);
    enablePiBrowserCdpDomains(record, ["Page"]).then(() => {
      const sub = subscribePiBrowserCdp(tabId2, ["Page.lifecycleEvent", "Page.domContentEventFired", "Page.loadEventFired", "Page.frameStoppedLoading"], async (_source, method, params) => {
...
async function diagnosePiBrowser(tabId2, msg) {
  ...
  return { ok: true, data: { tabId: Number(tabId2), tab, sessions: Array.from(piBrowserSessions.entries()).map(([tid, s2]) => ({ tabId: tid, ...s2 })), session: piBrowserSessions.get(Number(tabId2)) || null, queue: getPiBrowserQueueStats(tabId2), waits: activeWaits, activeWaits, listeners, frames, frameCount, iframeCount, inflight, readyState, last_errors, installed_marker, dispatcher_version, install_epoch, owner_session_id, install_fingerprint, cleanup_warnings, residue_signatures, version, epoch, diagnostics: diagnostics2, cdp: { persistent: !!cdp, debuggerTargets, ...cdpObservability, leaks: cdpLeaks }, active_subscriptions: activeCdpSubscriptions, cdp_domain_refs: cdpDomainRefs, cdp_cleanup_history: cdpCleanupHistory, domain_ref_leaks: cdpLeaks.domain_ref_leaks, subscription_leaks: cdpLeaks.subscription_leaks, debuggerTargets, dispatcher: status, persistent_cdp: !!cdp, timestamp: new Date(epoch).toISOString() } };
}
...
async function handlePiBrowserHookCommand(cmd, tabId2, msg) {
  ...
  if (cmd === "hook.uninstall") {
    ...
    if (shouldCleanup) {
      try {
        listenerCleanup = typeof cleanupPiBrowserPageListenersForTab === "function" ? await cleanupPiBrowserPageListenersForTab(tabId2, "hook_uninstall", msg.timeoutMs || msg.timeout_ms || 5e3) : { ok: true, data: { tabId: Number(tabId2), skipped: true, warning: "page listener cleanup helper unavailable" } };
      } catch (e) {
        listenerCleanup = piBrowserError(PI_BROWSER_ERROR_CODES.EVENT_SUBSCRIPTION_FAILED, "hook.uninstall page listener cleanup failed", { tabId: tabId2, reason: e instanceof Error ? e.message : String(e) });
      }
      const hookResponse = (
...
function piTransferFileChooserEvent(tabId2, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    let subscriptionId = subscribePiBrowserCdp(tabId2, "Page.fileChooserOpened", (_source, _method, params) => {
      cleanup();
...
function clearCspBypassAlarm() {
  try {
    chromeApi.alarms.clear?.(CSP_BYPASS_ALARM);
  } catch (_) {
  }
}
...
function cleanupPiBrowserTab(tabId2, reason) {
  ...
  try {
    ...
    if (pageCleanup && typeof pageCleanup.catch === "function") pageCleanup.catch((e) => console.warn("[PI-BROWSER] page listener cleanup failed", key, cleanupReason, runtimeErrorPreview(e)));
  } catch (e) {
