...
(() => {
  ...
  (function PiBrowserHookDispatcher() {
    ...
    function canonicalCommand(cmd) {
      ...
      return COMMAND_CANONICAL[key] || key;
    }
    function structuredError(error_code, message, details) {
    ...
    function currentHookRedactors() {
      ...
      redactMatchers = DEFAULT_REDACT_PATTERNS.concat(custom).map(compileHookRedactPattern).filter((item) => Boolean(item));
    ...
    function redactClone(v, depth = 0, seen) {
      ...
      if (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(v)) {
        const view = v;
        return { kind: view.constructor && view.constructor.name || "TypedArray", byteLength: view.byteLength, length: view.length };
      }
      ...
      const maxItems = numericOption("max_array_items", 100, 1, 1e4);
      const maxKeys = numericOption("max_object_keys", 100, 1, 1e4);
      if (Array.isArray(v)) {
        const out2 = v.slice(0, maxItems).map((x) => redactClone(x, (depth || 0) + 1, seen));
        if (v.length > maxItems) out2.push("\u2026[" + (v.length - maxItems) + " more items]");
        return out2;
      }
      const out = {};
    ...
    function addCleanupWarning(msg) {
      if (msg) cleanup_warnings.push(String(msg));
    }
    function addResidueSignature(sig) {
      if (!sig) return;
      const s = String(sig);
      if (residue_signatures.indexOf(s) < 0) residue_signatures.push(s);
    }
    function isPiWrapperSource(text) {
      return /__pi_browser_hooks|PiBrowserHookDispatcher|__PI_BROWSER_HOOKS__|network\.request|websocket\.open|storage\.set|crypto\.getRandomValues|dom\.sink|cookies\.read|console\./i.test(String(text || ""));
    }
    ...
    function perfSnapshot() {
      ...
      Object.keys(perfStats).forEach((k) => {
        ...
        out[k] = { count: b.count, last_ms: b.last_ms, max_ms: b.max_ms, avg_ms: b.avg_ms || 0, sample_count: b.samples.length, p50_ms: pct(0.5), p95_ms: pct(0.95) };
      });
      return out;
    }
    ...
    function clearEventNotifyTimer() {
      if (eventNotifyTimer) {
        clearTimeout(eventNotifyTimer);
    ...
    function notifyEvent(event) {
      ...
      const maxEvents = numericOption("batch_max_events", 50, 1, 1e4);
      const flushMs = numericOption("batch_flush_ms", 25, 0, 6e4);
    ...
    function validateTargets(nextTargets) {
      ...
      const allowed = /* @__PURE__ */ new Set(["network", "dom", "console", "error", "xpath", "cookies", "storage", "websocket", "crypto", "dom_sinks", "rules"]);
      for (const k of Object.keys(asRecord(nextTargets))) if (!allowed.has(k)) return k;
      return null;
    }
    function serializeError(err) {
      const e = asRecord(err);
      return err ? { name: e.name || errorName(err), message: e.message || errorMessage(err), stack: e.stack || "" } : null;
    }
    function hookXHR() {
    ...
    function hookWebSocket() {
      ...
      const nativeWebSocketRecord = NativeWebSocket;
    ...
    function piBrowserErrors() {
      ...
      const onError = (event) => {
        const e = event;
        push("error.uncaught", { message: e.message, source: e.filename, lineno: e.lineno, colno: e.colno, error: serializeError(e.error) });
      };
    ...
    function flushMutations() {
      ...
      const maxFlush = numericOption("mutation_flush_sample_limit", DEFAULT_LIMITS.mutation_flush_sample_limit, 1, 1e3);
      ...
      if (remaining > 0) push("dom.mutation.overflow", { queued_count: remaining, emitted_count: batch.length, hard_limit: numericOption("mutation_queue_hard_limit", DEFAULT_LIMITS.mutation_queue_hard_limit, 1, 1e5) });
      push("dom.mutation", summary);
      recordPerf("mutationFlush", t0);
      if (mutationQueue.length && !mutationTimer) mutationTimer = scheduleTimer(flushMutations, numericOption("mutation_batch_ms", 100, 0, 5e3));
    }
    function hookDOM() {
      if (!targets.dom || observer) return;
      const root = document.body || document.documentElement;
      if (!root || typeof MutationObserver === "undefined") return;
      const batchMax = numericOption("mutation_batch_max", 200, 1, 5e3);
      const batchMs = numericOption("mutation_batch_ms", 100, 0, 5e3);
      observer = new MutationObserver((muts) => {
        const hardLimit = numericOption("mutation_queue_hard_limit", DEFAULT_LIMITS.mutation_queue_hard_limit, 1, 1e5);
        muts.forEach((m) => {
    ...
    function hookStorage() {
      ...
      const setItemWrapper = function(key, value) {
      ...
      hookWrappers.storage.setItem = setItemWrapper;
      proto.setItem = setItemWrapper;
      const getItemWrapper = function(key) {
      ...
      hookWrappers.storage.getItem = getItemWrapper;
      proto.getItem = getItemWrapper;
      const removeItemWrapper = function(key) {
      ...
      hookWrappers.storage.removeItem = removeItemWrapper;
      ...
      hookWrappers.storage.clear = clearWrapper;
        ...
        try {
          subtleRecord[name] = function(...args) {
            push("crypto.subtle." + name, {
              ...
              format: typeof args[0] === "string" && ["raw", "pkcs8", "spki", "jwk"].indexOf(args[0]) >= 0 ? args[0] : void 0,
    ...
    function checkXPaths() {
      ...
      const maxPerTick = numericOption("xpath_max_per_tick", DEFAULT_LIMITS.xpath_max_per_tick, 1, 1e3);
      const largeThreshold = numericOption("xpath_large_result_threshold", DEFAULT_LIMITS.xpath_large_result_threshold, 1, 1e6);
      const repeatTicks = numericOption("xpath_large_result_repeat_ticks", DEFAULT_LIMITS.xpath_large_result_repeat_ticks, 1, 1e4);
      ...
      const idleAfter = numericOption("xpath_idle_after_ticks", 6, 1, 1e3);
      const maxMs = numericOption("xpath_poll_max_ms", 3e4, xpathPollMs || 1, 3e5);
      const nextMs = Math.min(maxMs, Math.max(1, xpathPollMs || 5e3) * (xpathIdleTicks >= idleAfter ? 4 : 1));
      xpathTimer = scheduleTimer(checkXPaths, nextMs);
    }
    function install(optsInput) {
      ...
      const requestedTargets = Object.assign({}, DEFAULT_TARGETS, asRecord(opts.targets));
      const requestedOptions = Object.assign({}, asRecord(opts.options));
      ...
      eventNotifyQueue = [];
      clearEventNotifyTimer();
      xpathPollMs = Number(options.xpath_poll_ms || 5e3);
      xpathIdleTicks = 0;
      xpathCache = {};
      ...
      hookStorage();
      hookCrypto();
      if (targets.xpath && targets.xpath.length) xpathTimer = scheduleTimer(checkXPaths, Math.max(1, xpathPollMs || 5e3));
      installed_at = now();
    ...
    function expectedSessionIdFrom(optsInput) {
      const opts = asRecord(optsInput);
      if (!opts) return null;
      const raw = opts.session_id !== void 0 ? opts.session_id : opts.sessionId;
      return raw === void 0 || raw === null || raw === "" ? null : String(raw);
    }
    function requireSession(op, expectedSessionId, allowMissing = false) {
    ...
    function collect(optsInput) {
      ...
      if (eventTypes.length) events = events.filter((e) => eventTypes.some((t) => e.type.indexOf(t) === 0));
    ...
    function clearBuffer(optsInput) {
      const opts = asRecord(optsInput);
    ...
    function uninstall(optsInput) {
      ...
      if (origFetch) {
        if (window.fetch === hookWrappers.fetch) window.fetch = origFetch;
        else addCleanupWarning("fetch wrapper identity changed before uninstall");
        origFetch = null;
        hookWrappers.fetch = null;
      }
      if (origWebSocket) {
        if (window.WebSocket === hookWrappers.websocket) window.WebSocket = origWebSocket;
        else addCleanupWarning("WebSocket wrapper identity changed before uninstall");
        origWebSocket = null;
        hookWrappers.websocket = null;
      }
      const consoleRecord = console;
      Object.keys(origConsole).forEach((k) => {
        if (consoleRecord[k] === hookWrappers.console[k]) consoleRecord[k] = origConsole[k];
        else addCleanupWarning("console." + k + " wrapper identity changed before uninstall");
      });
      origConsole = {};
      ...
      if (origStorage && typeof Storage !== "undefined") {
        ...
        Storage.prototype.key = origStorage.key;
        origStorage = null;
    ...
    function dispatch(cmdInput, args) {
      ...
      switch (cmd) {
        ...
        default:
          return structuredError(ERROR_CODES.INVALID_RULE, "Unknown Pi Browser command: " + cmd, { cmd });
      }
    }
    ...
    window.__PI_BROWSER_HOOKS__ = {
      ...
      getBufferStats: () => Object.assign({ count: buffer_count, size: buffer_size, usage: buffer_count / buffer_size, overflow }, bufferMetrics()),
      getPerfStats: () => perfSnapshot()
    };
    setState("CREATED", "bootstrap");
  })();
})();
//# sourceMappingURL=hook_dispatcher.js.map
