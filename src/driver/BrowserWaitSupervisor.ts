...
async function runLeasedWait(server: BrowserBridgeServer, command: BridgeCommand, options: { tabId?: number | string; timeoutMs?: number }, totalTimeoutMs: number, clock: WaitRunClock = {}): Promise<BrowserBridgeExecutionResult> {
...
export async function executeBrowserWaitWithSupervisor(server: BrowserBridgeServer, command: BridgeCommand, options: { tabId?: number | string; timeoutMs?: number } = {}): Promise<BrowserBridgeExecutionResult> {
