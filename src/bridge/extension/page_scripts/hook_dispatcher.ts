...
type BrowserPilotHookPageApi = Record<string, unknown> & {
	version?: string;
	dispatch?: BrowserPilotHookPageCallable;
	uninstall?: BrowserPilotHookPageCallable;
};
declare global {
	interface Window {
		__BROWSER_PILOT_HOOKS__?: BrowserPilotHookPageApi;
	}
}

/* ============================================================
 * Browser Pilot page-side hook dispatcher.
 * ============================================================ */
(function BrowserPilotHookDispatcher() {
	...
	type HookOptions = HookRecord & {
		redact_patterns?: unknown[];
		max_value_length?: unknown;
		buffer_size?: unknown;
		xpath_poll_ms?: unknown;
	};
	type HookStats = HookRecord &
		Record<
			| "network_events"
			| "dom_events"
			| "console_events"
	...
	function asRecord(value: unknown): HookRecord {
	...
	const UNHANDLED_CLONE = Symbol("unhandled-clone");

	function canonicalCommand(cmd: unknown): string {
		const key = String(cmd || "");
		return COMMAND_CANONICAL[key as keyof typeof COMMAND_CANONICAL] || key;
	}
	function structuredError(error_code: string, message: unknown, details?: unknown): HookResponse {
		return { ok: false, error_code, error: message, details: asRecord(details || {}) };
	}
	function now(): string {
	...
	function buildInstallFingerprint(
		sessionId: unknown,
		nextTargets: unknown,
		nextOptions: unknown,
		nextBufferSize: unknown,
		explicit: unknown,
	): string {
	...
	function compileHookRedactPattern(raw: unknown): RedactMatcher | null {
		...
		try {
			return { pattern, mode: "literal", regex: new RegExp(escapeRegExpLiteral(pattern), "gi") };
		} catch (_) {
			return null;
		}
	}
	...
	function redactSpecialObject(
		...
		return v instanceof Error ? redactClone(serializeError(v), depth + 1, seen) : UNHANDLED_CLONE;
	}
	function redactClone(v: unknown, depth = 0, seen?: WeakSet<object> | null): unknown {
		const scalar = redactScalar(v);
		if (scalar !== UNHANDLED_CLONE) return scalar;
		...
		const special = redactSpecialObject(objectValue, depth, seen);
		if (special !== UNHANDLED_CLONE) return special;
		...
		keys.slice(0, maxKeys).forEach((k) => {
			out[redactText(k)] = redactClone(record[k], (depth || 0) + 1, seen);
		});
	...
	function describeAlgorithm(algorithm: unknown): unknown {
		...
		if (algorithmRecord.hash && typeof algorithmRecord.hash === "object")
			out.hash = hash.name || safeString(algorithmRecord.hash, 256);
		return out;
	}
	function describeKey(key: unknown): unknown {
		...
		return {
			type: keyRecord.type,
			extractable: keyRecord.extractable,
			algorithm: describeAlgorithm(keyRecord.algorithm),
			usages: Array.prototype.slice.call(keyRecord.usages || []),
		};
	}
	function stackForEvent(): string {
		try {
	...
	const origCrypto: { subtle: HookFunctionRecord; getRandomValues: Crypto["getRandomValues"] | null } = {
		subtle: {},
	...
	const hookWrappers: {
		xhr: HookFunctionRecord;
		fetch: typeof window.fetch | null;
		websocket: typeof window.WebSocket | null;
		console: HookFunctionRecord;
		storage: HookFunctionRecord;
		crypto: HookFunctionRecord;
	} = { xhr: {}, fetch: null, websocket: null, console: {}, storage: {}, crypto: {} };
	...
	function addCleanupWarning(msg: unknown): void {
		if (msg) cleanup_warnings.push(String(msg));
	}
	function addResidueSignature(sig: unknown): void {
		if (!sig) return;
		const s = String(sig);
		if (residue_signatures.indexOf(s) < 0) residue_signatures.push(s);
	}
	function isBrowserPilotWrapperSource(text: unknown): boolean {
		return /__browser_pilot_hooks|BrowserPilotHookDispatcher|__BROWSER_PILOT_HOOKS__|network\.request|websocket\.open|storage\.set|crypto\.getRandomValues|dom\.sink|cookies\.read|console\./i.test(
			String(text || ""),
		);
	}
	function detectFunctionResidue(
		signature: string,
		read: () => unknown,
		matches = isBrowserPilotWrapperSource,
	): void {
		try {
			const value = read();
			if (typeof value === "function" && matches(Function.prototype.toString.call(value)))
				addResidueSignature(signature);
		} catch (_error) {
			/* best-effort hook residue detection */
		}
	}
	function detectResidue() {
		residue_signatures = [];
		detectFunctionResidue("fetch", () => window.fetch);
		if (typeof XMLHttpRequest !== "undefined" && XMLHttpRequest.prototype) {
			detectFunctionResidue("xhr.open", () => XMLHttpRequest.prototype.open);
			detectFunctionResidue("xhr.send", () => XMLHttpRequest.prototype.send);
		}
		detectFunctionResidue(
			"WebSocket",
			() => window.WebSocket,
			(text) => /WrappedWebSocket|websocket\.open|__BROWSER_PILOT_HOOKS__/i.test(String(text)),
		);
		if (typeof Storage !== "undefined" && Storage.prototype)
			detectFunctionResidue("storage.setItem", () => Storage.prototype.setItem);
		const consoleRecord = console as unknown as Record<string, unknown>;
		["log", "warn", "error", "info", "debug"].forEach((level) =>
			detectFunctionResidue("console." + level, () => consoleRecord[level]),
		);
	}
	...
	function perfSnapshot(): HookRecord {
		...
		Object.keys(perfStats).forEach((k) => {
			...
			const pct = (p: number) =>
				samples.length ? samples[Math.min(samples.length - 1, Math.floor((samples.length - 1) * p))] : 0;
			out[k] = {
				count: b.count,
				last_ms: b.last_ms,
				max_ms: b.max_ms,
	...
	function bufferMetrics(): HookRecord {
		...
		return {
			...
			buffer_used: used,
			buffer_utilization: capacity ? used / capacity : 0,
	...
	function storeEvent(event: HookEvent): void {
		if (buffer_count < buffer_size) {
			buffer[(buffer_start + buffer_count) % buffer_size] = event;
			buffer_count += 1;
			return;
		}
	...
	function push(type: string, data: unknown): void {
		const t0 = Date.now();
		if (paused && type !== "hook.lifecycle") return;
		seq += 1;
	...
	function setState(next: string, reason?: unknown): void {
		const prev = state;
		state = next;
		push("hook.lifecycle", { from: prev, to: next, reason: reason || "command" });
	}
	...
	function serializeError(err: unknown): HookRecord | null {
		const e = asRecord(err);
		return err
			? { name: e.name || errorName(err), message: e.message || errorMessage(err), stack: e.stack || "" }
			: null;
	}
	...
	function hookXHR() {
		...
		const sendWrapper = function (this: XhrWithMeta, body?: XMLHttpRequestBodyInit | null): void {
			...
			this.addEventListener("load", () =>
				push("network.response", {
					transport: "xhr",
					method: meta.method,
					url: meta.url,
					status: this.status,
					duration_ms: Date.now() - meta.t0,
					response_type: this.responseType || "",
					response_url: this.responseURL || meta.url,
				}),
			);
			this.addEventListener("error", () =>
				push("network.error", { transport: "xhr", url: meta.url, error: "XHR failed" }),
			);
	...
	function hookFetch() {
		...
		const fetchWrapper = function (input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
			...
			const url = String(
				typeof input === "string" ? input : input instanceof URL ? input.href : requestInput?.url || "",
			);
			const t0 = Date.now();
			push("network.request", {
				transport: "fetch",
				method,
				url,
				body: init && init.body != null ? describeBody(init.body) : null,
				stack: stackForEvent(),
			});
			return originalFetch
				...
				.catch((err: unknown) => {
					push("network.error", { transport: "fetch", url, error: errorMessage(err) });
					throw err;
				});
		};
	...
	function hookWebSocket() {
		...
		function WrappedWebSocket(url: string | URL, protocols?: string | string[]): WebSocket {
			...
			push("websocket.open", {
				id: wsid,
				url: String(url || ""),
				protocols: protocols == null ? null : safeString(protocols),
				stack: stackForEvent(),
			});
			ws.addEventListener("open", () => push("websocket.ready", { id: wsid, url: String(url || "") }));
			ws.addEventListener("message", (e) =>
				push("websocket.message", { id: wsid, url: String(url || ""), data: describeBody(e.data) }),
			);
	...
	function browserPilotErrors() {
		...
		const onRejection: EventListener = (event) => {
			...
			push("error.promise", { reason: safeString(e.reason), stack: asRecord(e.reason).stack });
		};
	...
	function flushMutations(): void {
		...
		const maxFlush = numericOption(
			"mutation_flush_sample_limit",
			DEFAULT_LIMITS.mutation_flush_sample_limit,
			1,
			1000,
		);
		...
		if (remaining > 0)
			push("dom.mutation.overflow", {
				queued_count: remaining,
				emitted_count: batch.length,
				hard_limit: numericOption(
					"mutation_queue_hard_limit",
					DEFAULT_LIMITS.mutation_queue_hard_limit,
					1,
					100000,
				),
			});
		...
		if (mutationQueue.length && !mutationTimer)
			mutationTimer = scheduleTimer(flushMutations, numericOption("mutation_batch_ms", 100, 0, 5000));
	}
	function hookDOM(): void {
		...
		observer = new MutationObserver((muts) => {
			...
			muts.forEach((m) => {
				...
				mutationQueue.push({
					type: m.type,
					target: elementRef(m.target),
					attribute_name: m.attributeName || "",
					added_count: m.addedNodes ? m.addedNodes.length : 0,
					removed_count: m.removedNodes ? m.removedNodes.length : 0,
				});
			});
			if (muts.length && mutationQueue.length >= hardLimit)
				push("dom.mutation.queue_overflow", {
					incoming_count: muts.length,
					queued_count: mutationQueue.length,
	...
	function hookDomSinks(): void {
		...
		function wrapDescriptor(proto: Element | Document | null | undefined, prop: string, label: string): void {
			...
			Object.defineProperty(proto, prop, {
				...
				set: function (this: unknown, value: unknown) {
					push("dom.sink.write", {
						sink: label,
						target: elementRef(this),
						value: safeString(String(value), 1024),
						stack: stackForEvent(),
					});
		...
		if (typeof nativeInsertAdjacentHTML === "function") {
			...
			Element.prototype.insertAdjacentHTML = function (position: InsertPosition, text: string): void {
				push("dom.sink.call", {
					...
					value: safeString(String(text), 1024),
					stack: stackForEvent(),
				});
		...
		if (typeof nativeWrite === "function") {
			...
			document.write = function (...text: string[]): void {
				push("dom.sink.call", {
					sink: "document.write",
					value: text.map((x) => safeString(String(x), 1024)),
					stack: stackForEvent(),
				});
	...
	function hookCookies(): void {
		...
		if (!desc) {
			proto = HTMLDocument && HTMLDocument.prototype;
			desc = proto && Object.getOwnPropertyDescriptor(proto, "cookie");
		}
	...
	function hookCrypto(): void {
		...
		if (!origCrypto.getRandomValues && typeof window.crypto.getRandomValues === "function") {
			...
			try {
				const getRandomValuesWrapper = function <T extends ArrayBufferView | null>(array: T): T {
					push("crypto.getRandomValues", { array: safeString(array), stack: stackForEvent() });
					if (array === null) return array;
					return originalGetRandomValues.call(window.crypto, array as ArrayBufferView<ArrayBuffer>) as T;
				};
				window.crypto.getRandomValues = getRandomValuesWrapper;
				origCrypto.getRandomValues = originalGetRandomValues;
				hookWrappers.crypto.getRandomValues = getRandomValuesWrapper as HookCallable;
			} catch (_error) {
		...
		[
			...
			const original = subtleRecord[name];
			try {
				const wrapper = function (...args: unknown[]) {
					push("crypto.subtle." + name, {
						...
						format:
							typeof args[0] === "string" && ["raw", "pkcs8", "spki", "jwk"].indexOf(args[0]) >= 0
								? args[0]
								: undefined,
				...
				subtleRecord[name] = wrapper;
				origCrypto.subtle[name] = original;
				hookWrappers.crypto[name] = wrapper;
			} catch (_error) {
	...
	function checkXPaths(): void {
		...
		const largeThreshold = numericOption(
			"xpath_large_result_threshold",
			DEFAULT_LIMITS.xpath_large_result_threshold,
			1,
			1000000,
		);
		const repeatTicks = numericOption(
			"xpath_large_result_repeat_ticks",
			DEFAULT_LIMITS.xpath_large_result_repeat_ticks,
			1,
			10000,
		);
		all.slice(0, maxPerTick).forEach((xpath) => {
			try {
				...
				if (prev !== r.snapshotLength || !large || xpathLargeResultTicks[xpath] >= repeatTicks) {
					...
					push("dom.xpath", {
						xpath,
						matched_nodes: r.snapshotLength,
						large_result: large,
						suppressed_repeats: large ? repeatTicks : 0,
					});
				}
			} catch (e) {
				const err = errorMessage(e);
				if (xpathCache[xpath] !== err) {
					changed = true;
					xpathCache[xpath] = err;
					push("dom.xpath", { xpath, error: err });
				}
			}
		});
	...
	function install(optsInput?: unknown): HookResponse {
		...
		if (opts.expected_version && String(opts.expected_version) !== VERSION) {
			return structuredError(ERROR_CODES.INJECTION_FAILED, "Browser Pilot dispatcher version mismatch", {
				expected_version: String(opts.expected_version),
				dispatcher_version: VERSION,
			});
		}
		const badTarget = validateTargets(opts.targets);
		if (badTarget)
			return structuredError(ERROR_CODES.UNSUPPORTED_TARGET, "Unsupported Browser Pilot target: " + badTarget, {
				target: badTarget,
			});
		const requestedSessionId = String(
			opts.session_id || "browser-pilot-hook-" + Date.now() + "-" + Math.random().toString(16).slice(2),
		);
		...
		const requestedBufferSize = Math.min(
			...
			Math.max(
				1,
				Math.trunc(
					Number(opts.buffer_size || requestedOptions.buffer_size || DEFAULT_BUFFER_SIZE) ||
						DEFAULT_BUFFER_SIZE,
				),
			),
		);
		const requestedFingerprint = buildInstallFingerprint(
			requestedSessionId,
			requestedTargets,
			requestedOptions,
			requestedBufferSize,
			opts.install_fingerprint,
		);
			...
			if (opts.force !== true && sameSession && sameFingerprint) {
				return {
					...
					data: {
						...
						cleanup_warnings: cleanup_warnings.slice(),
						residue_signatures: residue_signatures.slice(),
					},
				};
			}
			if (opts.force !== true) {
				return structuredError(
					...
					"Browser Pilot dispatcher is already installed with a different session or fingerprint",
					{
						state,
						session_id,
						owner_session_id,
		...
		xpathPollMs = Number(options.xpath_poll_ms || 5000);
		xpathIdleTicks = 0;
		xpathCache = {};
		xpathLargeResultTicks = {};
		perfStats = {};
		...
		hookStorage();
		hookCrypto();
		if (targets.xpath && targets.xpath.length)
			xpathTimer = scheduleTimer(checkXPaths, Math.max(1, xpathPollMs || 5000));
		installed_at = now();
		setState("INSTALLED", "install");
		return {
			ok: true,
			data: {
				session_id,
	...
	function requireSession(op: string, expectedSessionId?: unknown, allowMissing = false): HookResponse | null {
		...
		if (!session_id) {
			if (expectedSessionId)
				return structuredError(ERROR_CODES.SESSION_NOT_FOUND, op + " sessionId was not found", {
					state,
					requested_session_id: expectedSessionId,
				});
			return allowMissing
				? null
				: structuredError(ERROR_CODES.NO_SESSION, op + " requires an installed Browser Pilot session", {
						state,
					});
		}
	...
	function collect(optsInput?: unknown): HookResponse {
		const t0 = Date.now();
		const opts = asRecord(optsInput);
		const miss = requireSession("hook.collect", expectedSessionIdFrom(opts));
		if (miss) return miss;
	...
	function status(optsInput?: unknown): HookResponse {
		const opts = asRecord(optsInput);
		const miss = requireSession("hook.status", expectedSessionIdFrom(opts), true);
		if (miss) return miss;
		return {
			ok: true,
			data: {
				...
				stats: Object.assign(
					{},
					stats,
					{ buffer_count, buffer_size, buffer_usage: buffer_count / buffer_size },
					bufferMetrics(),
				),
				buffer_capacity: bufferMetrics().buffer_capacity,
				buffer_used: bufferMetrics().buffer_used,
				buffer_utilization: bufferMetrics().buffer_utilization,
				dropped_events: overflow,
				perf: perfSnapshot(),
				targets: clone(targets),
	...
	function pause(opts?: unknown): HookResponse {
		const miss = requireSession("hook.pause", expectedSessionIdFrom(opts));
		if (miss) return miss;
		paused = true;
		setState("PAUSED", "pause");
		return { ok: true, data: { session_id, state } };
	}
	function resume(opts?: unknown): HookResponse {
		const miss = requireSession("hook.resume", expectedSessionIdFrom(opts));
		if (miss) return miss;
		paused = false;
	...
	function evaluate(opts?: unknown): HookResponse {
		...
		try {
			...
			return structuredError(ERROR_CODES.INTERNAL_ERROR, errorMessage(e), serializeError(e));
		}
	}
	function clearBuffer(optsInput?: unknown): HookResponse {
		const opts = asRecord(optsInput);
		const miss = requireSession("hook.clear_buffer", expectedSessionIdFrom(opts));
		if (miss) return miss;
		buffer = new Array(buffer_size);
		buffer_start = 0;
		buffer_count = 0;
	...
	function restoreFunctionRecord(
		current: HookFunctionRecord,
		originals: HookFunctionRecord,
		wrappers: HookFunctionRecord,
		prefix: string,
	): void {
		Object.keys(originals).forEach((name) => {
			try {
				if (current[name] === wrappers[name]) current[name] = originals[name];
				else addCleanupWarning(prefix + name + " wrapper identity changed before uninstall");
			} catch (error) {
				addCleanupWarning(prefix + name + " restore failed: " + errorMessage(error));
			}
		});
	}
	function restoreHookFunctions(): void {
		if (origXHR) {
			restoreFunctionRecord(
				XMLHttpRequest.prototype as unknown as HookFunctionRecord,
				origXHR as unknown as HookFunctionRecord,
				hookWrappers.xhr,
				"xhr.",
			);
			origXHR = null;
		...
		const windowOriginals: HookFunctionRecord = {};
		const windowWrappers: HookFunctionRecord = {};
		if (origFetch) {
			windowOriginals.fetch = origFetch as HookCallable;
			windowWrappers.fetch = hookWrappers.fetch as HookCallable;
		}
		if (origWebSocket) {
			windowOriginals.WebSocket = origWebSocket as unknown as HookCallable;
			windowWrappers.WebSocket = hookWrappers.websocket as unknown as HookCallable;
		}
		restoreFunctionRecord(window as unknown as HookFunctionRecord, windowOriginals, windowWrappers, "");
		origFetch = null;
		hookWrappers.fetch = null;
		origWebSocket = null;
		hookWrappers.websocket = null;
		restoreFunctionRecord(console as unknown as HookFunctionRecord, origConsole, hookWrappers.console, "console.");
		origConsole = {};
		hookWrappers.console = {};
		if (origStorage && typeof Storage !== "undefined") {
			restoreFunctionRecord(
				Storage.prototype as unknown as HookFunctionRecord,
				origStorage as unknown as HookFunctionRecord,
				hookWrappers.storage,
				"storage.",
			);
			origStorage = null;
			hookWrappers.storage = {};
		}
		if (origCrypto.getRandomValues && window.crypto) {
			restoreFunctionRecord(
				window.crypto as unknown as HookFunctionRecord,
				{ getRandomValues: origCrypto.getRandomValues as HookCallable },
				hookWrappers.crypto,
				"crypto.",
			);
		}
		if (Object.keys(origCrypto.subtle).length && window.crypto?.subtle) {
			restoreFunctionRecord(
				window.crypto.subtle as unknown as HookFunctionRecord,
				origCrypto.subtle,
				hookWrappers.crypto,
				"crypto.subtle.",
			);
	...
	function restoreHookDescriptors(): void {
		if (origCookieDescriptor) {
			Object.defineProperty(origCookieDescriptor.proto, "cookie", origCookieDescriptor.desc);
			origCookieDescriptor = null;
		}
		...
		if (origDomSinks.insertAdjacentHTML)
			Element.prototype.insertAdjacentHTML =
				origDomSinks.insertAdjacentHTML as typeof Element.prototype.insertAdjacentHTML;
		if (origDomSinks.documentWrite) document.write = origDomSinks.documentWrite as typeof document.write;
		origDomSinks = {};
	}
	function stopHookCollectors(): void {
		errorHandlers.forEach((item) => window.removeEventListener(item[0], item[1]));
		errorHandlers = [];
		if (observer) {
			observer.disconnect();
			observer = null;
	...
	function uninstall(optsInput?: unknown): HookResponse {
		...
		stopHookCollectors();
		detectResidue();
		if (residue_signatures.length)
			addCleanupWarning("Browser Pilot hook residue detected after uninstall: " + residue_signatures.join(","));
		...
		return {
			...
			data: {
				session_id: old,
				state,
				events_collected: seq,
				overflow,
				dispatcher_version: VERSION,
				owner_session_id: oldOwner,
				install_fingerprint: oldFingerprint,
				install_epoch: oldEpoch,
	...
	function dispatch(cmdInput: unknown, args?: unknown): HookResponse {
		...
		switch (cmd) {
			...
			default:
				return structuredError(ERROR_CODES.INVALID_RULE, "Unknown Browser Pilot command: " + cmd, { cmd });
		}
	}
