import {
	BROWSER_PILOT_BRIDGE_HOST,
	BROWSER_PILOT_BRIDGE_HTTP_URL,
	BROWSER_PILOT_BRIDGE_PORT,
	BROWSER_PILOT_BRIDGE_PORT_RANGE_END,
	BROWSER_PILOT_BRIDGE_WS_URL,
} from "../service_worker/config";
...
const chromeRuntime = (globalThis as typeof globalThis & { chrome?: { runtime?: ChromeRuntimeLite } }).chrome
	?.runtime as ChromeRuntimeLite;
...
function wsUrlForPort(port: number): string {
	return port === BROWSER_PILOT_BRIDGE_PORT
		? BROWSER_PILOT_BRIDGE_WS_URL
		: `ws://${BROWSER_PILOT_BRIDGE_HOST}:${port}`;
}

function httpUrlForPort(port: number): string {
	return port === BROWSER_PILOT_BRIDGE_PORT
		? BROWSER_PILOT_BRIDGE_HTTP_URL
		: `http://${BROWSER_PILOT_BRIDGE_HOST}:${port}`;
}
	...
	keepaliveTimer = setTimeout(() => {
		...
		for (const socket of openSockets) {
			try {
				...
				cleanupTransportSocket(socket, "keepalive-send");
			}
		}
...
function connectWS(port: number): void {
	...
	socket.onopen = () => {
		...
		void post({ type: "browser-pilot-offscreen-connected", port, data: { openPorts: openPorts() } }).then(
			(forwarded) => {
	...
	socket.onmessage = (event: MessageEvent) => {
		if (sockets.get(port) !== socket) return;
		try {
			void post({ type: "browser-pilot-offscreen-ws-message", port, data: JSON.parse(String(event.data)) }).then(
				(forwarded) => {
					if (!forwarded) cleanupTransportSocket(socket, "service-worker-unreachable");
				},
			);
	...
	socket.onerror = (event: Event) => {
		console.debug("[BROWSER-PILOT-OFFSCREEN] Connection error; waiting for local server", {
			port,
			readyState: socket.readyState,
			type: event.type || "error",
		});
		if (socket.readyState !== WebSocket.OPEN && socket.readyState !== WebSocket.CONNECTING)
			cleanupTransportSocket(socket, "error");
	};
}
...
function handleRuntimeMessage(message: unknown, _sender: unknown, sendResponse: (response?: unknown) => void): boolean {
	...
	if (msg.type === "browser-pilot-offscreen-probe") {
		void probeAndConnectWS(msg.resetDelay === true, typeof msg.port === "number" ? msg.port : undefined).then(() =>
			sendResponse({ ok: true, openPorts: openPorts() }),
		);
		return true;
	}
