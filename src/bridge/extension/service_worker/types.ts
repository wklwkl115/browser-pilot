...
export type NetworkFrameRecord = {
	payloadData?: string;
	eventName?: string;
	eventId?: string;
	data?: string;
...
export type BrowserPilotChromeRuntime = {
	...
	onMessage: BrowserPilotChromeEvent<
		(
			message: unknown,
			sender: BrowserPilotBridgeSender,
...
export type BrowserPilotChromeDebugger = {
	...
	onDetach: BrowserPilotChromeEvent<
		(source: { tabId?: number; targetId?: string; sessionId?: string }, reason: string) => void
	>;
	onEvent: BrowserPilotChromeEvent<
		(source: { tabId?: number; targetId?: string; sessionId?: string }, method: string, params?: JsonRecord) => void
	>;
};
...
export type BrowserPilotChromeDownloads = {
	...
	onChanged: BrowserPilotChromeEvent<
		(delta: JsonRecord & { id?: number; state?: { current?: string }; filename?: unknown }) => void
	>;
...
export type BrowserPilotChromeApi = {
	...
	windows: {
		update(windowId: number, updateInfo: JsonRecord): Promise<unknown>;
		create(createData: JsonRecord): Promise<{ id?: number; tabs?: BrowserPilotChromeTab[] } | undefined>;
	};
	...
	alarms: {
		create(name: string, alarmInfo: JsonRecord): void;
		clear?(name: string): Promise<boolean> | boolean;
		onAlarm: BrowserPilotChromeEvent<(alarm: { name: string }) => void | Promise<void>>;
	};
	storage?: {
		session?: {
			get(keys?: string | string[] | JsonRecord): Promise<JsonRecord>;
			set(items: JsonRecord): Promise<void>;
	...
	webNavigation: Record<
		string,
		BrowserPilotChromeEvent<(details: JsonRecord & { tabId?: number; frameId?: number; url?: string }) => void>
	>;
};
