...
function screenshotTabNotFound(tabId: number, error: unknown): BrowserPilotBridgeResponse {
	return browserPilotError(BROWSER_PILOT_ERROR_CODES.TAB_NOT_FOUND, "screenshot.capture target tab not found", {
		tabId,
		reason: screenshotErrorMessage(error),
	});
}
...
async function getScreenshotTargetTab(
	tabId: number,
): Promise<{ ok: true; tab: BrowserPilotChromeTab } | { ok: false; error: unknown }> {
	try {
...
async function captureVisibleFallback(
	...
	if (target.tab.active !== true)
		return browserPilotError(
			BROWSER_PILOT_ERROR_CODES.UNSUPPORTED_TARGET,
			"captureVisibleTab fallback requires the target tab to be active",
			{ tabId, windowId: target.tab.windowId },
		);
	const actualFormat = format === "jpeg" ? "jpeg" : "png";
	const mime = "image/" + actualFormat;
	const dataUrl = await browserPilotWithTimeout(
		chrome.tabs.captureVisibleTab(Number(target.tab.windowId || 0), { format: actualFormat, quality }),
		timeoutMs,
...
async function captureScreenshotWithRetry(
	...
	for (let attempt = 0; attempt < attempts; attempt += 1) {
			...
			try {
				const resp = normalizePersistentBrowserPilotResponse(
					await cdp.send(
						...
						{ format, quality: msg.quality, captureBeyondViewport: msg.captureBeyondViewport === true },
						{ persistent: true, timeoutMs },
					),
				);
				if (resp && resp.ok !== false) {
					...
					if (resultRecord.data)
						return {
							ok: true,
							data: {
								screenshot: "data:image/" + format + ";base64," + resultRecord.data,
								format,
								method: "persistent_cdp",
							},
						};
				}
				lastErr = resp;
		...
		try {
			...
			attached = true;
			const result = await browserPilotWithTimeout(
				chrome.debugger.sendCommand({ tabId }, "Page.captureScreenshot", { format, quality: msg.quality }),
				timeoutMs,
				"Page.captureScreenshot",
			);
			const resultRecord = result && typeof result === "object" ? (result as JsonRecord) : {};
		...
		if (captureResult) {
			if (detachWarning && captureResult.data && typeof captureResult.data === "object")
				(captureResult.data as JsonRecord).cleanup = { debuggerDetach: detachWarning };
	...
	return browserPilotError(BROWSER_PILOT_ERROR_CODES.TIMEOUT, "screenshot.capture failed", {
		reason: screenshotErrorMessage(lastErr),
		attempts,
	});
}
export {
	isScreenshotMissingTabError,
	screenshotErrorMessage,
	screenshotTabNotFound,
	detachScreenshotDebugger,
	getScreenshotTargetTab,
