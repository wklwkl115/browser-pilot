...
import type {
	...
	BrowserPilotTabSyncTransport,
	BrowserPilotChromeTab,
	JsonRecord,
} from "./types.js";

// Tab list synchronization and tab lifecycle hooks.
...
function setBrowserPilotTabSyncTransport(deps: BrowserPilotTabSyncTransport) {
	if (!deps || typeof deps.getSocket !== "function" || typeof deps.probe !== "function")
...
function recordReplacement(from: number, to: number, at = Date.now(), kind?: "prerender-activation") {
	if (
		...
		!Number.isInteger(to) ||
		to <= 0 ||
		(from === to && kind !== "prerender-activation")
	)
		return;
...
async function sendTabsUpdate() {
	...
	const sockets =
		typeof transport.getSockets === "function"
			...
			: [transport.getSocket()].filter((socket): socket is BrowserPilotBridgeWebSocketLike => !!socket);
	...
	const tabsWithIdentity = await Promise.all(
		tabs.map(async (t: BrowserPilotChromeTab) => ({
			...
			...browserPilotPageIdentityFields(t),
...
function runTabSyncTask(reason: string, task: () => unknown) {
	try {
		...
		if (result && typeof (result as Promise<unknown>).catch === "function")
			void (result as Promise<unknown>).catch((e: unknown) => logTabSyncError(reason, e));
	} catch (e) {
...
function safeSendTabsUpdate(reason: string) {
	...
	tabSyncInFlight = (async () => {
		...
		.catch((error: unknown) => logTabSyncError(reason, error))
		.finally(() => {
...
function cleanupBrowserPilotTab(tabId: number, reason?: string) {
	...
	try {
		...
		if ((pageCleanup as Promise<unknown> | undefined)?.catch) {
			void (pageCleanup as Promise<unknown>).catch((error: unknown) =>
				console.warn(
					"[BROWSER-PILOT] page listener cleanup failed",
					key,
					cleanupReason,
					runtimeErrorPreview(error),
				),
			);
		}
	} catch (error) {
	...
	try {
		...
		console.warn("[BROWSER-PILOT-NET] recorder cleanup failed", key, runtimeErrorPreview(error));
	}
	try {
		cleanupInterceptSessionTab(tabId, cleanupReason);
	} catch (error) {
		console.warn("[BROWSER-PILOT-INTERCEPT] session cleanup failed", key, runtimeErrorPreview(error));
	}
	try {
		cleanupWsSessionsForTab(tabId, cleanupReason);
	} catch (error) {
		console.warn("[BROWSER-PILOT-WS] session cleanup failed", key, runtimeErrorPreview(error));
	}
	try {
		cleanupPersistentCdpForTab(tabId, cleanupReason);
	} catch (error) {
		console.warn("[BROWSER-PILOT-CDP] persistent session cleanup failed", key, runtimeErrorPreview(error));
	}
...
function installBrowserPilotTabSync(deps: BrowserPilotTabSyncTransport | undefined = undefined) {
	...
	chrome.tabs.onRemoved.addListener((tabId: number) => {
		...
		runTabSyncTask("tabs.onRemoved.identity", async () => {
			await forgetBrowserPilotTabIdentity(tabId);
	...
	chrome.tabs.onReplaced?.addListener((addedTabId: number, removedTabId: number) => {
		...
		runTabSyncTask("tabs.onReplaced.identity", async () => {
			await replaceBrowserPilotTabIdentity(removedTabId, addedTabId);
...
export {
	setBrowserPilotTabSyncTransport,
	requireBrowserPilotTabSyncTransport,
	sendTabsUpdate,
	logTabSyncError,
	runTabSyncTask,
	...
	safeSendTabsUpdate,
	cleanupBrowserPilotTab,
	installBrowserPilotTabSync,
	recordBrowserPilotPrerenderActivation,
};
