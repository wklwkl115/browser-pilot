...
function rawBridgeError(response: BrowserPilotBridgeResponse, command?: unknown): BrowserPilotBridgeResponse {
	const raw =
		response.error !== undefined ? response.error : response.message !== undefined ? response.message : response;
	...
	return bridgeError(
		String(
			response.error_code || rawRecord.error_code || rawRecord.code || BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR,
		),
		rawRecord.message ||
			(raw && typeof raw === "object"
				? String(rawRecord.code || rawRecord.name || "bridge command failed")
				: raw || "bridge command failed"),
		details,
	);
}
...
export function normalizePersistentBrowserPilotResponse(
	response: BrowserPilotBridgeResponse,
): BrowserPilotBridgeResponse {
	const error = runtimeRecord(response?.error);
	return response && response.ok === false && response.error && !response.error_code
		? browserPilotError(
				String(error.code || BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR),
				error.message || "persistent CDP command failed",
				error.details || {},
			)
		: response;
}
...
export async function browserPilotEval(
	tabId: number,
	expression: string,
	awaitPromise = true,
	options: BrowserPilotBridgeCommand = {},
	...
	if (cdp?.send) {
		const response = normalizePersistentBrowserPilotResponse(
			await cdp.send(
				...
				{ persistent: true, name: "eval", timeoutMs },
			),
		);
		if (!response || response.ok === false) return response;
		...
		if (result.exceptionDetails)
			return browserPilotError(
				...
				runtimeRecord(exceptionDetails.exception).description || "Runtime.evaluate failed",
				exceptionDetails,
			);
	...
	try {
		const command = chrome.debugger.sendCommand({ tabId }, "Runtime.evaluate", {
			expression,
			returnByValue: true,
			awaitPromise,
		});
		const result =
			timeoutMs === undefined
				? await command
				: await browserPilotWithTimeout(command, timeoutMs, "Runtime.evaluate");
		...
		try {
			...
			console.warn(
				"[BROWSER-PILOT] Failed to detach debugger after Runtime.evaluate fallback",
				tabId,
				runtimeErrorPreview(detachError),
			);
		}
		throw error;
	}
...
export async function callPageBrowserPilot(
	tabId: number,
	command: string,
	args: unknown,
	options: BrowserPilotBridgeCommand = {},
