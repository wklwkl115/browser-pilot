...
function newDocumentOptions(msg: BrowserPilotBridgeCommand): JsonRecord {
	...
	if (msg.timeoutMs !== undefined || msg.timeout_ms !== undefined)
...
async function callFrameCdp(
	...
	if (typeof operation !== "function")
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR, "Persistent CDP bridge is not loaded", {
			cmd,
		});
	const response = normalizePersistentBrowserPilotResponse(
		await (operation as (...values: unknown[]) => Promise<BrowserPilotBridgeResponse>)(...args),
	);
...
const frameHandlers: Record<string, FrameHandler> = {
	"frame.list": (cmd, tabId) =>
		callFrameCdp(cmd, "frameTree", [tabId, {}], (data) => {
	...
	"frame.evaluate": (cmd, tabId, msg) => {
		...
		return callFrameCdp(cmd, "evaluateInFrame", [tabId, String(msg.expression || ""), options], (data) => ({
			tabId,
	...
	"frame.addNewDocumentScript": (cmd, tabId, msg) => {
		...
		return callFrameCdp(cmd, "addNewDocumentScript", [tabId, String(msg.source), options], (data) => ({
			tabId,
			...data,
		}));
	},
	"frame.removeNewDocumentScript": (cmd, tabId, msg) => {
		...
		return callFrameCdp(
			...
			[tabId, String(msg.identifier), newDocumentOptions(msg)],
			(data) => ({ tabId, ...data }),
		);
	},
};

async function handleBrowserPilotFrameCommand(
	cmd: string,
	tabId: number,
