// Shared bridge metadata and tab helpers.
...
function browserPilotBridgeInfo() {
	...
	return {
		...
		runtimeRecovery: recovery
			...
			: null,
		lostHookSessions: (() => {
			const lost = lostHookSessionsGetter?.() || [];
...
async function getExtensionInstanceId(): Promise<string | undefined> {
	...
	try {
		...
		const existing =
			raw && typeof raw === "object" ? (raw as Record<string, unknown>)[EXTENSION_INSTANCE_ID_KEY] : undefined;
...
export {
	...
	isScriptable,
	registerLostHookSessionsGetter,
