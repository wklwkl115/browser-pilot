...
function recordBrowserPilotDocumentCommit(
	details: JsonRecord & { tabId?: number; frameId?: number; documentId?: string; url?: string },
): ExtensionPageIdentity | undefined {
...
function recordBrowserPilotSameDocumentUpdate(
	details: JsonRecord & { tabId?: number; frameId?: number; url?: string },
): ExtensionPageIdentity | undefined {
	if (Number(details.frameId ?? 0) !== 0) return undefined;
	return ensureBrowserPilotPageIdentity(details.tabId, details.url);
}

function replaceBrowserPilotPageIdentity(
...
function browserPilotPageIdentityFields(tab: BrowserPilotChromeTab): Record<string, unknown> {
	...
	return identity
		? { pageEpoch: identity.pageEpoch, ...(identity.documentId ? { documentId: identity.documentId } : {}) }
		: {};
}
