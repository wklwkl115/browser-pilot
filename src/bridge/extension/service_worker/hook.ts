// Browser Pilot hook/session commands.

import { chromeApi as chrome } from "./runtimeEnv";
import {
	BROWSER_PILOT_ERROR_CODES,
	BROWSER_PILOT_HOOK_DISPATCHER_FILE,
...
async function confirmBrowserPilotDispatcher(tabId: number, method: string): Promise<BrowserPilotBridgeResponse> {
	...
	return browserPilotError(
		BROWSER_PILOT_ERROR_CODES.INJECTION_FAILED,
		"Browser Pilot dispatcher readiness check failed",
		{ method, ping },
	);
}
...
const BROWSER_PILOT_HOOK_TARGETS: Record<string, { target: string; description: string; eventTypes: string[] }> = {
	...
	networkApi: {
		target: "network",
		description: "page fetch/XMLHttpRequest API call evidence, not passive Network recorder replacement",
		eventTypes: ["network.request", "network.response", "network.error"],
	},
	websocket: {
		target: "websocket",
		description: "WebSocket constructor, send, message, close, and error events",
	...
	crypto: {
		target: "crypto",
		description: "crypto.getRandomValues and crypto.subtle operation metadata",
		eventTypes: ["crypto.*"],
	},
	dom: {
		target: "dom",
		description: "MutationObserver DOM mutation summaries",
		eventTypes: ["dom.mutation", "dom.mutation.overflow", "dom.mutation.queue_overflow"],
	},
	domSinks: {
		target: "dom_sinks",
		description: "innerHTML/outerHTML/insertAdjacentHTML/document.write script text writes",
		eventTypes: ["dom.sink.*"],
	},
	cookies: {
		target: "cookies",
...
function listBrowserPilotHookTargets(): Array<JsonRecord> {
	return Object.entries(BROWSER_PILOT_HOOK_TARGETS).map(([id, spec]) => ({
		id,
		target: spec.target,
...
function expandBrowserPilotHookTargets(input: unknown): {
	targets: JsonRecord;
	expanded: Array<JsonRecord>;
	rejected: string[];
} {
	const requested = Array.isArray(input) ? input : input == null || input === "" ? [] : [input];
	const ids = requested.length
		? requested.map(normalizeHookTargetName).filter((item): item is string => Boolean(item))
		: [];
	const rejected = requested
		.map((item, index) => ({ item, id: normalizeHookTargetName(item), index }))
...
function hookInstallArgsFromMessage(msg: BrowserPilotBridgeCommand, targetOverride?: JsonRecord): JsonRecord {
	return {
		...
		options: msg.options,
		buffer_size:
			msg.bufferSize === undefined && msg.buffer_size === undefined
				? undefined
				: Number(msg.bufferSize ?? msg.buffer_size),
	...
	try {
		await browserPilotWithTimeout(
			chrome.scripting.executeScript({
				target: { tabId },
				world: "MAIN",
				files: [BROWSER_PILOT_HOOK_DISPATCHER_FILE],
			}),
			timeoutMs,
		...
		if (cdp.ok) {
			...
			if (confirmed.ok)
				return {
					ok: true,
					data: {
						...(confirmed.data && typeof confirmed.data === "object" ? (confirmed.data as JsonRecord) : {}),
						fallback_reason: scriptingErr instanceof Error ? scriptingErr.message : String(scriptingErr),
					},
				};
			return browserPilotError(
				BROWSER_PILOT_ERROR_CODES.INJECTION_FAILED,
				...
				{
					scripting: scriptingErr instanceof Error ? scriptingErr.message : String(scriptingErr),
					confirm: confirmed,
				},
			);
		}
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INJECTION_FAILED, "hook.install CDP fallback failed", {
			scripting: scriptingErr instanceof Error ? scriptingErr.message : String(scriptingErr),
			cdp: cdp,
		});
	} catch (cdpErr) {
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INJECTION_FAILED, "hook.install injection failed", {
			scripting: scriptingErr instanceof Error ? scriptingErr.message : String(scriptingErr),
...
) => BrowserPilotBridgeResponse | Promise<BrowserPilotBridgeResponse>;
type HookInstallTargets = {
	targetOverride?: JsonRecord;
	expandedTargets?: Array<JsonRecord>;
	rejectedTargets?: string[];
	error?: BrowserPilotBridgeResponse;
};
...
function listHookSessions(tabId: number, msg: BrowserPilotBridgeCommand): BrowserPilotBridgeResponse {
	...
	return {
		ok: true,
		data: {
			tabId: explicitTab ? Number(tabId) : undefined,
			sessions: sessionEntries.map(([tid, session]) => ({ tabId: tid, ...session })),
...
function listHookTargets(): BrowserPilotBridgeResponse {
	...
	return {
		...
		data: {
			...
			rejectedStrategyPresets: REJECTED_HOOK_STRATEGY_PRESETS.slice(),
		},
	};
}
...
function resolveHookInstallTargets(cmd: string, msg: BrowserPilotBridgeCommand): HookInstallTargets {
	...
	if (expanded.rejected.length)
		return {
			error: browserPilotError(
				...
				{ rejected: expanded.rejected, supported },
			),
		};
	if (!expanded.expanded.length)
		return {
			error: browserPilotError(
				BROWSER_PILOT_ERROR_CODES.INVALID_RULE,
				"hook.install requires at least one explicit hook target id",
				{ supported },
			),
		};
...
function installedHookSession(
	...
	return {
		...
		install_fingerprint:
			data.install_fingerprint !== undefined
...
async function recordSuccessfulHookInstall(
	tabId: number,
	msg: BrowserPilotBridgeCommand,
	response: BrowserPilotBridgeResponse,
	beforeStatus: BrowserPilotBridgeResponse | null,
	args: JsonRecord,
	targets: HookInstallTargets,
): Promise<void> {
	const data = hookResponseData(response);
	data.reused = data.idempotent === true || data.already_installed === true;
	data.preinstall_status = beforeStatus?.ok
		? "installed"
		: beforeStatus?.error_code
	...
	try {
		await persistState(
			...
			compactConfig({
				...
				targets: args.targets,
				options: msg.options,
				buffer_size: msg.bufferSize ?? msg.buffer_size,
			}),
			{ tabId, sessionId, recoveryPolicy: "manual" },
		);
	} catch (error) {
		console.warn("[BROWSER-PILOT-HOOK] Failed to persist hook session state", sessionId, error);
	}
}
...
async function installHook(
	...
	if (!response.ok && response.error_code === BROWSER_PILOT_ERROR_CODES.ALREADY_INSTALLED) {
		...
		if (response.ok && response.data && typeof response.data === "object")
			(response.data as JsonRecord).reconfigured = true;
	}
...
async function hookStatus(
	...
	if (response.ok && session)
		browserPilotSessions.set(tabId, {
			...session,
			state: String(hookResponseData(response).state || session.state || ""),
		});
	return response;
}
...
async function collectHookEvents(
	...
	return await callPageBrowserPilot(
		tabId,
		"hook.collect",
		{
			...
			timeout_ms: msg.timeoutMs ?? msg.timeout_ms,
			min_count: msg.minCount ?? msg.min_count,
		},
		{ timeoutMs: msg.timeoutMs ?? msg.timeout_ms },
	);
}

async function runHookSessionCommand(
	cmd: string,
	tabId: number,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
...
function shouldCleanupHookUninstall(response: BrowserPilotBridgeResponse): boolean {
	return (
		response.ok ||
		response.error_code === BROWSER_PILOT_ERROR_CODES.NO_SESSION ||
		response.error_code === BROWSER_PILOT_ERROR_CODES.NOT_INSTALLED
	);
}

async function cleanupUninstalledHook(
	tabId: number,
	msg: BrowserPilotBridgeCommand,
	response: BrowserPilotBridgeResponse,
	sessionId: string,
	...
	const listenerCleanup:
		BrowserPilotBridgeResponse | { ok: true; data: { tabId: number; skipped: true; warning: string } } =
		typeof cleanupBrowserPilotPageListenersForTab === "function"
			? await (
					cleanupBrowserPilotPageListenersForTab as (
						...args: unknown[]
					) => Promise<BrowserPilotBridgeResponse>
				)(tabId, "hook_uninstall", msg.timeoutMs || msg.timeout_ms || 5000).catch((error) =>
					browserPilotError(
			...
			: {
					ok: true,
					data: { tabId: Number(tabId), skipped: true, warning: "page listener cleanup helper unavailable" },
				};
	const cleanup = listenerCleanup.data || listenerCleanup;
	if (response.ok && response.data && typeof response.data === "object")
		(response.data as JsonRecord).listener_cleanup = cleanup;
	...
	try {
		await forgetState("hook", `${Number(tabId)}:${sessionId}`);
	} catch (error) {
		console.warn("[BROWSER-PILOT-HOOK] Failed to forget hook session state", sessionId, error);
	}
}
...
async function uninstallHook(
	...
	if (requestedSessionId && localSession?.session_id && String(localSession.session_id) !== requestedSessionId) {
		return browserPilotError(
			BROWSER_PILOT_ERROR_CODES.INVALID_SESSION,
	...
	if (shouldCleanupHookUninstall(response))
		await cleanupUninstalledHook(
			...
			msg,
			response,
			localSession?.session_id ? String(localSession.session_id) : "default",
		);
	return response;
}
...
const HOOK_COMMAND_HANDLERS: Readonly<Record<string, HookCommandHandler>> = {
	...
	"hook.evaluate": (_cmd, tabId, msg) =>
		browserPilotEval(tabId, String(msg.expression || ""), msg.awaitPromise !== false, {
			timeoutMs: msg.timeoutMs ?? msg.timeout_ms,
		}),
	...
	"hook.addEventListener": async (_cmd, tabId, msg) =>
		(await addEventListener(tabId, msg)) as BrowserPilotBridgeResponse,
	"hook.removeEventListener": async (_cmd, tabId, msg) =>
		(await removeEventListener(tabId, msg)) as BrowserPilotBridgeResponse,
	"hook.getPerformanceEntries": async (_cmd, tabId, msg) =>
		(await getPerformanceEntries(tabId, msg)) as BrowserPilotBridgeResponse,
};

async function handleBrowserPilotHookCommand(
	cmd: string,
	tabId: number,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
	const handler = Object.prototype.hasOwnProperty.call(HOOK_COMMAND_HANDLERS, cmd)
		? HOOK_COMMAND_HANDLERS[cmd]
		: undefined;
	return handler
		? await handler(cmd, tabId, msg)
		: browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "Unknown Browser Pilot hook command: " + cmd, {
				cmd,
			});
}
...
registerRecovery(async (results) => {
	const result = await recoverState("hook", {
		...
		recover: async (record) => {
			...
			if (browserPilotSessions.has(key))
				return { recovered: false, historyLost: true, reason: "session already exists" };
			try {
				// Check if dispatcher still exists on the page
				const ping = await callPageBrowserPilot(tabId, "hook.status", { session_id: sessionId }).catch(
					() => null,
				);
				if (ping && ping.ok) {
					...
					browserPilotSessions.set(key, {
						...
						install_fingerprint:
							data.install_fingerprint !== undefined ? String(data.install_fingerprint) : undefined,
					});
...
export {
	injectBrowserPilotDispatcherViaCdp,
	confirmBrowserPilotDispatcher,
	browserPilotHookSessionId,
	browserPilotHookSessionArgs,
