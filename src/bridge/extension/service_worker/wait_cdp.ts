...
import {
	...
	runtimeErrorMessage as waitCdpErrorMessage,
	runtimeRecord as waitCdpRecord,
} from "./runtimeSupport.js";
import type {
	JsonRecord,
	BrowserPilotCdpDomainRef,
	BrowserPilotCdpSubscription,
	BrowserPilotWaitRecord,
} from "./types";

// Browser Pilot wait CDP domain refcount, subscription and diagnostics helpers.
...
function browserPilotCdpDomainKey(tabId: unknown, domain: unknown): string {
	return Number(tabId) + ":" + String(domain);
}
function browserPilotCdpHolderId(record: Partial<BrowserPilotWaitRecord> | null | undefined): string {
...
function rememberBrowserPilotCdpCleanup(entry: JsonRecord = {}): void {
	...
	if (browserPilotCdpCleanupHistory.length > 200)
		browserPilotCdpCleanupHistory.splice(0, browserPilotCdpCleanupHistory.length - 200);
}
async function sendBrowserPilotCdpDomainCommand(
	tabId: number,
	domain: string,
	action: string,
	...
	if (cdp?.send && modeHint !== "chrome.debugger") {
		const resp = normalizePersistentBrowserPilotResponse(
			await cdp.send(tabId, method, {}, { name: "wait", persistent: true }),
		);
	...
	if (action === "enable") {
		await chrome.debugger.attach({ tabId: Number(tabId) }, "1.3").catch((e: unknown) => {
			if (!/Another debugger|already attached|Debugger is already attached/i.test(waitCdpErrorMessage(e)))
				throw e;
		});
	}
...
function scheduleBrowserPilotCdpDomainDisable(
	ref: BrowserPilotCdpDomainRef | null | undefined,
	reason?: string,
	holderId?: string,
	action?: string,
): boolean {
	...
	ref.disablePromise = sendBrowserPilotCdpDomainCommand(tabId, domain, "disable", mode)
		.then(() => {
		...
		.catch((e) => {
			...
			if (current === ref && current.disableToken === token) {
				...
				if (
					...
					/no tab|no target|closed|detached/i.test(errorText)
				) {
...
function releaseBrowserPilotCdpDomains(
	record: BrowserPilotWaitRecord | null | undefined,
	domains: Iterable<string> | string[] | undefined,
	reason?: string,
) {
	...
	for (const domain of unique.reverse()) {
		...
		if (!ref) {
			try {
				...
				/* best-effort cdp domain set cleanup */
			}
			continue;
		}
...
function forceReleaseBrowserPilotCdpDomainsForTab(tabId: unknown, reason?: string) {
	...
	for (const [, ref] of Array.from(browserPilotCdpDomainRefs.entries())) {
		...
		const holders = Array.from(ref.holders.values()).map((holder) => ({
		...
		ref.count = 0;
		if (
			scheduleBrowserPilotCdpDomainDisable(ref, reason, holders.map((h) => h.holderId).join(","), "force_disable")
		)
			disabled += 1;
	}
	return { released, disabled };
}
async function enableBrowserPilotCdpDomains(
...
async function attachDebuggerForWait(record: BrowserPilotWaitRecord, domains: Iterable<string> | string[] | undefined) {
	return await enableBrowserPilotCdpDomains(record, domains);
}
function subscribeBrowserPilotCdp(
	tabId: number,
	event: string | string[],
	handler: (source: { tabId?: number }, method: string, params: JsonRecord) => void,
	record?: BrowserPilotCdpSubscriptionRecord | null,
): string | null {
	...
	const rec: BrowserPilotCdpSubscription = {
		subscriptionId,
		tabId: Number(tabId),
		events,
		createdAt: Date.now(),
	...
	const set = browserPilotCdpTabRefs.get(Number(tabId)) || new Set();
	set.add(subscriptionId);
	browserPilotCdpTabRefs.set(Number(tabId), set);
...
function unsubscribeBrowserPilotCdp(subscriptionId: string): boolean {
	...
	if (!rec) return false;
	try {
		chrome.debugger.onEvent.removeListener(rec.handler);
	} catch (error) {
		console.warn("[BROWSER-PILOT-WAIT] Failed to remove debugger event listener", subscriptionId, error);
	}
	...
	if (set) {
		...
		if (!set.size) browserPilotCdpTabRefs.delete(Number(rec.tabId));
	}
	return true;
}
...
function diagnoseBrowserPilotCdpSubscriptions(tabId?: unknown) {
	return Array.from(browserPilotCdpSubscriptions.values())
		.filter((s) => tabId === undefined || Number(s.tabId) === Number(tabId))
		.map((s) => ({
...
function diagnoseBrowserPilotCdpDomainRefs(tabId?: unknown) {
	return Array.from(browserPilotCdpDomainRefs.values())
		.filter((r) => tabId === undefined || Number(r.tabId) === Number(tabId))
		.map((r) => ({
			key: r.key,
			tabId: r.tabId,
			domain: r.domain,
...
function diagnoseBrowserPilotCdpCleanupHistory(tabId?: unknown) {
	return browserPilotCdpCleanupHistory
		.filter((e) => tabId === undefined || Number(e.tabId) === Number(tabId))
		.slice(-50)
		.map((e) => ({ ...e, age_ms: Date.now() - e.t }));
}
export {
	browserPilotCdpSubscriptions,
	browserPilotCdpTabRefs,
	browserPilotCdpDomainRefs,
	browserPilotCdpCleanupHistory,
	browserPilotCdpSubSeq,
	browserPilotCdpDomainKey,
	browserPilotCdpHolderId,
	rememberBrowserPilotCdpCleanup,
	sendBrowserPilotCdpDomainCommand,
	acquireBrowserPilotCdpDomain,
	scheduleBrowserPilotCdpDomainDisable,
	releaseBrowserPilotCdpDomains,
	forceReleaseBrowserPilotCdpDomainsForTab,
