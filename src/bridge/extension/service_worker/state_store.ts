...
type RuntimeStateKind = "network" | "intercept" | "hook" | "ws" | "cdp";
...
async function findLostRuntimeSession(
	kind: RuntimeStateKind,
	tabId: number,
	sessionId: string,
	...
	return {
		...
		workerBootId: record.workerBootId,
		updatedAt: record.updatedAt,
		details: runtimeRecord(record.config),
	};
}
...
function summarizeLostRuntimeSession(record: JsonRecord | undefined): JsonRecord | undefined {
	...
	return {
		...
		tabId: record.tabId,
		sessionId: record.sessionId,
		details: runtimeRecord(record.details),
	};
}
...
function enqueueWrite(task: () => Promise<StateStoreWriteResult>): Promise<StateStoreWriteResult> {
	const next = writeChain.catch(() => ({ ok: true }) as StateStoreWriteResult).then(task);
...
async function recover(
	...
	opts: {
		...
		) => Promise<{ recovered: boolean; historyLost: boolean; reason?: string }>;
		validateTab?: boolean;
	} = {},
...
async function runStartupRecovery(): Promise<RuntimeRecoverySummary> {
	...
	for (const registrar of recoveryRegistrars) {
		try {
			...
			console.warn(
				"[BROWSER-PILOT-STATE] Recovery registrar failed",
