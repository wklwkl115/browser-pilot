// Browser Pilot Network recorder CDP event, lifecycle and command runtime.
...
import {
	enableBrowserPilotCdpDomains,
	releaseBrowserPilotCdpDomains,
	subscribeBrowserPilotCdp,
	unsubscribeBrowserPilotCdp,
} from "./wait_cdp";
import { makeWaitId, normalizeBrowserPilotTimeoutMs } from "./wait_coordinator";
import {
	appendBounded,
	browserPilotNetworkHandleRecorderCdpEvent,
	browserPilotNetworkMaybeCaptureBody,
} from "./network_events";
import {
...
const findLostNetworkRuntimeSession =
	typeof findLostRuntimeSession === "function" ? findLostRuntimeSession : async () => undefined;
const summarizeLostNetworkRuntimeSession =
	typeof summarizeLostRuntimeSession === "function" ? summarizeLostRuntimeSession : () => undefined;
type NetworkCommandResult = Record<string, unknown>;
type NetworkRecorderLookup =
	{ recorder: NetworkRecorder; error?: never } | { recorder?: never; error: BrowserPilotBridgeResponse };
...
async function cdpSendNetworkCommand(
	tabId: number,
	method: string,
	params: JsonRecord = {},
	timeoutMs?: number,
	...
	if (cdp?.send) {
		const resp = normalizePersistentBrowserPilotResponse(
			await cdp.send(tabId, method, params || {}, { persistent: true, name: "network_recorder", timeoutMs }),
		);
		const error = asRecord(resp?.error);
		if (!resp || resp.ok === false)
			throw new Error(String(error.message || resp?.message || resp?.error || method + " failed"));
		const data = asRecord(resp.data);
		return asRecord(data.result || resp.result || resp.data || {});
	}
	return asRecord(
		await browserPilotWithTimeout(
			chrome.debugger.sendCommand({ tabId: Number(tabId) }, method, params || {}),
			timeoutMs || 5000,
			method,
		),
	);
...
async function maybeCaptureNetworkBody(recorder: NetworkRecorder, rec: NetworkRecord): Promise<void> {
	return await browserPilotNetworkMaybeCaptureBody(recorder, rec, {
		cdpSendNetworkCommand,
		classifyNetworkBodyError,
		wakeNetworkWaits,
	});
}
function handleNetworkRecorderCdpEvent(
	recorder: NetworkRecorder,
	source: { tabId?: number },
	method: string,
	params: JsonRecord = {},
): void {
	browserPilotNetworkHandleRecorderCdpEvent(recorder, source, method, params, {
		wakeNetworkWaits,
...
async function activateNetworkRecorder(recorder: NetworkRecorder): Promise<void> {
	...
	if (config.storePostData)
		try {
			await cdpSendNetworkCommand(tabId, "Network.enable", { maxPostDataSize: config.maxPostDataBytes }, 2000);
		} catch (error) {
			rememberNetworkError(recorder, "Network.enable.maxPostDataSize", error);
		}
	subscribeBrowserPilotCdp(
		tabId,
		NETWORK_RECORDER_EVENTS,
		(source, method, params) => handleNetworkRecorderCdpEvent(recorder, source, method, params),
		recorder.cdpRecord,
	);
	try {
		await cdpSendNetworkCommand(tabId, "Page.setLifecycleEventsEnabled", { enabled: true }, 2000);
	} catch (error) {
		rememberNetworkError(recorder, "Page.setLifecycleEventsEnabled", error);
	}
...
async function persistNetworkRecorderState(recorder: NetworkRecorder, action: string): Promise<void> {
	try {
		const persisted = await persistState(
			"network",
			networkRecorderKey(recorder.tabId, recorder.config.sessionId),
			recorderPublicConfig(recorder.config),
			{ tabId: recorder.tabId, sessionId: recorder.config.sessionId, recoveryPolicy: "auto" },
		);
		if (persisted.generation !== undefined) recorder.stateGeneration = Number(persisted.generation);
		if (!persisted.ok && persisted.error)
			rememberNetworkDiagnostic(recorder, { t: Date.now(), action: "persist_failed", error: persisted.error });
	} catch (error) {
		console.warn(
			`[BROWSER-PILOT-NET] Failed to persist recorder state during ${action}`,
			recorder.config.sessionId,
			error,
		);
	}
}
async function startNetworkRecorder(
	tabId: number,
	...
	if (recorder && recorder.active)
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.ALREADY_INSTALLED, "network recorder already started", {
			tabId,
			sessionId: config.sessionId,
		});
	if (recorder)
		await stopNetworkRecorder(tabId, { sessionId: config.sessionId, keepBuffer: false, reason: "restart" }).catch(
			() => {},
		);
	...
	try {
		...
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR, "network.start failed", {
			tabId,
			sessionId: config.sessionId,
			error: errorText(e),
		});
...
function cleanupNetworkRecorder(
	recorder: NetworkRecorder | null | undefined,
	reason?: string,
	options: { keepBuffer?: boolean } = {},
): { stopped: boolean; summary?: NetworkRecorderSummary | null } {
	...
	rememberNetworkDiagnostic(recorder, {
		...
		action: "stop",
		reason: reason || "stopped",
		keepBuffer: options.keepBuffer !== false,
	});
...
async function stopNetworkRecorder(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	if (!recorder)
		return browserPilotError(
			BROWSER_PILOT_ERROR_CODES.NETWORK_RECORDER_NOT_STARTED,
	...
	try {
		await forgetState("network", networkRecorderKey(tabId, sessionId));
	} catch (error) {
		console.warn("[BROWSER-PILOT-NET] Failed to forget recorder state on stop", sessionId, error);
	}
...
function cleanupNetworkRecorderTab(tabId: number, reason?: string): Array<{ sessionId: string; recorderId: string }> {
	...
	for (const recorder of Array.from(browserPilotNetworkRecorders.values())) {
		...
		void forgetState("network", recorder.key).catch((error) =>
			console.warn("[BROWSER-PILOT-NET] Failed to forget recorder state during tab cleanup", recorder.key, error),
		);
...
async function listNetworkRecorderEntries(
	tabId: number,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
	...
	const items = all.slice(offset, limit ? offset + limit : undefined).map((rec) =>
		networkRecordSummary(rec, {
			includeDetails: msg.includeDetails === true || msg.include_details === true,
			includeBody: msg.includeBody === true || msg.include_body === true,
		}),
	);
...
async function getNetworkRecorderEntry(
	tabId: number,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
	const found = await requireNetworkRecorder(tabId, msg);
	...
	if (!id)
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "network.get requires requestId or id", {
			tabId,
		});
	const rec = findNetworkRecorderEntry(recorder, id);
	if (!rec)
		return browserPilotError(
			BROWSER_PILOT_ERROR_CODES.REQUEST_NOT_FOUND || "REQUEST_NOT_FOUND",
			"network request not found",
			{ tabId, sessionId: recorder.sessionId, requestId: id },
		);
	return {
		ok: true,
		data: networkRecordClone(rec, { includeBody: msg.includeBody === true || msg.include_body === true }),
	};
}
function findNetworkRecorderEntry(recorder: NetworkRecorder, id: string): NetworkRecord | undefined {
	return (
		recorder.byRequestId.get(id) ||
		recorder.entries.find((entry) => String(entry.id) === id || String(entry.requestId) === id)
	);
}
function unavailableNetworkBody(
	tabId: number,
	recorder: NetworkRecorder,
	requestId: string,
	ref: string,
	rec: NetworkRecord | undefined,
): BrowserPilotBridgeResponse {
...
async function getNetworkRecorderBody(
	tabId: number,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
	const found = await requireNetworkRecorder(tabId, msg);
	...
	const bodyRef = firstTruthyOr<string | undefined>(
		undefined,
		ref,
		rec?.bodyRef,
		recorder.bodyByRequestId.get(requestId),
	);
	if (!bodyRef) return unavailableNetworkBody(tabId, recorder, requestId, ref, rec);
	const body = recorder.bodyStore.get(bodyRef);
	if (!body)
		return browserPilotError(
			BROWSER_PILOT_ERROR_CODES.BODY_UNAVAILABLE,
	...
	if (maxBytesRaw !== undefined) {
		const maxBytes = numberInRange(
			...
			body.bytes,
			0,
			Math.max(body.bytes || 0, body.originalLength || 0, 10 * 1024 * 1024),
		);
		const trunc = body.base64Encoded
			? truncateBase64Body(body.body || "", maxBytes)
			: truncateStringByBytes(body.body || "", maxBytes);
		out = {
			...out,
			body: trunc.value,
			bodyTruncated: firstTruthyOr(false, body.bodyTruncated, trunc.truncated),
			bytes: trunc.bytes,
		};
	}
...
function makeHarEntry(rec: NetworkRecord, body: NetworkBodyStoreEntry | null): NetworkHarEntry {
	...
	const startedDateTime = rec.wallTime
		...
		: new Date(rec.createdAt || Date.now()).toISOString();
	...
	if (body) {
		...
		content.encoding = body.base64Encoded ? "base64" : undefined;
	...
	return {
		...
		time: Math.max(
			0,
			Number(firstTruthyOr(Date.now(), rec.finishedAt, rec.updatedAt)) -
				Number(firstTruthyOr(Date.now(), rec.createdAt)),
		),
		request: {
			method: firstTruthyOr("GET", request.method),
			url: firstTruthyOr("", request.url, response.url),
			httpVersion: firstTruthyOr("HTTP/1.1", response.protocol),
			headers: requestHeaders,
			queryString: [],
			cookies: [],
			headersSize: -1,
			bodySize: postData ? estimateStringBytes(postData) : 0,
			postData: postData
				? {
						mimeType: getHeaderValue(request.headers, "content-type"),
						text: postData,
		...
		response: {
			status: Number(firstTruthyOr(0, response.status, responseExtra.statusCode)),
			statusText: firstTruthyOr("", response.statusText),
			httpVersion: firstTruthyOr("HTTP/1.1", response.protocol),
			headers: responseHeaders,
			cookies: [],
			content,
			redirectURL: getHeaderValue(response.headers, "location"),
			headersSize: -1,
			bodySize: firstTruthyOr(-1, rec.data.encodedDataLength, rec.encodedDataLength),
			_error: firstTruthyOr(undefined, rec.errorText),
		},
		cache: {},
...
async function exportNetworkRecorderHar(
	tabId: number,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
	const found = await requireNetworkRecorder(tabId, msg);
	...
	const recorder = found.recorder;
	const includeBodies =
		msg.includeBody === true ||
		msg.include_body === true ||
		msg.includeBodies === true ||
		msg.include_bodies === true;
	const filters = {
		sinceSeq: msg.sinceSeq ?? msg.since_seq,
		url: msg.url,
		urlContains: msg.urlContains ?? msg.url_contains,
	...
	if (String(msg.format || "").toLowerCase() === "json") {
		...
		return {
			...
			data: {
				...
				entries: records.map((r) => networkRecordClone(r, { includeBody: includeBodies })),
				bodies,
			},
		};
	}
	const entries = records.map((rec) =>
		makeHarEntry(rec, includeBodies && rec.bodyRef ? recorder.bodyStore.get(rec.bodyRef) || null : null),
	);
	return {
		ok: true,
		data: {
			log: {
				version: "1.2",
				creator: { name: "Browser Pilot NetworkRecorder", version: "1.0" },
				pages: [],
				entries,
			},
...
function networkRecordMatchesWaitCriteria(
	recorder: NetworkRecorder,
	...
	if (
		wsFrame !== undefined &&
		!firstTruthyOr<NetworkFrameRecord[]>([], record.wsFrames).some((frame) => networkWsFrameMatches(frame, wsFrame))
	)
		return false;
	const sseEvent = firstDefinedOr(undefined, criteria.sseEvent, criteria.sse_event);
	if (
		sseEvent !== undefined &&
		!firstTruthyOr<NetworkFrameRecord[]>([], record.sseEvents).some((event) =>
...
function networkWaitMatches(
	recorder: NetworkRecorder,
	wait: Pick<NetworkRecorderWait, "condition" | "criteria" | "idleMs" | "count">,
	eventType: string,
	rec: NetworkRecord | null,
): boolean {
	...
	if (condition === "idle") {
		const quietFor =
			Date.now() - Number(firstTruthyOr(recorder.createdAt, recorder.lastEventAt, recorder.startedAt));
...
function finishNetworkRecorderWait(
	...
	if (ok) {
		...
		if (
			errorCode === BROWSER_PILOT_ERROR_CODES.TIMEOUT ||
			errorCode === BROWSER_PILOT_ERROR_CODES.NETWORK_RECORDER_TIMEOUT
		)
...
function wakeNetworkWaits(
	...
	for (const wait of Array.from(recorder.waits.values())) {
		...
		if (networkWaitMatches(recorder, wait, eventType, rec)) {
			...
			finishNetworkRecorderWait(recorder, wait, true, null, null, {
				event: eventType,
				request: rec ? networkRecordSummary(rec, { includeDetails: true }) : null,
			});
		}
	}
}
function networkWaitIdleMs(msg: BrowserPilotBridgeCommand, timeoutMs: number): number {
	return numberInRange(
		firstDefinedOr<unknown>(undefined, msg.idleMs, msg.idle_ms),
		500,
		50,
		Math.max(50, firstTruthyOr(300000, timeoutMs)),
	);
}
...
function immediateCountNetworkMatch(recorder: NetworkRecorder, criteria: JsonRecord, count: number): JsonRecord | null {
	...
	return matches.length >= count
		? {
				...
				required: count,
				requests: matches.slice(-10).map((record) => networkRecordSummary(record)),
			}
		: null;
}
function immediateNetworkRecordEvents(record: NetworkRecord, condition: string): string[] {
	...
	if (
		...
		firstTruthyOr<NetworkFrameRecord[]>([], record.wsFrames).length
	)
		events.push("websocket");
	if (
		(condition === "sse" || condition === "eventsource") &&
		firstTruthyOr<NetworkFrameRecord[]>([], record.sseEvents).length
	)
		events.push("sse");
	return events;
}
function immediateEventNetworkMatch(
	recorder: NetworkRecorder,
	condition: string,
	criteria: JsonRecord,
): JsonRecord | null {
	...
	for (const record of recorder.entries) {
		for (const event of immediateNetworkRecordEvents(record, condition)) {
			if (networkWaitMatches(recorder, wait, event, record))
				return { event, request: networkRecordSummary(record, { includeDetails: true }) };
		}
	}
	return null;
}
function immediateNetworkWaitMatch(
	recorder: NetworkRecorder,
	condition: string,
	criteria: JsonRecord,
	msg: BrowserPilotBridgeCommand,
...
async function waitNetworkRecorder(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	if (instant || timeoutMs === 0) {
		...
		return browserPilotError(
			BROWSER_PILOT_ERROR_CODES.NETWORK_RECORDER_TIMEOUT,
	...
	return await new Promise<BrowserPilotBridgeResponse>((resolve) => {
		...
		const wait: NetworkRecorderWait = {
			waitId,
			condition,
			criteria,
			createdAt: Date.now(),
			...
			idleMs: networkWaitIdleMs(msg, timeoutMs),
			count: networkWaitCount(msg),
			lastMatchSeq: 0,
		};
		wait.abortHandler = () =>
			finishNetworkRecorderWait(
		...
		if (condition === "idle" || condition === "count")
		...
		if (previous)
			finishNetworkRecorderWait(
				...
				false,
				BROWSER_PILOT_ERROR_CODES.CANCELLED,
				"network wait replaced",
				{ waitId },
			);
		recorder.waits.set(waitId, wait);
	});
}
async function handleNetworkRecorderCommand(
	tabId: number,
	cmd: string,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
	...
	switch (cmd) {
		case "network.start":
			return await startNetworkRecorder(tabId, command);
		case "network.stop":
			return await stopNetworkRecorder(tabId, command);
		case "network.status": {
			const recorder = getActiveNetworkRecorder(tabId, command);
			if (!recorder) {
				const sessionId = defaultNetworkSessionId(command);
				const lost = summarizeLostNetworkRuntimeSession(
					await findLostNetworkRuntimeSession("network", tabId, sessionId),
				);
				return {
					ok: true,
					data: {
						...
						stateLost: !!lost,
						lostSession: lost,
						recorders: Array.from(browserPilotNetworkRecorders.values())
		...
		case "network.clear": {
			const found = await requireNetworkRecorder(tabId, command);
			if (found.error) return found.error;
			const cleared = clearNetworkRecorderBuffer(found.recorder);
			return { ok: true, data: { ...networkRecorderSummary(found.recorder), cleared } };
		}
		case "network.list":
			return await listNetworkRecorderEntries(tabId, command);
		case "network.get":
			return await getNetworkRecorderEntry(tabId, command);
		case "network.body":
			return await getNetworkRecorderBody(tabId, command);
		case "network.exportHar":
			return await exportNetworkRecorderHar(tabId, command);
		case "network.wait":
			return await waitNetworkRecorder(tabId, command);
	}
	return browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "Unknown network recorder command: " + cmd, {
		cmd,
		tabId,
	});
}
	...
	const result = await recoverState("network", {
		...
		recover: async (record) => {
			...
			if (!tabId || !configRecord)
				return { recovered: false, historyLost: true, reason: "missing tabId or config" };
			...
			if (browserPilotNetworkRecorders.has(key))
				return { recovered: false, historyLost: true, reason: "recorder already exists" };
			try {
