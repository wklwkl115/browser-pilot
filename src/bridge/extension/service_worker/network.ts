...
async function startNetworkRecorder(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
  ...
  if (recorder && recorder.active && msg.reconfigure !== false) {
    ...
    try {
      ...
      console.warn('[BROWSER-PILOT-NET] Failed to persist recorder state during reconfigure', config.sessionId, error);
    }
  ...
  try {
    ...
    try {
      ...
      console.warn('[BROWSER-PILOT-NET] Failed to persist recorder state during start', config.sessionId, error);
    }
...
async function getNetworkRecorderBody(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
  ...
  if (!bodyRef) {
    ...
    return browserPilotError(BROWSER_PILOT_ERROR_CODES.BODY_UNAVAILABLE, `network body is unavailable (${reason})`, { tabId, sessionId:recorder.sessionId, requestId, bodyRef:ref, bodyAvailability:rec?.bodyAvailability || 'not_requested', bodyUnavailableReason:reason, bodyError:rec?.bodyError || null, request:rec ? networkRecordSummary(rec) : null });
  }
