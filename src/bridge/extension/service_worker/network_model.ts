// Browser Pilot Network recorder state, config, filtering and body storage helpers.
...
function networkRecorderKey(tabId: unknown, sessionId: unknown): string {
	return Number(tabId) + ":" + String(sessionId || "default");
}
function defaultNetworkSessionId(msg: BrowserPilotBridgeCommand | JsonRecord | null | undefined): string {
	return String(msg?.sessionId || msg?.session_id || "default");
}
function getHeaderValue(headers: unknown, name: unknown): string {
	if (!headers || !name) return "";
	const target = String(name).toLowerCase();
	for (const [k, v] of Object.entries(asRecord(headers)))
		if (String(k).toLowerCase() === target) return String(v == null ? "" : v);
	return "";
}
function headersObjectToArray(headers: unknown): Array<{ name: string; value: string }> {
	return Object.entries(asRecord(headers)).map(([name, value]) => ({
		name,
...
function truncateStringByBytes(
	str: unknown,
	maxBytes: unknown,
): { value: string; truncated: boolean; originalLength: number; bytes: number } {
	const text = String(str == null ? "" : str);
...
function networkRecordFilterView(rec: NetworkRecord) {
	...
	return {
		url: String(request.url || rec.url || ""),
		type: String(rec.type || rec.resourceType || "").toLowerCase(),
		method: String(request.method || rec.method || "GET").toUpperCase(),
		status: Number(response.status ?? rec.status),
	};
}
function makeNetworkRecorderFilter(config: NetworkRecorderConfig): NetworkRecorderConfig["filter"] {
	...
	const resourceTypes = new Set(
		(Array.isArray(config.resourceTypes) ? config.resourceTypes : []).map((x) => String(x).toLowerCase()),
	);
	...
	const statuses = new Set(
		(Array.isArray(config.statuses) ? config.statuses : []).map((x) => Number(x)).filter(Number.isFinite),
	);
	return function networkRecorderFilter(rec: NetworkRecord, phase: string): NetworkFilterDecision {
		...
		if (includeUrls.length && !includeUrls.some((p) => matchNetworkPattern(view.url, p)))
			return { match: false, reason: "include_url" };
		if (excludeUrls.length && excludeUrls.some((p) => matchNetworkPattern(view.url, p)))
			return { match: false, reason: "exclude_url" };
		if (resourceTypes.size && !allResourceTypes && !resourceTypes.has(view.type))
			return { match: false, reason: "resource_type" };
...
function normalizeNetworkBodyMimeAllow(msg: BrowserPilotBridgeCommand | JsonRecord): string[] {
	...
	return normalizeNetworkStringList(raw, BROWSER_PILOT_NETWORK_DEFAULT_BODY_MIME_ALLOW).map((x) =>
		String(x).toLowerCase(),
	);
}
function networkResponseMimeType(rec: NetworkRecord): string {
	return String(
		rec?.response?.mimeType ||
			getHeaderValue(rec?.response?.headers, "content-type") ||
			getHeaderValue(rec?.responseExtraInfo?.headers, "content-type") ||
			"",
	).toLowerCase();
}
function isLikelyBinaryNetworkMime(mime: unknown): boolean {
	...
	return (
		...
		/(octet-stream|pdf|zip|gzip|br|protobuf|wasm|msword|excel|powerpoint)/i.test(text)
	);
}
function networkMimeMatchesToken(mime: string, token: unknown): boolean {
	...
	if (normalizedToken === "js" || normalizedToken === "javascript")
...
function networkBodyMimeDecision(config: NetworkRecorderConfig, rec: NetworkRecord): NetworkBodyMimeDecision {
	const allow = Array.isArray(config.bodyMimeAllow)
		? config.bodyMimeAllow
		: BROWSER_PILOT_NETWORK_DEFAULT_BODY_MIME_ALLOW;
	...
	if (allow.some((token) => networkMimeMatchesToken(mime, token)))
		return { match: true, reason: "mime_allowed", mimeType: mime };
	return {
		match: false,
		availability: isLikelyBinaryNetworkMime(mime) ? "binary" : "not_requested",
		reason: isLikelyBinaryNetworkMime(mime) ? "binary_mime" : "mime_not_allowed",
		mimeType: mime,
	};
}
function classifyNetworkBodyError(error: unknown): { availability: string; reason: string } {
	const text = errorText(error || "error");
	if (/No resource with given identifier|No data found|no resource|not found|expired|evicted/i.test(text))
		return { availability: "expired", reason: "cdp_body_expired" };
...
function setNetworkBodyAvailability(
	rec: NetworkRecord | null | undefined,
	availability: string,
	reason: string | null,
	extra: JsonRecord | null = null,
): void {
	if (!rec) return;
...
function networkConfigEnabled(msg: BrowserPilotBridgeCommand | JsonRecord, ...names: string[]): boolean {
	return names.every((name) => msg[name] !== false);
}
function normalizeNetworkRecorderConfig(msg: BrowserPilotBridgeCommand | JsonRecord = {}): NetworkRecorderConfig {
	...
	const maxEntries = numberInRange(
		networkConfigValue(msg, "maxEntries", "max_entries"),
		BROWSER_PILOT_NETWORK_DEFAULT_MAX_ENTRIES,
		1,
		20000,
	);
	const maxAgeMs = numberInRange(
		networkConfigValue(msg, "maxAgeMs", "max_age_ms"),
		BROWSER_PILOT_NETWORK_DEFAULT_MAX_AGE_MS,
		0,
		24 * 60 * 60 * 1000,
	);
	const maxBodyBytes = numberInRange(
		networkConfigValue(msg, "maxBodyBytes", "max_body_bytes"),
		BROWSER_PILOT_NETWORK_DEFAULT_MAX_BODY_BYTES,
		0,
		10 * 1024 * 1024,
	);
	const maxPostDataBytes = numberInRange(
		networkConfigValue(msg, "maxPostDataBytes", "max_post_data_bytes"),
		Math.min(maxBodyBytes, 65536),
		0,
		1024 * 1024,
	);
	const maxFrames = numberInRange(
		networkConfigValue(msg, "maxFrames", "max_frames", "maxWebSocketFrames", "max_websocket_frames"),
		BROWSER_PILOT_NETWORK_MAX_WS_FRAMES,
		0,
		5000,
	);
	const maxFrameBytes = numberInRange(
		networkConfigValue(
			msg,
			"maxFrameBytes",
			"max_frame_bytes",
	...
	const maxSseEvents = numberInRange(
		networkConfigValue(msg, "maxSseEvents", "max_sse_events"),
		BROWSER_PILOT_NETWORK_MAX_SSE_EVENTS,
		0,
		5000,
	);
	...
	const resourceTypes = rawResourceTypes.length
		? rawResourceTypes
		: captureBodies
			? BROWSER_PILOT_NETWORK_DEFAULT_BODY_RESOURCE_TYPES
			: [];
...
function createNetworkRecorder(tabId: unknown, config: NetworkRecorderConfig): NetworkRecorder {
	...
	const recorder = {
		...
		sessionId,
		key: networkRecorderKey(tabId, sessionId),
		recorderId: "netrec_" + Number(tabId) + "_" + ++browserPilotNetworkRecorderSeq,
		active: false,
		createdAt: Date.now(),
		startedAt: 0,
		stoppedAt: 0,
		...
		waits: new Map(),
		seqBase: browserPilotNetworkEntrySeq,
		counters: {
			request: 0,
			requestExtraInfo: 0,
			response: 0,
...
function getActiveNetworkRecorder(
	tabId: unknown,
...
function networkRecorderSummary(recorder: NetworkRecorder | null | undefined): NetworkRecorderSummary | null {
	...
	const activeWaits = Array.from(recorder.waits.values()).map((w) => ({
		waitId: w.waitId,
	...
	const summary: NetworkRecorderSummary = {
		...
		cdp: {
			...
			refs: diagnoseBrowserPilotCdpDomainRefs(recorder.tabId).filter((r) =>
...
function ensureNetworkEntry(recorder: NetworkRecorder, requestId: unknown): NetworkRecord {
	...
	if (!rec) {
		rec = {
			...
			tabId: recorder.tabId,
			sessionId: recorder.sessionId,
			seq: ++browserPilotNetworkEntrySeq,
			createdAt: Date.now(),
			updatedAt: Date.now(),
			wallTime: null,
			timestamp: null,
...
function pruneNetworkRecorder(recorder: NetworkRecorder | null | undefined): void {
	...
	if (maxAgeMs > 0) {
		while (
			...
			now - Number(recorder.entries[0].updatedAt || recorder.entries[0].createdAt || now) > maxAgeMs
		) {
			const old = recorder.entries.shift();
			if (old) {
				recorder.byRequestId.delete(old.requestId);
...
function networkRecordMatchesIdentity(rec: NetworkRecord, filters: JsonRecord): boolean {
	...
	if (filters.method && String(rec.request?.method || "").toUpperCase() !== String(filters.method).toUpperCase())
		return false;
...
function networkRecordMatchesUrl(rec: NetworkRecord, filters: JsonRecord): boolean {
	...
	if (
		...
		!filters.includeUrls.some((p) => matchNetworkPattern(url, String(p)))
	)
		return false;
	if (
		Array.isArray(filters.excludeUrls) &&
		filters.excludeUrls.length &&
		filters.excludeUrls.some((p) => matchNetworkPattern(url, String(p)))
	)
		return false;
	return true;
}
function networkRecordMatchesResponse(rec: NetworkRecord, filters: JsonRecord): boolean {
	...
	if (
		mime &&
		!matchNetworkPattern(
			String(rec.response?.mimeType || getHeaderValue(rec.response?.headers, "content-type") || ""),
			String(mime),
		)
	)
		return false;
...
function networkRecordMatchesList(rec: NetworkRecord, filters: JsonRecord | null | undefined): boolean {
	...
	return (
		networkRecordMatchesIdentity(rec, normalized) &&
		networkRecordMatchesUrl(rec, normalized) &&
		networkRecordMatchesResponse(rec, normalized)
	);
}
...
function networkWsFrameMatches(frame: NetworkFrameRecord, criterion: unknown): boolean {
	...
	if (criterion && typeof criterion === "object") {
		...
		if (criterionRecord.opcode !== undefined && Number(frame.opcode) !== Number(criterionRecord.opcode))
			return false;
		if (
			criterionRecord.payloadContains !== undefined &&
			!networkCriterionMatchesText(frame.payloadData, criterionRecord.payloadContains)
		)
			return false;
		if (
			criterionRecord.payload_contains !== undefined &&
			!networkCriterionMatchesText(frame.payloadData, criterionRecord.payload_contains)
		)
			return false;
		return true;
	}
...
function networkSseEventMatches(event: NetworkFrameRecord, criterion: unknown): boolean {
	...
	if (criterion && typeof criterion === "object") {
		const criterionRecord = criterion as JsonRecord;
		if (
			criterionRecord.eventName !== undefined &&
			!networkCriterionMatchesText(event.eventName, criterionRecord.eventName)
		)
			return false;
		if (
			criterionRecord.event_name !== undefined &&
			!networkCriterionMatchesText(event.eventName, criterionRecord.event_name)
		)
			return false;
		if (
			criterionRecord.eventId !== undefined &&
			!networkCriterionMatchesText(event.eventId, criterionRecord.eventId)
		)
			return false;
		if (
			criterionRecord.event_id !== undefined &&
			!networkCriterionMatchesText(event.eventId, criterionRecord.event_id)
		)
			return false;
		if (
			criterionRecord.dataContains !== undefined &&
			!networkCriterionMatchesText(event.data, criterionRecord.dataContains)
		)
			return false;
		if (
			criterionRecord.data_contains !== undefined &&
...
function networkRecordBodySummary(rec: NetworkRecord): JsonRecord {
	...
	return {
		...
		bodyAvailability: rec.bodyAvailability || (bodyRef ? "captured" : "not_requested"),
...
function networkRecordTrafficSummary(rec: NetworkRecord): JsonRecord {
	...
	return {
		encodedDataLength: data.encodedDataLength || 0,
		dataLength: data.dataLength || 0,
		hasPostData: !!request.hasPostData || request.postData !== undefined,
		postDataTruncated: !!request.postDataTruncated,
		postDataBytes: request.postData !== undefined ? estimateStringBytes(request.postData) : undefined,
		redirects: (rec.redirects || []).length,
		wsFrameCount: (rec.wsFrames || []).length,
		sseEventCount: (rec.sseEvents || []).length,
	};
}
function networkRecordSummary(
	rec: NetworkRecord,
	options: { includeDetails?: boolean; includeBody?: boolean } = {},
): NetworkRecordSummary {
	options = options || {};
...
function storeNetworkBody(
	...
	rec: NetworkRecord,
	bodyResult: JsonRecord | null | undefined,
): void {
	...
	const stored: NetworkBodyStoreEntry = {
		bodyRef,
	...
	rec.bodyRef = bodyRef;
	rec.bodyPreview = base64Encoded
		? null
		: truncateStringByBytes(trunc.value, Math.min(2048, recorder.config.maxBodyBytes)).value;
...
export {
	...
	browserPilotNetworkEntrySeq,
	browserPilotNetworkBodySeq,
	browserPilotNetworkWaitNotifier,
