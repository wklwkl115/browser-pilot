...
type NavigationDetails = JsonRecord & {
	tabId?: number;
	frameId?: number;
	url?: string;
	error?: string;
	transitionType?: string;
	transitionQualifiers?: unknown;
};
type NavigationState = {
	ok: boolean;
	url: string | null;
	tab: BrowserPilotChromeTab | null;
	metrics: LoadMetrics | null;
	stage: string | null;
	reason?: string;
};

// Browser Pilot navigation and load-state wait helpers.
// Loaded after wait_coordinator.js and before wait.js by background.js.

async function navigateBrowserPilot(
	tabId: number,
	...
	try {
		...
		return browserPilotError(
			...
			`wait.navigate requires a valid absolute URL (e.g. https://example.com); got ${JSON.stringify(String(url))}`,
			{ url },
		);
	}
	...
	if (cdp?.send) {
		const resp = normalizePersistentBrowserPilotResponse(
			await cdp.send(
				...
				"Page.navigate",
				{ url },
				{ persistent: true, name: msg.cdpSessionName || "navigate" },
			),
		);
	...
	try {
		...
		try {
			...
			console.warn(
				"[BROWSER-PILOT-WAIT] Failed to detach debugger after Page.navigate error",
				tabId,
				detachError,
			);
		}
...
async function navigateAndWait(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	if (!msg.url)
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "wait.navigateAndWait requires url", {});
	...
	if (waitUntil === "networkidle") waited = await waitForNetworkIdle(tabId, { ...msg, timeoutMs });
		...
		waited = await waitForLoadState(tabId, {
			...msg,
			state: waitUntil === "load" ? "complete" : waitUntil,
			timeoutMs,
		});
...
async function waitForNavigation(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	const record = registerWait(tabId, "navigation", {
		waitId: wait_id,
		wait_id,
		requestId,
		request_id: requestId,
	...
	const currentNavigationState = async (source: string): Promise<NavigationState> => {
		...
		if (sameDocument === true && (targetUrl || urlContains))
			return { ok: true, url, tab, metrics, stage: "same-document" };
		if (waitUntil === "commit" || waitUntil === "committed")
			return { ok: true, url, tab, metrics, stage: "commit" };
		if (waitUntil === "domcontentloaded" && loadStateSatisfied("domcontentloaded", tab, metrics))
			return { ok: true, url, tab, metrics, stage: "domcontentloaded" };
		if ((waitUntil === "load" || waitUntil === "complete") && loadStateSatisfied("complete", tab, metrics))
			return { ok: true, url, tab, metrics, stage: "complete" };
	...
	if (timeoutMs === 0) {
		...
		return finishBrowserPilotWait(
			...
			"wait.navigation immediate check failed",
			{
				wait_id,
				request_id: requestId,
				timeout_ms: 0,
	...
	const isMainWebNavigation = (details: NavigationDetails): boolean =>
		Number(details?.tabId) === Number(tabId) && Number(details?.frameId || 0) === 0;
	const cdp = browserPilotPersistentCdp();
	if (cdp?.send) {
		try {
			const probe = normalizePersistentBrowserPilotResponse(
				await cdp.send(
					tabId,
					"Runtime.evaluate",
					{ expression: "location.href", returnByValue: true },
					{ persistent: true, name: "wait_navigation_probe", timeoutMs: Math.min(timeoutMs, 1000) },
				),
			);
	...
	return await new Promise<BrowserPilotBridgeResponse>((resolve) => {
		...
		const terminalData = (source: string, extra: JsonRecord = {}): JsonRecord => ({
			wait_id,
			request_id: requestId,
			targetUrl,
			urlContains,
			...
			if (ok) resolve(finishBrowserPilotWait(record, true, terminalData(source, extra)));
				...
				resolve(
					finishBrowserPilotWait(
						...
						false,
						null,
						code || BROWSER_PILOT_ERROR_CODES.NAVIGATION_TIMEOUT,
						message || "waitForNavigation timed out",
						terminalData(source, extra),
					),
				);
		};
		const completeIfReady = (source: string, url: unknown, stage: string, extra: JsonRecord = {}): boolean => {
			...
			if (stage === "error") {
				finish(
					false,
					source,
					{ ...(extra || {}), url: lastUrl, stage },
					BROWSER_PILOT_ERROR_CODES.NAVIGATION_TIMEOUT,
					"waitForNavigation failed",
				);
				return true;
			}
			if (waitUntil === "commit" || waitUntil === "committed") {
			...
			if (
				...
				(stage === "domcontentloaded" || stage === "load" || stage === "complete")
			) {
				finish(true, source, { ...(extra || {}), url: lastUrl, stage });
				return true;
			}
		...
		const checkCurrent = async (source: string): Promise<void> => {
			...
			if (
				...
				(metrics?.readyState === "interactive" ||
					metrics?.readyState === "complete" ||
					tab?.status === "complete")
			)
				return finish(true, source || "currentUrl", {
					url,
			...
			if (
				(waitUntil === "load" || waitUntil === "complete") &&
				(metrics?.readyState === "complete" || tab?.status === "complete")
			)
				return finish(true, source || "currentUrl", {
					url,
					stage: "complete",
		...
		const failIfAbort = () => {
			if (record.abortController?.signal?.aborted)
				finish(false, "abort", {}, BROWSER_PILOT_ERROR_CODES.CANCELLED, "waitForNavigation cancelled");
		};
		try {
		...
		const onTabsUpdated = (changedTabId: number, changeInfo: JsonRecord, updatedTab: BrowserPilotChromeTab) => {
			...
			if (changeInfo?.status === "complete" || updatedTab?.status === "complete")
				completeIfReady("chrome.tabs.onUpdated", url, "complete", { changeInfo, title: updatedTab?.title });
		};
		chrome.tabs.onUpdated.addListener(onTabsUpdated);
		record.listeners.push({ remove: () => chrome.tabs.onUpdated.removeListener(onTabsUpdated) });
		if (chrome.webNavigation) {
			const onBeforeNavigate = (details: NavigationDetails) => {
				if (!isMainWebNavigation(details)) return;
				recordWaitEvent(record, {
					method: "chrome.webNavigation.onBeforeNavigate",
					url: details.url,
					frameId: details.frameId,
				});
			};
			const onCommitted = (details: NavigationDetails) => {
				if (!isMainWebNavigation(details)) return;
				recordWaitEvent(record, {
					method: "chrome.webNavigation.onCommitted",
					url: details.url,
					transitionType: details.transitionType,
				});
				completeIfReady("chrome.webNavigation.onCommitted", details.url, "commit", {
					transitionType: details.transitionType,
			...
			const onErrorOccurred = (details: NavigationDetails) => {
				if (!isMainWebNavigation(details)) return;
				recordWaitEvent(record, {
					method: "chrome.webNavigation.onErrorOccurred",
					url: details.url,
					error: details.error,
				});
				if (urlMatches(details.url))
					completeIfReady("chrome.webNavigation.onErrorOccurred", details.url, "error", {
						error: details.error,
					});
			};
			const onSameDocument = (details: NavigationDetails) => {
				if (!isMainWebNavigation(details)) return;
				recordWaitEvent(record, { method: "chrome.webNavigation.sameDocument", url: details.url });
				if (sameDocument && urlMatches(details.url))
					finish(true, "chrome.webNavigation.sameDocument", {
						url: details.url,
						sameDocument: true,
						transitionType: details.transitionType,
					});
			...
			record.listeners.push({
				remove: () => {
					try {
						...
						/* best-effort webNavigation listener cleanup */
					}
					try {
		...
		enableBrowserPilotCdpDomains(record, ["Page"])
			.then(() => {
				const sub = subscribeBrowserPilotCdp(
					tabId,
					[
						"Page.frameNavigated",
						"Page.navigatedWithinDocument",
					...
					(_source, method, params) => {
						...
						const isMainFrame =
							method === "Page.frameNavigated"
								? !frame.parentId
								: !mainFrameId || !frameId || String(frameId) === String(mainFrameId);
						...
						if (method === "Page.frameNavigated") {
							...
							if (sameDocument && urlMatches(lastUrl))
								finish(true, "Page.navigatedWithinDocument", {
									url: lastUrl,
									frameId,
									sameDocument: true,
								});
							...
							if (params?.name === "DOMContentLoaded")
								completeIfReady("Page.lifecycleEvent", lastUrl, "domcontentloaded", {
									frameId,
									name: params.name,
								});
							if (params?.name === "load")
								completeIfReady("Page.lifecycleEvent", lastUrl, "load", { frameId, name: params.name });
...
function loadStateSatisfied(
	state: string,
	tab: BrowserPilotChromeTab | null | undefined,
	metrics: LoadMetrics | null | undefined,
): boolean {
	const rs = metrics?.readyState || "";
	if (state === "domcontentloaded")
		return rs === "interactive" || rs === "complete" || metrics?.domContentLoaded === true;
	if (state === "load" || state === "complete")
		return tab?.status === "complete" || rs === "complete" || metrics?.load === true;
	return false;
}
...
async function waitForLoadState(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	if (!["domcontentloaded", "load", "complete"].includes(targetState))
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "wait.loadState unsupported state", {
			state: targetState,
		});
	const timeoutMs = normalizeBrowserPilotTimeoutMs(msg);
	const record = registerWait(tabId, "load_state", {
		state: targetState,
		timeout_ms: timeoutMs,
		waitId: msg.waitId,
		wait_id: msg.wait_id,
	...
	return await new Promise<BrowserPilotBridgeResponse>((resolve) => {
		...
		const failIfAbort = () => {
			if (record.abortController?.signal?.aborted)
				complete(
					finishBrowserPilotWait(record, false, null, "CANCELLED", waitAbortMessage(record), { targetState }),
				);
		};
		try {
			record.abortController.signal.addEventListener("abort", failIfAbort, { once: true });
		...
		record.timers.push(timeoutHandle);
		void enableBrowserPilotCdpDomains(record, ["Page"])
			.then(() => {
				const sub = subscribeBrowserPilotCdp(
					tabId,
					[
					...
					(_source, method, params) => {
						...
						if (
							(targetState === "domcontentloaded" &&
								(method === "Page.domContentEventFired" || params.name === "DOMContentLoaded")) ||
							((targetState === "load" || targetState === "complete") &&
								(method === "Page.loadEventFired" ||
									params.name === "load" ||
									method === "Page.frameStoppedLoading"))
						) {
							complete(
								finishBrowserPilotWait(record, true, { state: targetState, method, params, cdp: true }),
							);
						}
					},
					record,
		...
		const onUpdated = (changedTabId: number, changeInfo: JsonRecord, updatedTab: BrowserPilotChromeTab) => {
			...
			if (
				(targetState === "complete" || targetState === "load") &&
				(changeInfo.status === "complete" || updatedTab?.status === "complete")
			)
				complete(
		...
		void (async () => {
			...
			if (loadStateSatisfied(targetState, recheckTab, recheckMetrics)) {
				complete(
					finishBrowserPilotWait(record, true, {
						state: targetState,
						url: recheckMetrics?.url || recheckTab?.url,
						title: recheckMetrics?.title || recheckTab?.title,
						recheck: true,
						readyState: recheckMetrics?.readyState,
					}),
				);
			}
		})();
