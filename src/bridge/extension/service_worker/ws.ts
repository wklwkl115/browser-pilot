...
function requireOpenSession(tabId: number, msg: BrowserPilotBridgeCommand) {
	...
	if (!session) return { error: browserPilotError("WEBSOCKET_SESSION_NOT_FOUND", "ws session not found", { cmd: msg.cmd, tabId, sessionId: wsSessionId(msg) }) };
	if (session.state !== "open") return { error: browserPilotError("WEBSOCKET_SESSION_NOT_OPEN", "ws session is not open", { cmd: msg.cmd, tabId, sessionId: session.sessionId, state: session.state, url: session.url }) };
	return { session };
}
...
async function openWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	if (!config.url) return browserPilotError("WEBSOCKET_INVALID_INPUT", "ws.open requires explicit url", { cmd: msg.cmd, tabId, sessionId: config.sessionId, field: "url" });
	const existing = getWsSession(tabId, config.sessionId);
	if (existing && (existing.state === "opening" || existing.state === "open")) return browserPilotError("WEBSOCKET_SESSION_ALREADY_OPEN", "ws session already open", { cmd: msg.cmd, tabId, sessionId: config.sessionId, url: existing.url, state: existing.state });
	if (existing) {
		try {
...
async function sendWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	try {
		...
		return browserPilotError("WEBSOCKET_SEND_FAILED", `ws send failed: ${errorText(error)}`, { cmd: msg.cmd, tabId, sessionId: session.sessionId, url: session.url, error: errorText(error) });
	}
...
function waitWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	const session = requireSession(tabId, msg);
	if (!session) return Promise.resolve(browserPilotError("WEBSOCKET_SESSION_NOT_FOUND", "ws session not found", { cmd: msg.cmd, tabId, sessionId: wsSessionId(msg) }));
	...
	if (unsafe) return Promise.resolve(browserPilotError("WEBSOCKET_INVALID_MATCHER", "unsafe ws regex matcher", { cmd: msg.cmd, tabId, sessionId: session.sessionId, regex: regexText, reason: unsafe }));
	...
	if (session.state !== "open") return Promise.resolve(browserPilotError("WEBSOCKET_WAIT_ABORTED", "ws wait aborted because session is not open", { cmd: msg.cmd, tabId, sessionId: session.sessionId, state: session.state, afterSeq, contains, regex: regexText }));
	return new Promise((resolve) => {
		...
		const timer = setTimeout(() => {
			...
			settled = true;
			cleanup();
			resolve(browserPilotError("WEBSOCKET_WAIT_TIMEOUT", `ws wait timed out after ${timeoutMs}ms`, { cmd: msg.cmd, tabId, sessionId: session.sessionId, timeoutMs, afterSeq, contains, regex: regexText }));
		}, timeoutMs);
		const cleanup = () => {
			clearTimeout(timer);
		...
		const onClose = () => {
			...
			settled = true;
			cleanup();
			resolve(browserPilotError("WEBSOCKET_WAIT_ABORTED", "ws wait aborted because session closed", { cmd: msg.cmd, tabId, sessionId: session.sessionId, state: session.state, afterSeq, contains, regex: regexText }));
		};
		const onError = () => {
			if (settled) return;
			settled = true;
...
async function replayWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	const session = requireSession(tabId, msg);
	if (!session) return browserPilotError("WEBSOCKET_SESSION_NOT_FOUND", "ws session not found", { cmd: msg.cmd, tabId, sessionId: wsSessionId(msg) });
	const rawSteps = Array.isArray(msg.steps) ? msg.steps : [];
	if (!rawSteps.length) return browserPilotError("WEBSOCKET_INVALID_INPUT", "ws.replay requires explicit steps", { cmd: msg.cmd, tabId, sessionId: session.sessionId, field: "steps" });
...
async function handleBrowserPilotWsCommand(cmd: string, tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
