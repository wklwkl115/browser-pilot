...
function normalizePayload(data: unknown): NormalizedWsPayload {
	...
	if (typeof data === "string") {
		text = data;
		if (text.length > BROWSER_PILOT_WS_MAX_INBOUND_FRAME_BYTES)
			return { oversized: true, bytes: text.length, binary };
...
function requireSession(tabId: number, msg: BrowserPilotBridgeCommand) {
	return getWsSession(tabId, wsSessionId(msg));
}

function requireOpenSession(tabId: number, msg: BrowserPilotBridgeCommand) {
	const session = requireSession(tabId, msg);
	if (!session)
		return {
			error: browserPilotError("WEBSOCKET_SESSION_NOT_FOUND", "ws session not found", {
				cmd: msg.cmd,
				tabId,
	...
	if (session.state !== "open")
		return {
			error: browserPilotError("WEBSOCKET_SESSION_NOT_OPEN", "ws session is not open", {
				cmd: msg.cmd,
				tabId,
...
async function openWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	const config = normalizeWsOpenConfig(msg);
	if (!config.url)
		return browserPilotError("WEBSOCKET_INVALID_INPUT", "ws.open requires explicit url", {
			cmd: msg.cmd,
			tabId,
	...
	if (existing && (existing.state === "opening" || existing.state === "open"))
		return browserPilotError("WEBSOCKET_SESSION_ALREADY_OPEN", "ws session already open", {
			cmd: msg.cmd,
			tabId,
			sessionId: config.sessionId,
			url: existing.url,
	...
	if (existing) {
		...
		browserPilotWsSessions.delete(String(existing.key || ""));
	}
	...
	const messageListener = (event: MessageEvent) => {
		...
		if (payload.oversized) {
			...
			session.state = "error";
			rememberWsTranscript(session, {
				event: "error",
				direction: "inbound",
				error: session.lastError,
				bytes: payload.bytes,
				maxBytes: BROWSER_PILOT_WS_MAX_INBOUND_FRAME_BYTES,
			});
	...
	return await new Promise((resolve) => {
		...
		const timer = setTimeout(() => {
			...
			resolve(
				browserPilotError("WEBSOCKET_OPEN_TIMEOUT", `ws open timed out after ${config.timeoutMs}ms`, {
					cmd: msg.cmd,
					tabId,
					sessionId: config.sessionId,
					url: config.url,
		...
		const onOpenError = () => {
			...
			browserPilotWsSessions.delete(String(session.key));
			resolve(
				browserPilotError("WEBSOCKET_OPEN_FAILED", "ws open failed", {
					cmd: msg.cmd,
					tabId,
					sessionId: config.sessionId,
					url: config.url,
...
async function sendWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	try {
		...
		return browserPilotError("WEBSOCKET_SEND_FAILED", `ws send failed: ${errorText(error)}`, {
			cmd: msg.cmd,
			tabId,
			sessionId: session.sessionId,
			url: session.url,
	...
	const entry = rememberWsTranscript(session, {
		event: "send",
		direction: "outbound",
		text,
	...
	return {
		ok: true,
		data: { ...wsSessionSummary(session), sent: { seq: entry?.seq, bytes: entry?.bytes, preview: entry?.preview } },
	};
}

function waitWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	const session = requireSession(tabId, msg);
	if (!session)
		return Promise.resolve(
			browserPilotError("WEBSOCKET_SESSION_NOT_FOUND", "ws session not found", {
				cmd: msg.cmd,
				tabId,
				sessionId: wsSessionId(msg),
			}),
	...
	if (unsafe)
		return Promise.resolve(
			browserPilotError("WEBSOCKET_INVALID_MATCHER", "unsafe ws regex matcher", {
				cmd: msg.cmd,
				tabId,
				sessionId: session.sessionId,
				regex: regexText,
	...
	const immediate = session.transcript.find(
		(entry) =>
			entry.event === "message" &&
			entry.direction === "inbound" &&
			Number(entry.seq) > afterSeq &&
			(!contains || String(entry.text || "").includes(contains)) &&
			(!regex || regex.test(String(entry.text || ""))),
	);
	if (immediate)
		return Promise.resolve({
			ok: true,
			data: {
				session: wsSessionSummary(session),
	...
	if (session.state !== "open")
		return Promise.resolve(
			browserPilotError("WEBSOCKET_WAIT_ABORTED", "ws wait aborted because session is not open", {
				cmd: msg.cmd,
				tabId,
				sessionId: session.sessionId,
				state: session.state,
	...
	return new Promise((resolve) => {
		...
		const timer = setTimeout(() => {
			...
			cleanup();
			resolve(
				browserPilotError("WEBSOCKET_WAIT_TIMEOUT", `ws wait timed out after ${timeoutMs}ms`, {
					cmd: msg.cmd,
					tabId,
					sessionId: session.sessionId,
					timeoutMs,
		...
		const onMessage = (event: MessageEvent) => {
			if (session.state !== "open") {
				...
				cleanup();
				resolve(
					browserPilotError("WEBSOCKET_WAIT_ABORTED", "ws wait aborted because session errored", {
						cmd: msg.cmd,
						tabId,
						sessionId: session.sessionId,
						state: session.state,
			...
			const matched =
				...
				entry.direction === "inbound" &&
				Number(entry.seq) > afterSeq &&
				(!contains || payload.text.includes(contains)) &&
				(!regex || regex.test(payload.text));
		...
		const onClose = () => {
			...
			cleanup();
			resolve(
				browserPilotError("WEBSOCKET_WAIT_ABORTED", "ws wait aborted because session closed", {
					cmd: msg.cmd,
					tabId,
					sessionId: session.sessionId,
					state: session.state,
...
async function replayWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	const session = requireSession(tabId, msg);
	if (!session)
		return browserPilotError("WEBSOCKET_SESSION_NOT_FOUND", "ws session not found", {
			cmd: msg.cmd,
			tabId,
			sessionId: wsSessionId(msg),
		});
	...
	if (!rawSteps.length)
		return browserPilotError("WEBSOCKET_INVALID_INPUT", "ws.replay requires explicit steps", {
			cmd: msg.cmd,
			tabId,
			sessionId: session.sessionId,
			field: "steps",
	...
	for (let index = 0; index < steps.length; index += 1) {
		...
		const sendResult = await sendWs(tabId, {
			...msg,
			cmd: "ws.send",
			text: String(step.text ?? step.message ?? ""),
		});
		if (!sendResult.ok) {
			return {
				ok: false,
		...
		if (typeof step.contains === "string" || typeof step.regex === "string") {
			const timeoutMs =
				typeof step.timeoutMs === "number"
					...
					: typeof step.timeout_ms === "number"
						? step.timeout_ms
						: undefined;
			const waitResult = await waitWs(tabId, {
				...msg,
				cmd: "ws.wait",
				afterSeq: asRecord(sent).seq,
			...
			stepResult.matched = {
				matcher: asRecord(waitResult.data).matcher,
				entry: asRecord(waitResult.data).entry,
				waitedMs: asRecord(waitResult.data).waitedMs,
				matchedImmediately: asRecord(waitResult.data).matchedImmediately,
			};
		}
...
function collectWs(tabId: number, msg: BrowserPilotBridgeCommand): BrowserPilotBridgeResponse {
	const session = requireSession(tabId, msg);
	if (!session)
		return browserPilotError("WEBSOCKET_SESSION_NOT_FOUND", "ws session not found", {
			cmd: msg.cmd,
...
async function closeWs(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	const session = requireSession(tabId, msg);
	if (!session)
		return browserPilotError("WEBSOCKET_SESSION_NOT_FOUND", "ws session not found", {
			cmd: msg.cmd,
	...
	if (session.state === "closed" || session.state === "error")
		return { ok: true, data: { session: wsSessionSummary(session) } };
...
async function handleBrowserPilotWsCommand(
	cmd: string,
	tabId: number,
	...
	if (cmd === "ws.status") {
		...
		return {
			ok: true,
			data: { session: wsSessionSummary(session, sessionId), stateLost: !!lost, lostSession: lost },
		};
	}
	...
	return browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, `Unknown ws command: ${cmd}`, { cmd, tabId });
}

function cleanupWsSessionsForTab(tabId: number, reason = "tab_cleanup") {
	const cleanupState =
		typeof cleanupWsSessionsForTabState === "function"
	...
	const result = await recoverState("ws", {
		...
		recover: async (_record) => {
			...
			return {
				recovered: false,
				historyLost: true,
				reason: "WebSocket sessions are not auto-recovered after restart",
			};
		},
	});
	results.push(result);
