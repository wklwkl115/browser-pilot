...
import type {
	...
	BrowserPilotChromeMessageSender,
} from "./types";
import { recordBrowserPilotPrerenderActivation } from "./tab_sync";

// Protocol validation and command dispatch for Browser Pilot Bridge messages.

let browserPilotBridgeRouterInstalled = false;
const BROWSER_PILOT_BRIDGE_MAX_RESPONSE_BYTES = 32 * 1024 * 1024;

function handleBrowserPilotRuntimeEventMessage(
	msgType: string,
	sender: BrowserPilotChromeMessageSender,
...
async function handleBrowserPilotBridgeMessage(
	msg: BrowserPilotBridgeCommand,
	sender: BrowserPilotChromeMessageSender,
) {
	const validation = validateBrowserPilotBridgeProtocolMessage(msg);
	if (!validation.ok)
		return bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, validation.error, validation.details);
...
function installBrowserPilotBridgeRouter() {
	...
	chrome.runtime.onMessage.addListener(
		(msg: unknown, sender: BrowserPilotChromeMessageSender, sendResponse: (response: unknown) => void) => {
			// Pass-through guard: let transport.ts handle offscreen-prefixed messages.
			if (
				msg &&
				typeof msg === "object" &&
				typeof (msg as JsonRecord).type === "string" &&
				String((msg as JsonRecord).type).startsWith("browser-pilot-offscreen-")
			)
				return false;
...
function sendBrowserPilotBridgeWsCommandResult(
	socket: BrowserPilotBridgeWebSocketLike,
	id: string | number,
	msg: BrowserPilotBridgeCommand,
	res: BrowserPilotBridgeResponse,
) {
	...
	const responseBytes =
		payload.length > BROWSER_PILOT_BRIDGE_MAX_RESPONSE_BYTES
			? payload.length
	...
	socket.send(
		JSON.stringify({
			...
			error: {
				...
				message:
					"browser response exceeds the bridge response budget; narrow the command filters or lower its maxBytes/limit",
				details: {
					cmd: msg.cmd,
					responseBytesAtLeast: responseBytes,
					maxBytes: BROWSER_PILOT_BRIDGE_MAX_RESPONSE_BYTES,
				},
			},
		}),
	);
...
function sendBrowserPilotBridgeWsInputError(
	socket: BrowserPilotBridgeWebSocketLike,
	id: string | number,
	error: string,
	details: JsonRecord = {},
	...
	socket.send(
		JSON.stringify({
			...
			id,
			error,
			details: { ...(details || {}), dispatchStarted: false, acked: false },
		}),
	);
}
...
function isRoutedBrowserPilotBridgeWsEnvelope(
	data: BrowserPilotBridgeWsEnvelope,
): data is RoutedBrowserPilotBridgeWsEnvelope {
...
function parseBrowserPilotBridgeWsCode(rawCode: unknown): unknown {
	...
	try {
		...
		if (
			...
			typeof (parsed as JsonRecord).cmd === "string"
		)
			return parsed;
	} catch (_error) {
...
async function dispatchValidatedBrowserPilotBridgeWsCommand(
	data: RoutedBrowserPilotBridgeWsEnvelope,
	msg: BrowserPilotBridgeCommand,
	socket: BrowserPilotBridgeWebSocketLike,
): Promise<void> {
...
async function handleBrowserPilotBridgeWsCommand(
	data: RoutedBrowserPilotBridgeWsEnvelope,
	code: object,
	socket: BrowserPilotBridgeWebSocketLike,
): Promise<void> {
	...
	if (typeof codeObj.cmd !== "string" || !codeObj.cmd.trim()) {
		sendBrowserPilotBridgeWsInputError(socket, data.id, 'Message object must contain a non-empty "cmd" field', {
			codeType: "object",
		});
		return;
	}
	...
	if (!validation.ok) {
		sendBrowserPilotBridgeWsCommandResult(
			socket,
			data.id,
			candidate,
			bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, validation.error, {
				...(validation.details || {}),
...
async function handleBrowserPilotBridgeWsMessage(
	data: BrowserPilotBridgeWsEnvelope,
	socket: BrowserPilotBridgeWebSocketLike,
) {
	...
	sendBrowserPilotBridgeWsInputError(socket, data.id, `Unsupported message code type: ${typeof code}`, {
		codeType: typeof code,
	});
}
export {
	BROWSER_PILOT_BRIDGE_MAX_RESPONSE_BYTES,
	installBrowserPilotBridgeRouter,
	handleBrowserPilotBridgeMessage,
	sendBrowserPilotBridgeWsCommandResult,
	sendBrowserPilotBridgeWsInputError,
	handleBrowserPilotBridgeWsMessage,
};
