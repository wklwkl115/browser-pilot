...
async function waitForNetworkIdle(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
  ...
  if (timeoutMs === 0) return finishBrowserPilotWait(record, true, { state:'network_idle', idle_ms:0, inflight:0, immediate:true });
  ...
  return await new Promise<BrowserPilotBridgeResponse>(resolve => {
    const complete = (res: BrowserPilotBridgeResponse) => { try { if (idleTimer) clearTimeout(idleTimer); } catch (_) { /* best-effort idle timer cleanup */ } resolve(res); };
    const failIfAbort = () => { if (record.abortController?.signal?.aborted) complete(finishBrowserPilotWait(record, false, null, 'CANCELLED', waitAbortMessage(record), { inflight:Array.from(inflight.values()) })); };
    ...
    const armIdle = () => {
      ...
      if (inflight.size <= maxInflight) idleTimer = setTimeout(() => complete(finishBrowserPilotWait(record, true, { state: 'network_idle', idle_ms: idleMs, inflight: inflight.size, ignored: ignored.slice(-100), events: record.cdpEvents.slice(-50) })), idleMs);
    };
    const timeoutHandle = setTimeout(() => complete(finishBrowserPilotWait(record, false, null, BROWSER_PILOT_ERROR_CODES.TIMEOUT, 'wait.networkIdle timed out', { timeout_ms: timeoutMs, idle_ms: idleMs, inflight: Array.from(inflight.values()), ignored: ignored.slice(-100), events: record.cdpEvents.slice(-50), last_error: record.lastError })), timeoutMs);
    ...
    subscribeBrowserPilotCdp(tabId, ['Network.requestWillBeSent','Network.responseReceived','Network.requestServedFromCache','Network.loadingFinished','Network.loadingFailed','Network.webSocketCreated','Network.eventSourceMessageReceived'], onEvent, record);
    armIdle();
  });
}
