...
type NetworkIdleRequest = JsonRecord & {
	requestId: string;
	url: string;
	type: string;
	method: string;
...
function stringList(value: unknown): string[] {
	return Array.isArray(value) ? value.map((x: unknown) => String(x)) : [];
}

// Browser Pilot network-idle wait helpers.
// Loaded before wait.js by background.js.

function compileNetworkIdleFilter(msg: BrowserPilotBridgeCommand): (req: NetworkIdleRequest) => NetworkIdleDecision {
	const ignoreUrl = stringList(
		Array.isArray(msg.ignoreUrls) ? msg.ignoreUrls : Array.isArray(msg.ignore_urls) ? msg.ignore_urls : [],
	);
	const includeUrl = stringList(
		Array.isArray(msg.includeUrls) ? msg.includeUrls : Array.isArray(msg.include_urls) ? msg.include_urls : [],
	);
	...
	const ignoreResourceTypes = new Set(
		stringList(
	...
	return function shouldTrack(req: NetworkIdleRequest): NetworkIdleDecision {
		...
		if (resourceTypes.size && !resourceTypes.has(type))
			return { track: false, reason: "resource_type_not_included" };
		if (ignoreResourceTypes.has(type)) return { track: false, reason: "ignored_resource_type" };
		if (!includePreflight && (type === "preflight" || method === "OPTIONS"))
			return { track: false, reason: "preflight" };
		if (!includeBeacon && (type === "ping" || req.initiatorType === "beacon"))
			return { track: false, reason: "beacon" };
		if (ignoreMethods.has(method)) return { track: false, reason: "ignored_method" };
		return { track: true, reason: "tracked", ignoreLongPollingMs };
	};
}
async function waitForNetworkIdle(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	const record = registerWait(tabId, "network_idle", {
		idle_ms: idleMs,
	...
	const maybeExpireLongPolling = () => {
		...
		for (const [id, item] of Array.from(inflight.entries())) {
			if (item.ignoreLongPollingMs && now - Number(item.startedAt || now) >= item.ignoreLongPollingMs) {
				inflight.delete(id);
				ignored.push({
					requestId: id,
					reason: "long_polling",
					url: item.url,
					age_ms: now - Number(item.startedAt || now),
				});
			}
		}
	};
	...
	return await new Promise<BrowserPilotBridgeResponse>((resolve) => {
		const complete = (res: BrowserPilotBridgeResponse) => {
			try {
				...
				/* best-effort idle timer cleanup */
			}
			resolve(res);
		};
		const failIfAbort = () => {
			if (record.abortController?.signal?.aborted)
				complete(
					finishBrowserPilotWait(record, false, null, "CANCELLED", waitAbortMessage(record), {
		...
		const onEvent = (source: { tabId?: number }, method: string, params: JsonRecord = {}) => {
			...
			if (method === "Network.requestWillBeSent") {
				...
				const req: NetworkIdleRequest = {
					requestId: String(params.requestId || makeWaitId(tabId, "request")),
				...
				if (decision.track)
					inflight.set(req.requestId, {
						...req,
						startedAt: Date.now(),
						ignoreLongPollingMs: decision.ignoreLongPollingMs,
					});
