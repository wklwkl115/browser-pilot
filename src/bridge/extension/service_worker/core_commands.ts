...
import {
	...
	runtimeErrorMessage as coreErrorMessage,
	runtimeRecord as coreRecord,
...
// Non-native bridge commands: tabs, extension reload, batch, CDP.

type BridgeWakeProbe = (resetDelay: boolean) => unknown;
type ValidatedBridgeCommand =
	{ ok: true; command: BrowserPilotBridgeCommand } | { ok: false; error?: string; details?: JsonRecord };
type BrowserPilotCoreDispatchContext = { resolveParams?: (params: unknown) => JsonRecord };
type BrowserPilotCoreCommandHandler = (
	msg: BrowserPilotBridgeCommand,
	sender: BrowserPilotBridgeSender,
	context: BrowserPilotCoreDispatchContext,
) => Promise<BrowserPilotBridgeResponse>;
let bridgeWakeProbe: BridgeWakeProbe | null = null;

function coreErrorDetails(error: unknown): JsonRecord {
...
async function probeTabCapabilities(
	tabId: number,
	tab: { url?: string; status?: string },
	timeoutMs: number,
): Promise<BrowserPilotBridgeResponse> {
	if (!isScriptable(tab.url))
		return bridgeError(BROWSER_PILOT_ERROR_CODES.UNSUPPORTED_TARGET, "Target tab is not scriptable", {
			tabId,
			url: tab.url,
			scriptable: false,
		});
	try {
		...
		return bridgeError(BROWSER_PILOT_ERROR_CODES.UNSUPPORTED_TARGET, "Target tab failed the scriptability probe", {
			tabId,
			url: tab.url,
			scriptable: false,
			error: coreErrorDetails(error),
		});
	}
	...
	if (!cdp?.send)
		return bridgeError(
			...
			"Target tab has no persistent CDP capability",
			{ tabId, url: tab.url, scriptable: true, cdpAvailable: false },
		);
	const response = normalizePersistentBrowserPilotResponse(
		await cdp.send(
			...
			{ name: "operation-tab-readiness", persistent: true, timeoutMs },
		),
	);
	if (response?.ok === false)
		return bridgeError(BROWSER_PILOT_ERROR_CODES.UNSUPPORTED_TARGET, "Target tab CDP capability probe failed", {
			tabId,
			url: tab.url,
			scriptable: true,
			cdpAvailable: false,
...
async function waitForCreatedTabReady(tabId: number, timeoutMs: number): Promise<BrowserPilotBridgeResponse> {
	...
	if (tab.status !== "complete")
		return bridgeError(
			...
			"Created tab did not reach ready state before the command deadline",
			{ tabId, url: tab.url, status: tab.status, timeoutMs },
		);
		...
		func: () => {
			...
			if (!state) {
				state = {
					seq: 1,
					at: Date.now(),
					epoch: globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random()}`,
				};
				holder[key] = state;
			}
...
async function handleBridgeWake(
	msg: BrowserPilotBridgeCommand,
	sender: BrowserPilotBridgeSender,
): Promise<BrowserPilotBridgeResponse> {
...
function normalizeBrowserPilotCreateTabUrl(
	value: unknown,
): { ok: true; url: string } | { ok: false; error: string; details: JsonRecord } {
	...
	if (protocol === "javascript:" || protocol === "data:")
		return {
			ok: false,
			error: "tabs.create does not accept javascript: or data: URLs; use browser_execute for JavaScript in an existing tab",
...
async function handleTabsCommand(msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	try {
		if (!msg.method || msg.method === "list") {
			...
			const data = await Promise.all(
				tabs.map(async (t) => ({
					...
					...(await browserPilotTabIdentityFields(t)),
				})),
			);
			return { ok: true, data };
		}
		if (msg.method === "switch") {
			...
			const capability = await probeTabCapabilities(
				tabId,
				tab,
				Math.max(100, Number(msg.timeoutMs ?? msg.timeout_ms ?? 5000)),
			);
			return capability.ok === false
				? capability
				: { ok: true, data: { active: true, ...coreRecord(capability.data) } };
		}
		if (msg.method === "create") {
			const normalized = normalizeBrowserPilotCreateTabUrl(msg.url);
			if (!normalized.ok)
				return bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, normalized.error, {
					cmd: msg.cmd,
					method: msg.method,
					...normalized.details,
				});
			if (msg.incognito) {
				...
				const allowed = await new Promise<boolean>((resolve) => {
					try {
						if (chrome.extension && typeof chrome.extension.isAllowedIncognitoAccess === "function")
							chrome.extension.isAllowedIncognitoAccess((a: boolean) => resolve(!!a));
				...
				if (!allowed)
					return bridgeError(
						BROWSER_PILOT_ERROR_CODES.UNSUPPORTED_TARGET,
						"Incognito access is not granted to the Browser Pilot Bridge extension",
						{
							cmd: msg.cmd,
							method: msg.method,
							recovery:
								'Open chrome://extensions, find "Browser Pilot Bridge" -> Details -> enable "Allow in incognito", then retry browser_tabs create with incognito:true',
						},
					);
				const win = await chrome.windows.create({
					url: normalized.url,
					incognito: true,
					focused: msg.active !== false,
				});
				const incognitoTab = win && Array.isArray(win.tabs) ? win.tabs[0] : undefined;
				if (!incognitoTab || incognitoTab.id === undefined)
					return bridgeError(
						BROWSER_PILOT_ERROR_CODES.UNSUPPORTED_TARGET,
						"Incognito window was created but no tab was returned",
						{ cmd: msg.cmd, method: msg.method },
					);
				const ready = await waitForCreatedTabReady(
					incognitoTab.id,
					Math.max(100, Number(msg.timeoutMs ?? msg.timeout_ms ?? 5000)),
				);
				...
				return {
					...
					data: {
						...
						incognito: true,
						...(await browserPilotTabIdentityFields(incognitoTab)),
						...coreRecord(ready.data),
					},
				};
			}
			...
			if (tab.id === undefined)
				return bridgeError(
					BROWSER_PILOT_ERROR_CODES.UNSUPPORTED_TARGET,
					"Created tab did not return a tab id",
					{ cmd: msg.cmd, method: msg.method },
				);
			const ready = await waitForCreatedTabReady(
				tab.id,
		...
		if (msg.method === "close") {
			...
			if (!Number.isInteger(targetTabId) || targetTabId <= 0) {
				return bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "tabs.close requires a valid targetTabId", {
					cmd: msg.cmd,
					method: msg.method,
					targetTabId: rawTarget,
				});
			}
		...
		return bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "Unknown tabs method: " + String(msg.method), {
			cmd: msg.cmd,
			method: msg.method,
		});
	} catch (e) {
		return bridgeError(BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR, coreErrorMessage(e), {
			cmd: msg.cmd,
			method: msg.method,
		});
	}
...
async function handleManagementCommand(msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	try {
		...
		return bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "Unknown management method: " + String(msg.method), {
			cmd: msg.cmd,
			method: msg.method,
		});
	} catch (e) {
...
async function handleContentFingerprintCommand(
	msg: BrowserPilotBridgeCommand,
	sender: BrowserPilotBridgeSender,
): Promise<BrowserPilotBridgeResponse> {
	const tabId = Number(msg.tabId || sender.tab?.id || 0);
	if (!tabId)
		return bridgeError(BROWSER_PILOT_ERROR_CODES.NO_SESSION, "content.fingerprint requires a tabId", {
			cmd: msg.cmd,
			tabId: msg.tabId,
		});
	let messageError: unknown;
	try {
		const response = coreRecord(
			await chrome.tabs.sendMessage(tabId, { cmd: "browserPilot.contentFingerprint" }, { frameId: 0 }),
		);
		if (response.ok !== false) {
			const data = coreRecord(response.data ?? response);
			const identity = browserPilotPageIdentityForTab(
				tabId,
				typeof data.url === "string" ? data.url : sender.tab?.url,
			);
			return {
				ok: true,
				data: {
	...
	try {
		...
		return bridgeError(BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR, "content fingerprint unavailable", {
			cmd: msg.cmd,
			tabId,
...
async function handleCDP(
	...
	if (!tabId)
		return bridgeError(
			BROWSER_PILOT_ERROR_CODES.NO_SESSION,
			context.resolveParams ? "no tabId for batch cdp command" : "no tabId",
			{ cmd: msg.cmd, method: msg.method },
		);
	const cdp = browserPilotPersistentCdp();
	if (!cdp?.send)
		return bridgeError(BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR, "persistent CDP helper is not loaded", {
			cmd: msg.cmd,
			method: msg.method,
			tabId,
		});
	...
	const resp = normalizePersistentBrowserPilotResponse(
		await cdp.send(tabId, String(msg.method || ""), params, {
			name: "default",
			persistent: false,
	...
	if (resp && resp.ok !== false)
		return { ok: true, data: data.result !== undefined ? data.result : resp.result || resp.data };
	return bridgeError(BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR, resp?.error || resp?.message || "CDP send failed", {
		cmd: msg.cmd,
		method: msg.method,
		tabId,
	});
...
async function handlePersistentCDP(
	msg: BrowserPilotBridgeCommand,
	sender: BrowserPilotBridgeSender,
): Promise<BrowserPilotBridgeResponse> {
...
function validateBrowserPilotBridgeProtocolMessage(msg: BrowserPilotBridgeCommand): ValidatedBridgeCommand {
	const protocol = BrowserPilotNativeProtocol as BrowserPilotNativeProtocolRuntime & {
		validateCommand?: (command: unknown, options?: JsonRecord) => ValidatedBridgeCommand;
	};
...
async function dispatchBrowserPilotBridgeCommand(
	msg: BrowserPilotBridgeCommand,
	sender: BrowserPilotBridgeSender,
	context: BrowserPilotCoreDispatchContext = {},
): Promise<BrowserPilotBridgeResponse> {
	...
	if (isBrowserPilotNativeCommand(optionalString(msg.cmd)))
		return (await handleBrowserPilotNativeCommand(msg, sender)) as BrowserPilotBridgeResponse;
...
async function handleBatch(
	...
	const resolve$N = (params: unknown): JsonRecord =>
		JSON.parse(
			JSON.stringify(params || {}).replace(/"\$(\d+)\.([^"]+)"/g, (_: string, i: string, path: string) => {
				let v: unknown = R[Number(i)];
		...
		for (const c of commands) {
			try {
				if (!c || typeof c !== "object") {
					R.push(
						bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "invalid batch command", {
							cmd: msg.cmd,
							raw: c,
						}),
					);
				...
				if (handler) {
					R.push(
						normalizeBridgeResponse(
							await dispatchBrowserPilotBridgeCommand(c, sender, { resolveParams: resolve$N }),
							c.cmd,
						),
					);
					...
					if (!validation.ok)
						R.push(
							bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, validation.error, validation.details),
						);
					else
						R.push(
							normalizeBridgeResponse(
								await dispatchBrowserPilotBridgeCommand(validation.command, sender),
								c.cmd,
							),
						);
				} else {
					R.push(
						bridgeError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "unknown cmd: " + String(c.cmd), {
							cmd: c.cmd,
							raw: c,
						}),
					);
				...
				R.push(
					bridgeError(BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR, coreErrorMessage(e), {
						cmd: c && c.cmd,
						method: c && c.method,
						tabId: c && c.tabId,
...
export {
	setBridgeWakeProbe,
	handleBridgeWake,
	normalizeBrowserPilotCreateTabUrl,
	handleTabsCommand,
	handleManagementCommand,
	handleCDP,
	handlePersistentCDP,
	validateBrowserPilotBridgeProtocolMessage,
	resolveBrowserPilotCoreCommandHandler,
