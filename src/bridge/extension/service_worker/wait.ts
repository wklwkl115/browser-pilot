...
import {
	...
	callPageBrowserPilot,
	normalizePersistentBrowserPilotResponse,
	browserPilotError,
	browserPilotPersistentCdp,
	runtimeErrorMessage as waitErrorMessage,
	runtimeRecord as waitRecord,
} from "./runtimeSupport.js";
import { browserPilotSessions } from "./state_store.js";
import {
	diagnoseBrowserPilotCdpCleanupHistory,
	diagnoseBrowserPilotCdpDomainRefs,
	diagnoseBrowserPilotCdpSubscriptions,
} from "./wait_cdp";
import {
	cancelWaitsForTab,
	clearWait,
...
// Browser Pilot wait, event listener and CDP wait coordination commands.

async function waitForAny(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	const losers: number[] = [];
	const waits = (
		Array.isArray(msg.waits) ? msg.waits : Array.isArray(msg.conditions) ? msg.conditions : []
	) as WaitChild[];
	if (!waits.length)
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "wait.any requires waits/conditions", {});
	...
	const childWaitIds = waits.map((w, i) =>
		String((w && (w.waitId || w.wait_id)) || makeWaitId(tabId, "any_child_" + i)),
	);
	const children = waits.map((w: WaitChild, i: number) => ({
		index: i,
		wait: w,
	...
	const tasks = waits.map((w: WaitChild, i: number) =>
	...
	const parentTimeout = new Promise<never>((_, reject) => {
		parentTimer = setTimeout(() => {
			cleanup("parent_timeout");
			reject({
				error_code: BROWSER_PILOT_ERROR_CODES.TIMEOUT,
	...
	try {
		...
		const firstTimeout = errors.find(
			(x: JsonRecord) =>
				x &&
				(x.error_code === BROWSER_PILOT_ERROR_CODES.TIMEOUT ||
					/timeout|timed out|parent timeout/i.test(String(x.error || x.message || ""))),
		);
		if (firstTimeout)
			return browserPilotError(
				...
				firstTimeout.error ||
					...
					"wait.any timed out after " + parentTimeoutMs + "ms; no condition matched",
...
async function waitForAll(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	if (!waits.length)
		return browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "wait.all requires waits/conditions", {});
	const parentTimeoutMs = normalizeBrowserPilotTimeoutMs(msg);
	const failFast = msg.failFast !== false && msg.fail_fast !== false;
	const children = waits.map((w: WaitChild, i: number) => ({
		index: i,
		kind: w?.kind || w?.type || w?.cmd || "selector",
		timeout_ms: normalizeBrowserPilotTimeoutMs(w || {}, parentTimeoutMs),
	}));
	const controllers = waits.map(() => new AbortController());
	...
	const toResult = (settled: SettledWait, i: number): BrowserPilotBridgeResponse => {
		...
		return browserPilotError(
			reason.error_code || BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR,
	...
	const tasks = waits.map((w: WaitChild, i: number) =>
		dispatchBrowserPilotWait(
			tabId,
			{
				...
				abortController: controllers[i],
				timeoutMs: Math.min(
					normalizeBrowserPilotTimeoutMs(w || {}, parentTimeoutMs),
					Math.max(0, deadline - Date.now()),
				),
			},
			w.cmd || w.type || w.kind || "selector",
		).then(
			(result: BrowserPilotBridgeResponse) => {
				if (failFast && (!result || result.ok === false) && failureIndex < 0) {
					failureIndex = i;
					cleanup(BROWSER_PILOT_ERROR_CODES.CANCELLED, i);
				}
				return result;
			},
			(error: unknown) => {
	...
	if (failures.length) {
		...
		return browserPilotError(
			first.result?.error_code || BROWSER_PILOT_ERROR_CODES.TIMEOUT,
			"wait.all condition failed",
			{ failed_index: first.index, failure_index: first.index, failures, children, results, aggregate },
		);
	}
	return { ok: true, data: aggregate };
}
function normalizeBrowserPilotWaitKind(kind: unknown, msg?: BrowserPilotBridgeCommand): string {
	const raw = String(kind || msg?.kind || msg?.type || msg?.cmd || "selector")
		.trim()
		.replace(/^browserPilot[._-]/i, "");
	const withoutWaitPrefix = raw.replace(/^wait[._-]/i, "");
	return withoutWaitPrefix.replace(/[._-]/g, "").toLowerCase();
}
async function dispatchBrowserPilotWait(
	tabId: number,
	...
	if (k === "waitforloadstate" || k === "loadstate" || k === "load" || k === "domcontentloaded" || k === "complete") {
		...
		const st = String(msg.state ?? msg.loadState ?? msg.load_state ?? "")
			.replace(/[._-]/g, "")
			.toLowerCase();
	...
	if (k === "waitfornetworkidle" || k === "networkidle")
		return (await waitForNetworkIdle(tabId, msg)) as BrowserPilotBridgeResponse;
	if (k === "waitfornavigation" || k === "navigation")
		return (await waitForNavigation(tabId, msg)) as BrowserPilotBridgeResponse;
	if (k === "waitforselector" || k === "selector" || k === "css")
		return (await waitForSelector(tabId, msg)) as BrowserPilotBridgeResponse;
	if (k === "navigateandwait" || k === "navigate")
		return (await navigateAndWait(tabId, msg)) as BrowserPilotBridgeResponse;
	return browserPilotError(BROWSER_PILOT_ERROR_CODES.INVALID_RULE, "Unknown wait condition: " + kind, {
		kind,
		normalized: k,
		wait: msg,
	});
...
async function cancelWait(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	return {
		...
		data: {
			...
			cancelled_count,
			waitId: waitId || null,
			pending: Array.from(browserPilotWaits.values())
				.filter((r) => Number(r.tabId) === Number(tabId))
				.map((r) => ({ waitId: r.waitId, kind: r.kind, age_ms: Date.now() - r.createdAt })),
		},
	};
}
...
function extractBrowserPilotRuntimeValue(resp: BrowserPilotBridgeResponse): JsonRecord | null {
	...
	return (
		(waitRecord(waitRecord(data.result).result).value as JsonRecord) ||
		(waitRecord(result.result).value as JsonRecord) ||
		(waitRecord(data.result).value as JsonRecord) ||
		(result.value as JsonRecord) ||
		null
	);
}
async function cleanupBrowserPilotPageListenersForTab(
	tabId: number,
	reason?: string,
	timeoutMs?: number,
): Promise<BrowserPilotBridgeResponse> {
	...
	const resp = normalizePersistentBrowserPilotResponse(
		await cdp.send(
			...
			{ persistent: true, name: "event_listener_cleanup", timeoutMs: timeoutMs || 5000 },
		),
	);
...
async function addEventListener(tabId: number, msg: BrowserPilotBridgeCommand): Promise<BrowserPilotBridgeResponse> {
	...
	const listenerId =
		msg.listenerId ||
		msg.listener_id ||
		"listener_" + tabId + "_" + Date.now() + "_" + Math.random().toString(36).slice(2);
      ...
      return { ok:true, listenerId, eventType, selector, target: selector || 'document', replaced: !!prior };
		...
		const resp = normalizePersistentBrowserPilotResponse(
			await cdp.send(
				...
				"Runtime.evaluate",
				{ expression, returnByValue: true },
				{ persistent: true, name: "event_listener", timeoutMs: msg.timeoutMs || 5000 },
			),
		);
		if (!resp || resp.ok === false) return resp;
		pageResult = extractBrowserPilotRuntimeValue(resp);
		if (pageResult && pageResult.ok === false)
			return browserPilotError(
				String(pageResult.code || BROWSER_PILOT_ERROR_CODES.INVALID_RULE),
				pageResult.message || "hook.addEventListener failed",
				pageResult,
			);
	}
	const sub: JsonRecord & { tabId: number; listenerId: string } = {
		tabId: Number(tabId),
	...
	return {
		ok: true,
		data: { tabId: Number(tabId), listenerId, listener_id: listenerId, eventType, selector, page: pageResult },
	};
}
      ...
      return { removed:true, listenerId, eventType:rec.eventType, selector:rec.selector || null, eventCount:rec.eventCount || 0 };
		...
		const resp = normalizePersistentBrowserPilotResponse(
			await cdp.send(
				...
				"Runtime.evaluate",
				{ expression, returnByValue: true },
				{ persistent: true, name: "event_listener_remove", timeoutMs: msg.timeoutMs || 5000 },
			),
		);
		if (!resp || resp.ok === false) return resp;
		pageResult = extractBrowserPilotRuntimeValue(resp);
...
async function getPerformanceEntries(
	tabId: number,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
	const input = msg || {};
	...
	if (cdp?.send) {
		const resp = normalizePersistentBrowserPilotResponse(
			await cdp.send(
				...
				"Runtime.evaluate",
				{ expression, returnByValue: true },
				{ persistent: true, name: "performance_entries", timeoutMs },
			),
		);
		if (!resp || resp.ok === false) return resp;
		...
		if (result.exceptionDetails)
			return browserPilotError(
				...
				waitRecord(exceptionDetails.exception).description || "Runtime.evaluate failed",
				exceptionDetails,
			);
...
async function diagnoseSelectorForWait(tabId: number, msg: BrowserPilotBridgeCommand): Promise<JsonRecord | null> {
	...
	if (!selector)
		return waitId
			...
			: null;
	const probe = await callPageBrowserPilot(tabId, "hook.evaluate", {
...
async function diagnoseBrowserPilot(
	tabId: number,
	msg: BrowserPilotBridgeCommand,
): Promise<BrowserPilotBridgeResponse> {
	const tab = await chrome.tabs.get(tabId).catch((e: unknown) => ({ error: waitErrorMessage(e) }));
	const status = await callPageBrowserPilot(tabId, "hook.status", {}).catch((e: unknown) =>
		browserPilotError(BROWSER_PILOT_ERROR_CODES.NO_SESSION, waitErrorMessage(e), {}),
	);
	const cdp = browserPilotPersistentCdp();
	...
	const installed_marker = !!(
		...
		!["CLOSED", "CREATED"].includes(String(statusData.state || "").toUpperCase())
	);
	...
	const activeWaits = Array.from(browserPilotWaits.values())
		.filter((r) => Number(r.tabId) === Number(tabId))
		.map((r) => ({
			waitId: r.waitId,
			kind: r.kind,
	...
	const selectorDiagnostics = await diagnoseSelectorForWait(tabId, msg || {}).catch((e: unknown) => ({
		error: waitErrorMessage(e),
	}));
	const listeners = Array.from(browserPilotWaits.eventSubscriptionValues())
	...
	try {
		if (chrome.debugger?.getTargets)
			debuggerTargets = (await chrome.debugger.getTargets()).filter(
				(t: JsonRecord) => Number(t.tabId) === Number(tabId),
			);
	} catch (e) {
	...
	const cdpLeaks = {
		...
		subscription_leaks: activeCdpSubscriptions.filter(
			(s) => s.waitId && !browserPilotWaits.has(waitKey(s.tabId, s.waitId)),
		),
	};
	return {
		ok: true,
		data: {
			tabId: Number(tabId),
			tab,
			sessions: Array.from(browserPilotSessions.entries()).map(([tid, s]) => ({ tabId: tid, ...s })),
			session: browserPilotSessions.get(Number(tabId)) || null,
			waits: activeWaits,
			activeWaits,
			recentWaits,
			selectorDiagnostics,
			...
			epoch,
			diagnostics,
			cdp: { persistent: !!cdp, debuggerTargets, ...cdpObservability, leaks: cdpLeaks },
...
export {
	waitForAny,
	waitForAll,
	normalizeBrowserPilotWaitKind,
	dispatchBrowserPilotWait,
	cancelWait,
	extractBrowserPilotRuntimeValue,
	cleanupBrowserPilotPageListenersForTab,
	addEventListener,
	removeEventListener,
	getPerformanceEntries,
	diagnoseBrowserPilot,
};
