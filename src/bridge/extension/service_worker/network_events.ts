...
export function appendBounded<T>(
	...
	item: T,
	max: number,
	overflowCounterTarget: { overflow?: number } | null = null,
): void {
	if (max <= 0) return;
	arr.push(item);
	if (arr.length > max) {
...
export async function browserPilotNetworkMaybeCaptureBody(
	...
	if (!mimeDecision.match) {
		...
		setNetworkBodyAvailability(rec, mimeDecision.availability || "not_requested", mimeDecision.reason, {
	...
	try {
		const result = await deps.cdpSendNetworkCommand(
			recorder.tabId,
...
function recordNetworkEvent(
	recorder: NetworkRecorder,
	params: JsonRecord,
	deps: RecorderEventDeps,
	counter: RecorderCounter,
	eventType: string,
	update: (rec: NetworkRecord) => void,
	requestId: unknown = params.requestId,
): NetworkRecord {
...
function requestDetails(recorder: NetworkRecorder, rec: NetworkRecord, request: JsonRecord): NetworkRecord["request"] {
	return {
		...(rec.request || {}),
		url: String(request.url || rec.request?.url || ""),
		method: String(request.method || rec.request?.method || "GET"),
		headers: recorder.config.storeHeaders ? asRecord(request.headers || rec.request?.headers || {}) : {},
...
function responseDetails(
	...
	rec: NetworkRecord,
	response: JsonRecord,
): NonNullable<NetworkRecord["response"]> {
	return {
		url: String(response.url || rec.request?.url || ""),
		status: Number(response.status || 0),
		statusText: String(response.statusText || ""),
		headers: recorder.config.storeHeaders ? asRecord(response.headers) : {},
		mimeType: String(response.mimeType || ""),
		charset: String(response.charset || ""),
		connectionReused: !!response.connectionReused,
		connectionId: response.connectionId == null ? undefined : String(response.connectionId),
		...
		securityState: String(response.securityState || ""),
...
function handleRequestExtraInfo(recorder: NetworkRecorder, params: JsonRecord, deps: RecorderEventDeps): void {
	recordNetworkEvent(recorder, params, deps, "requestExtraInfo", "request_extra", (rec) => {
		rec.requestExtraInfo = {
			headers: recorder.config.storeHeaders ? asRecord(params.headers) : {},
			associatedCookies: Array.isArray(params.associatedCookies) ? params.associatedCookies : [],
...
function handleResponseExtraInfo(recorder: NetworkRecorder, params: JsonRecord, deps: RecorderEventDeps): void {
	recordNetworkEvent(recorder, params, deps, "responseExtraInfo", "response_extra", (rec) => {
		...
		rec.responseExtraInfo = {
			headers: recorder.config.storeHeaders ? asRecord(params.headers) : {},
			blockedCookies: Array.isArray(params.blockedCookies) ? params.blockedCookies : [],
			statusCode,
...
function handleServedFromCache(recorder: NetworkRecorder, params: JsonRecord, deps: RecorderEventDeps): void {
	recordNetworkEvent(recorder, params, deps, "servedFromCache", "cache", (rec) => {
		rec.fromCache = true;
	});
}
...
function handleLoadingFinished(recorder: NetworkRecorder, params: JsonRecord, deps: RecorderEventDeps): void {
	const rec = recordNetworkEvent(recorder, params, deps, "finished", "finished", (entry) => {
		...
		if (params.encodedDataLength !== undefined)
			entry.data.encodedDataLength = Math.max(
				Number(entry.data.encodedDataLength || 0),
...
export function browserPilotNetworkHandleRecorderCdpEvent(
	recorder: NetworkRecorder,
	...
	try {
		...
		if (handler) {
			...
			appendBounded(
				...
				{ t: Date.now(), method, frameId: params.frameId, name: params.name, loaderId: params.loaderId },
				100,
			);
