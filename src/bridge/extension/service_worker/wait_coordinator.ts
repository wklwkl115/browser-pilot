...
import {
	...
	rememberBrowserPilotCdpCleanup,
...
// Browser Pilot wait registry, cleanup, timeout and event subscription helpers.
...
type TerminalWaitRecord = JsonRecord & {
	key: string;
	waitId: string;
	tabId: number;
	kind: string;
...
class WaitCoordinator {
	...
	epoch = 0;
	constructor() {
		this.activeWaits = new Map<string, BrowserPilotWaitRecord>();
		this.eventSubscriptions = new Map<string, BrowserPilotEventSubscription>();
		this.terminalWaits = new Map<string, TerminalWaitRecord>();
		this.epoch = 0;
	}
	makeWaitId(tabId: unknown, kind: unknown): string {
		return makeWaitId(tabId, kind);
	}
	waitKey(tabId: unknown, waitId: unknown): string {
		return waitKey(tabId, waitId);
	}
	eventSubscriptionKey(tabId: unknown, listenerId: unknown): string {
	...
	get(key: string): BrowserPilotWaitRecord | undefined {
		return this.activeWaits.get(key);
	}
	set(key: string, record: BrowserPilotWaitRecord): this {
		this.activeWaits.set(key, record);
		return this;
	}
	...
	terminalValues(tabId?: unknown): TerminalWaitRecord[] {
		this.pruneTerminal();
		return Array.from(this.terminalWaits.values()).filter(
			(w) => tabId === undefined || tabId === null || Number(w.tabId) === Number(tabId),
		);
	}
	pruneTerminal(): void {
		const now = Date.now();
		for (const [key, wait] of Array.from(this.terminalWaits.entries()))
			if (now - Number(wait.completedAt || 0) > BROWSER_PILOT_TERMINAL_WAIT_MAX_AGE_MS)
				this.terminalWaits.delete(key);
		const overflow = this.terminalWaits.size - BROWSER_PILOT_TERMINAL_WAIT_MAX_RECORDS;
		if (overflow > 0)
			for (const key of Array.from(this.terminalWaits.keys()).slice(0, overflow)) this.terminalWaits.delete(key);
	}
	values(): IterableIterator<BrowserPilotWaitRecord> {
		return this.activeWaits.values();
	}
	entries(): IterableIterator<[string, BrowserPilotWaitRecord]> {
	...
	[Symbol.iterator](): IterableIterator<[string, BrowserPilotWaitRecord]> {
	...
	eventSubscription(listenerId: unknown, tabId?: unknown): BrowserPilotEventSubscription | null {
		if (tabId !== undefined && tabId !== null)
			return this.eventSubscriptions.get(eventSubscriptionKey(tabId, listenerId)) || null;
		const matches = Array.from(this.eventSubscriptions.values()).filter(
	...
	deleteEventSubscription(listenerId: unknown, tabId?: unknown): boolean {
		if (tabId !== undefined && tabId !== null)
			return this.eventSubscriptions.delete(eventSubscriptionKey(tabId, listenerId));
		let deleted = false;
	...
	eventSubscriptionValues(): IterableIterator<BrowserPilotEventSubscription> {
	...
	cleanupWait(record: BrowserPilotWaitRecord, reason?: string): void {
		return cleanupWait(record, reason);
	}
	cleanupWaitsForFrame(tabId: unknown, frameId: unknown, reason?: string): number {
		return cleanupWaitsForFrame(tabId, frameId, reason);
	}
	cleanupWaitsForUninstall(tabId: unknown): number {
		return cleanupWaitsForUninstall(tabId);
	}
	diagnostics(tabId?: unknown) {
		const scopedEvents = Array.from(this.eventSubscriptions.values()).filter(
...
function cleanupWait(record: BrowserPilotWaitRecord, reason?: string): void {
	return cleanupBrowserPilotWait(record, reason);
}
function cleanupWaitsForFrame(tabId: unknown, frameId: unknown, reason?: string): number {
	let n = 0;
	for (const r of Array.from(browserPilotWaits.values()))
		if (Number(r.tabId) === Number(tabId) && String(r.frameId || "") === String(frameId || "")) {
			cleanupBrowserPilotWait(r, reason || "FRAME_DETACHED");
			n++;
		}
	return n;
}
function cleanupWaitsForUninstall(tabId: unknown): number {
...
try {
	...
	if (
		...
		!waitGlobal.__browserPilotUnhandledRejectionCleanupInstalled
	) {
...
function normalizeBrowserPilotTimeoutMs(
	msg: BrowserPilotBridgeCommand | null | undefined,
	fallback = BROWSER_PILOT_DEFAULT_WAIT_TIMEOUT_MS,
): number {
	const hasExplicit =
		msg && (msg.timeoutMs !== undefined || msg.timeout_ms !== undefined || msg.timeout !== undefined);
...
function makeWaitId(tabId: unknown, kind: unknown): string {
	return "wait_" + Number(tabId) + "_" + String(kind || "generic") + "_" + Date.now() + "_" + ++browserPilotWaitSeq;
}
function waitKey(tabId: unknown, waitId: unknown): string {
	return Number(tabId) + ":" + String(waitId);
}
function eventSubscriptionKey(tabId: unknown, listenerId: unknown): string {
	return Number(tabId) + "::" + String(listenerId);
}
function terminalWaitRecord(
...
function waitAbortMessage(record: BrowserPilotWaitRecord): string {
	return "browserPilot wait " + record.waitId + " cancelled";
}
...
function registerWait(tabId: number, kind: string, criteria: BrowserPilotBridgeCommand = {}): BrowserPilotWaitRecord {
	...
	const record: BrowserPilotWaitRecord = {
...
function clearWait(record: BrowserPilotWaitRecord | null | undefined, reason?: string): void {
	...
	if (shouldAbortWaitCleanupReason(reason)) {
		try {
			...
			/* best-effort abort during wait cleanup */
		}
	}
	...
	for (const sid of record.cdpSubscriptions.splice(0)) {
		try {
			...
			console.warn("[BROWSER-PILOT-WAIT] Failed to unsubscribe CDP wait subscription", sid, error);
		}
	}
...
function cleanupBrowserPilotWait(record: BrowserPilotWaitRecord, reason?: string): void {
	return clearWait(record, reason);
}
function isWaitRecordForTab(record: BrowserPilotWaitRecord | null | undefined, tabId: unknown): boolean {
...
function cleanupTabWaits(tabId: unknown, reason?: string, options: CleanupTabWaitsOptions = {}) {
	...
	cleanupEventSubscriptionsForTab(tabId);
	if (cleaned || orphaned || opts.remember !== false)
		rememberBrowserPilotCdpCleanup({
			tabId: Number(tabId),
			...
			remaining_waits: Array.from(browserPilotWaits.values()).filter((r) => isWaitRecordForTab(r, tabId)).length,
		});
...
function finishBrowserPilotWait(
	record: BrowserPilotWaitRecord,
	ok: boolean,
	...
	return browserPilotError(errorCode || BROWSER_PILOT_ERROR_CODES.INTERNAL_ERROR, message || "wait failed", {
		...base,
		...(details || {}),
	});
}
export {
	...
	browserPilotWaits,
	BROWSER_PILOT_ORPHAN_WAIT_MAX_AGE_MS,
	cleanupBrowserPilotOrphanWaits,
	browserPilotWaitSeq,
	BROWSER_PILOT_DEFAULT_WAIT_TIMEOUT_MS,
