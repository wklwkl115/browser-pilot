...
async function createSelftestTab(steps: Array<Record<string, unknown>>): Promise<number> {
	...
	const createEnv = parseSelftestJson(requireSelftestToolOk("create-temp-tab", create), "create-temp-tab") as { data?: { tabId?: number } };
	const tabId = createEnv.data?.tabId;
...
async function executeSelftest(tabId: number, steps: Array<Record<string, unknown>>): Promise<void> {
	...
	const ok = execText.includes("browser-pilot selftest ok");
