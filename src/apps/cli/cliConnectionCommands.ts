...
import { daemonContractReport, findDaemon, isDaemonReadyForReuse, stopDaemon } from "../daemon/daemonControl.js";
...
export async function runStatusCommand(argv: string[]): Promise<number> {
	...
	const specs: FlagSpec[] = [
		{ name: "tabs", flag: "--tabs", kind: "boolean", required: false, description: "Include full tabs[] instead of the default compact tabCount/activeTab fields." },
		{ name: "check", flag: "--check", kind: "boolean", required: false, description: "Exit 1 when the local and daemon command contracts do not match." },
	];
	const parsed = parseArgs(specs, argv);
	if (!parsed.ok) return renderUsageError(parsed.error, renderMode(parsed.globals));
	if (parsed.value.globals.help) {
		process.stdout.write("browser-pilot status --json\n\nRead-only browser connection status. Does not start daemon or bridge.\n\nFlags:\n  --tabs                         Include full tabs[]. Default is compact tabCount/activeTab.\n  --check                        Exit 1 if the daemon contract does not match local.\n");
		return EXIT.ok;
	}
	const env = await connectionStatus(process.cwd(), 15_000, { tabs: parsed.value.params.tabs === true });
	const check = ((env.contract as { check?: { ok?: boolean; code?: string } } | undefined)?.check);
	const failed = parsed.value.params.check === true && check?.ok !== true;
	if (mode === "json") {
		if (!failed) return renderLocalJson({ ...env, ...(parsed.value.params.check === true ? { checked: true } : {}) });
		writeJsonEnvelope({ ...env, checked: true, ok: false, exitCode: EXIT.toolError, code: check?.code ?? "DAEMON_CONTRACT_MISMATCH" });
		return EXIT.toolError;
	}
	process.stdout.write(`ready: ${env.ready === true ? "true" : "false"} (${String(env.readiness ?? "unknown")})\n`);
	process.stdout.write(`contract: ${check?.ok === true ? "match" : "mismatch"}\n`);
...
async function daemonStatusMode(mode: ReturnType<typeof jsonMode>): Promise<number> {
	...
	const status = { command: "daemon.status", running: true, pid: found.info.pid, controlPort: found.info.controlPort, version: found.info.version, expectedVersion: daemonVersion(), versionStale: !isDaemonReadyForReuse(found), contract: daemonContractReport(found), ...found.status };
...
function daemonStatusMissing(mode: ReturnType<typeof jsonMode>): number {
	if (mode === "json") return renderLocalJson({ command: "daemon.status", running: false, daemon: null, expectedVersion: daemonVersion(), contract: daemonContractReport(), staleLockfile: staleLockfileDiagnostic() });
