...
import { TAB_SCOPED_TOOL_GUIDELINE, boundedExecutionParams, browserCookieBindingParams, executeWebSecurityToolShell, harReplayParams, maxTemplatesParam, rateLimitPerSecondParam, redirectControlParams, requestSequenceParams, resolveBooleanParam, sharedWebSecurityBrowserSessionParams, sharedWebSecurityParams, normalizeWebSecurityToolParams, type WebSecuritySharedToolParams } from "./shared";
...
export function registerTemplateTool({ pi, ensureStarted }: ToolRegistrarContext) {
	pi.registerTool({
		...
		promptGuidelines: [TAB_SCOPED_TOOL_GUIDELINE, "Use browser_template with explicit scope. engine:builtin is the default small exposure/API baseline or custom template path; engine:nuclei is the deeper mature template path."],
		parameters: Type.Object({
			...
			headers: Type.Optional(Type.Any({ description: "Base request headers; template headers merge over them or nuclei replay override." })),
			...
			templateIds: Type.Optional(Type.Any({ description: "builtin mode: built-in template ids or tags to run. Omit templateIds/templates/templatePath to run the small built-in exposure/API baseline; use all for every built-in template. nuclei mode: optional nuclei template ids passed via -id." })),
			...
			tags: Type.Optional(Type.Any({ description: "nuclei mode only: optional nuclei tag selector list passed via -tags." })),
			excludeTags: Type.Optional(Type.Any({ description: "nuclei mode only: optional nuclei exclude-tag selector list passed via -etags." })),
			severities: Type.Optional(Type.Any({ description: "nuclei mode only: optional nuclei severity selector list passed via -severity." })),
			authors: Type.Optional(Type.Any({ description: "nuclei mode only: optional nuclei author selector list passed via -author." })),
			...harReplayParams({
