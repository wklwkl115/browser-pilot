		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			return await runTool(async () => {
				...
				if (action === "read") {
					...
					return await jsonToolResult({ action: "read", ...result, id: params.id, uri: params.uri }, { browserSessionId: undefined, detailLevel: undefined, outputPath: undefined, maxChars }, ctx, { toolName: "browser_memory", fallbackName: "browser-memory-read.json" });
				}
				if (action === "recall") {
					...
					return await jsonToolResult({ action: "recall", scopeKind: params.scopeKind, scopeKey: params.scopeKey, query: params.query, cards }, { browserSessionId: undefined, detailLevel: undefined, outputPath: undefined, maxChars }, ctx, { toolName: "browser_memory", fallbackName: "browser-memory-recall.json" });
				}
