import { buildControlsSourceEntity, buildDomEntityFromScanActionable, buildReferencedTargetEntity, buildRegionEntityFromListHint, buildVisionRegionFromCanvasActionable, type ScanEntityContext } from "../abml/entity.js";
...
function annotateNode(node: Record<string, unknown>, slot: string, refId: string): Record<string, unknown> {
	const refs = isRecord(node.__piEntityRefs) ? node.__piEntityRefs : {};
	return { ...node, __piEntityRefs: { ...refs, [slot]: refId } };
}
