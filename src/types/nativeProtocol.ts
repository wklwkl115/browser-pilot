...
export type CommandSpec = {
	...
	internal?: boolean;
	/** Public commands default to "core"; "advanced" ones are discoverable through the resource index only. */
	tier?: "core" | "advanced";
	methods?: string[];
...
function commandPayload(command: Record<string, unknown>, allowResolvedTarget: boolean): Record<string, unknown> {
	return Object.fromEntries(
		Object.entries(command).filter(
			([key]) => !COMMAND_ENVELOPE_FIELDS.has(key) && !(allowResolvedTarget && key === "target"),
		),
	);
}

function validateCommandEnvelope(command: Record<string, unknown>): string | undefined {
	if (command.tabId !== undefined && (!Number.isInteger(command.tabId) || Number(command.tabId) <= 0))
	...
	if (command.timeoutMs !== undefined && (!Number.isInteger(command.timeoutMs) || Number(command.timeoutMs) < 0))
		return "timeoutMs must be a non-negative integer";
	return undefined;
}
...
export function validateBridgeCommand(
	command: unknown,
	options: { allowMissingTabId?: boolean; allowResolvedTarget?: boolean; publicCall?: boolean } = {},
): BridgeCommandValidation {
	const currentSchema = getNativeCommandProtocolSchema();
	if (!isRecord(command))
		return { ok: false, error: "Bridge command must be an object", details: { commandType: typeof command } };
	if (typeof command.cmd !== "string" || !command.cmd.trim())
		return { ok: false, error: "Bridge command requires string cmd", details: { cmd: command.cmd } };
	...
	if (options.publicCall) {
		...
		if (field)
			return {
				ok: false,
				error: `${cmd}: ${field} is runtime-managed; use the tool-level targetRef only when tab disambiguation is required`,
				details: { cmd, field },
			};
	}
	...
	if (methods.length) {
		...
		if (spec.methodRequired && !hasValue(rawMethod))
			return { ok: false, error: `${cmd} requires method`, details: { cmd } };
	...
	if (missing.length)
		return { ok: false, error: `${cmd} missing required fields: ${missing.join(", ")}`, details: { cmd, missing } };

	const requiredAny = [...(spec.requiredAny || []), ...(methodSpec?.requiredAny || [])];
	if (!requiredAnySatisfied(checked, requiredAny))
		return { ok: false, error: `${cmd} requires one of field groups`, details: { cmd, requiredAny } };
	if (
		canonicalCmd === "persistent_cdp" &&
		checked.action === "detachTarget" &&
		!hasValue(checked.targetId) &&
		!hasValue(checked.sessionId)
	) {
		return {
			ok: false,
			error: `${cmd} detachTarget requires targetId or sessionId`,
			details: { cmd, action: checked.action },
		};
	}
	...
	if (spec.tabScoped && !options.allowMissingTabId && toTabId(checked.tabId) === undefined)
