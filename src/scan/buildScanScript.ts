...
(async () => {
  ...
  function collectActionables(root, refElements) {
    ...
    for (const el of source) {
      ...
      const item = { index: out.length, selector: selectorFor(el), tag: el.tagName.toLowerCase(), role: roleOf(el), action, label: labelOf(el), text: clean(el.innerText || el.textContent || '', 120), clickable: isClickable, editable: isEditable, disabled: !!el.disabled || el.getAttribute('aria-disabled') === 'true', focused: document.activeElement === el, ...(checkedState === undefined ? {} : { checked: checkedState }), ...(currentAttr ? { current: currentAttr } : {}), ...(el.tagName === 'INPUT' && el.type ? { inputKind: String(el.type).toLowerCase() } : {}), ...(controlsSelectors.length ? { controlsSelectors } : {}), ...(ownsSelectors.length ? { ownsSelectors } : {}), ...((expandedAttr === 'true' || expandedAttr === 'false') && controlsSelectors.length ? { expandedTargetSelectors: controlsSelectors } : {}), handlers: handlers.slice(0, 6), rect: visible.rect, point: visible.point, hitOk: visible.hitOk, hitTarget: visible.hitTarget, ...((el.tagName === 'A' || el.tagName === 'AREA') && el.href ? { href: String(el.href) } : {}), ...(visible.hitOk === false && visible.occluderSelector ? { occluderSelector: visible.occluderSelector } : {}) };
