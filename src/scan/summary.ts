...
import type {
	PageWorldScanBundleV1,
	ScanActionable,
	ScanCanvasRegion,
	ScanListHint,
} from "../kernels/abml/pageWorldScan.js";
...
function nodeRefId(
	node: ScanActionable | ScanListHint | ScanCanvasRegion,
	built: { descriptor: Parameters<typeof summaryRefIdForDescriptor>[0] },
	slot: string,
): string {
...
function listHintDuplicateNames(listHints: ScanListHint[]): Set<string> {
	...
	for (const [index, item] of listHints.entries()) {
		const built = buildRegionEntityFromListHint(
			item,
			{ observationId: "scan:list-hint-name", capturedAt: 0 },
			index,
		);
...
function buildListRegionEntity(
	node: ScanListHint,
...
export function buildScanEntities(
	item: PageWorldScanBundleV1,
	options: ScanEntityOptions = {},
): {
	entities: Entity[];
	primaryEntities: Entity[];
	listEntities: Entity[];
	visualRegions: Entity[];
	referencedEntities: Entity[];
	controlsSources: Entity[];
} {
	const context = scanEntityContext(item, options);
	const actionables = item.structure.actionables.filter(
		(node) => node.referenceOnly !== true && node.relationOnly !== true,
	);
	...
	const visualRegions = dedupeEntities(
		canvasRegions.map((node) => {
	...
	const listEntities = dedupeEntities(
		item.structure.listHints.map((node, index) => buildListRegionEntity(node, context, index, duplicateListNames)),
	);
	const actionableSelectors = new Set(
		actionEntities
			.map((entity) => (typeof entity.hints?.selector === "string" ? entity.hints.selector : null))
			.filter(Boolean),
	);
	const referencedOnly = referencedEntities.filter(
		(entity) => typeof entity.hints?.selector !== "string" || !actionableSelectors.has(entity.hints.selector),
	);
	const controlsSourceOnly = controlsSourceEntities.filter(
		(entity) => typeof entity.hints?.selector !== "string" || !actionableSelectors.has(entity.hints.selector),
	);
	const entities = dedupeEntitiesByRef([
		...actionEntities,
		...referencedOnly,
		...controlsSourceOnly,
		...listEntities,
	...
	const referencedSurvivors = [...referencedOnly, ...controlsSourceOnly].filter((entity) =>
		entities.includes(entity),
	);
	const actionableCandidates = actionEntities.filter((entity) =>
		String(entity.hints?.jsonPath || "").startsWith("data.structure.actionables["),
	);
	const highSignal = (entity: Entity) =>
		(entity.state?.current !== undefined && entity.state.current !== false) ||
		entity.state?.checked === true ||
		entity.state?.selected === true ||
		entity.state?.pressed === true;
...
export function scanEntitiesFromGroups(built: ReturnType<typeof buildScanEntities>): Entity[] {
	return dedupeEntitiesByRef([
		...built.entities,
		...built.primaryEntities,
		...built.listEntities,
		...built.visualRegions,
		...built.referencedEntities,
		...built.controlsSources,
	]);
}
