...
export function defineSqliCommand({ commands, ensureStarted }: CommandRegistrarContext) {
	commands.define({
		name: "browser_sqli",
		label: "Browser SQLi",
		...
		parameters: strictCommandParameters({
			...
			wordlistPath: Type.Optional(Type.String({ description: "engine:builtin only: local wordlist file path (one payload per line, # = comment) whose entries are appended to the default error-probe payloads. Use src/commands/webSecurity/wordlists/sqli-error-payloads.txt as a base or provide a community wordlist. No-override default behavior is byte-identical." })),
			timePayloadWordlistPath: Type.Optional(Type.String({ description: "engine:builtin only: local wordlist file path (one payload per line, # = comment) whose entries are appended to the default time-probe payloads. Use src/commands/webSecurity/wordlists/sqli-time-payloads.txt as a base." })),
			...
			allowLauncherOverride: Type.Optional(Type.Boolean({ description: "engine:sqlmap only: required when sqlmapPath or BROWSER_PILOT_SQLMAP_PATH overrides the auto-detected launcher. Default false." })),
