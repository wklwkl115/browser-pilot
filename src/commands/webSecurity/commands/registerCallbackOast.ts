...
export function defineCallbackOastCommand({ commands, ensureStarted }: CommandRegistrarContext) {
	commands.define({
		...
		parameters: strictCommandParameters({
			...
			basePath: Type.Optional(Type.String({ description: "Callback path template. {{correlationId}} is replaced; default /__browser_pilot_oast/{{correlationId}}." })),
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			return executeWebSecurityToolShell(ensureStarted, current, ctx, {
				commandName: "browser_callback_oast",
