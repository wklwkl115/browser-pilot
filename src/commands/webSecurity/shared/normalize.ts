 ...
 * caller's .browser-pilot/ rather than the daemon's working directory.
 */
...
function allowedWordlistRoots(): string[] {
	...
	return [cwd, path.join(cwd, ".browser-pilot")];
}
...
function resolveScopedWordlistPath(filePath: string): string {
	...
	if (!allowed) {
		throw wordlistPathError("browser webSecurity wordlistPath must stay under the current working directory or .browser-pilot/", { path: filePath, resolved, allowedRoots });
	}
	return resolved;
}
