...
import type {
	...
	CollectionSummary,
	CompactCollection,
	ObservationFrontierItem,
	PageObservationContent,
	PageObservationV3,
	PageObservationView,
	PublicCausalSummary,
	PublicRelationSummary,
	PublicVisualObservation,
...
export function semanticContentSections(content: PageObservationContent): ObservationContentSection[] {
	...
	if (positions[0]!.start > 0)
		sections.push({ label: "Page introduction", text: text.slice(0, positions[0]!.start).trim() });
...
function structureMapLines(observation: PageObservationV3): string[] {
	...
	for (const collection of observation.collections ?? []) {
		...
		const total =
			collection.total === undefined ? String(collection.observed) : `${collection.observed}/${collection.total}`;
		...
		lines.push(
			`collection ${label} (${total}, ${collection.completeness})${template ? ` item=${template.role}` : ""}${defaults ? ` defaults=${defaults}` : ""}${varies ? ` varies=${varies}` : ""}`,
		);
	}
	return lines;
}
...
function rootContentProjection(
	observation: PageObservationV3,
	sections: IndexedContentSection[],
): RootContentProjection | undefined {
	...
	const previewChars = sections.length
		? Math.max(0, Math.floor((budgetAfterPrefix - labelChars) / sections.length))
		: 0;
	...
	for (const { section, fullBody, body, line } of previews) {
		...
		includedBySection.set(
			...
			complete
				...
				: Math.min(
						section.text.length,
						(section.text.startsWith(section.label) ? labelVisible : 0) + bodyVisible,
					),
		);
		offset += line.length + 1;
	}
	return {
		text: `${prefix}${separator}${previews.map((preview) => preview.line).join("\n")}`.slice(
			0,
			ROOT_CONTENT_MAX_CHARS,
		),
		complete: false,
		includedBySection,
	};
		...
		return [
			{
				...
				memberRefs: item.memberRefs
					.filter((ref): ref is string => typeof ref === "string" && ref.startsWith("bp-ref://"))
					.slice(0, ROOT_SAMPLE_SIZE),
			},
		];
	});
}
...
export function publicWarnings(value: PageObservationV3["diagnostics"]): string[] {
	...
	const visualUnavailable =
		...
		value.providerFailures.some((failure) => isRecord(failure) && failure.provider === "visual");
	return [
		...new Set([
			...(value.abmlIntegrated === false
				? ["Semantic structure was unavailable; using the page scan fallback."]
				: []),
			...(isRecord(value.axFusion) && value.axFusion.degraded === true
				? ["Accessibility enrichment was incomplete."]
				: []),
...
export function publicGist(gist: PageObservationV3["gist"], title?: string): PageObservationView["gist"] {
	const landmarks = Array.isArray(gist?.landmarks)
		? gist.landmarks.filter((item): item is string => typeof item === "string")
		: [];
	return title || landmarks.length
		? { ...(title ? { title } : {}), ...(landmarks.length ? { landmarks } : {}) }
		: undefined;
}
...
export function publicSnapshotProjection(
	value: PageObservationV3["snapshotProjection"],
	...
	return {
		...
		templates: value.templates.map((template) => ({
			...
			exceptions: template.exceptions.map((item) => ({
				index: item.index,
			...
			...(template.sample
				? {
						sample: {
							...(template.sample.ref ? { ref: template.sample.ref } : {}),
							...(template.sample.name ? { name: template.sample.name } : {}),
							...(template.sample.value ? { value: template.sample.value } : {}),
						},
					}
				: {}),
		})),
...
export function publicTreeDiff(value: PageObservationV3["treeDiff"]): Record<string, unknown> | undefined {
	if (!value) return undefined;
	const instances = (
		items: Array<{ ref: string; confidence: string; name?: string; value?: string; posInSet?: number }>,
	) =>
		items.map((item) => ({
			ref: item.ref,
	...
	return {
		...
		templates: value.templates.map((template) => ({
			...
			changed: {
				count: template.changed.count,
				instances: template.changed.instances.map((item) => ({
					ref: item.afterRef,
					confidence: item.confidence,
					fields: item.fields,
					...(item.name ? { name: item.name } : {}),
			...
			...(template.reordered
				? { reordered: { changed: true, commonCount: template.reordered.commonCount } }
				: {}),
		})),
	};
}
...
function descriptor(
	observation: PageObservationV3,
	path: string,
	input: Omit<ObservationResourceDescriptor, "uri" | "mimeType" | "path" | "expiresAt" | "snapshotId">,
): ObservationResourceDescriptor {
	return {
		...input,
...
function fullObservationResource(
	observation: PageObservationV3,
	path: string,
	ref: string,
	label: string,
): { resource: ObservationResourceDescriptor; item: ObservationFrontierItem } {
...
function addSnapshotProjectionResource(
	observation: PageObservationV3,
	path: string,
	resources: ObservationResourceDescriptor[],
	...
	const resource = descriptor(observation, path, {
		name: label,
		ref,
		kind: "details",
		label,
		jsonPath: "snapshotProjection",
	});
	...
	items.push({
		...
		label,
		observed: 0,
		total: projection.templates.length,
		resourceUri: resource.uri,
	});
}

function limitedFrontier(
	observation: PageObservationV3,
	path: string,
	resources: ObservationResourceDescriptor[],
	items: ObservationFrontierItem[],
) {
	const priority: Record<ObservationFrontierItem["kind"], number> = {
		"action-space": 0,
		content: 1,
		details: 2,
		"collection-window": 3,
	};
	const itemPriority = (item: ObservationFrontierItem): number =>
		item.kind === "action-space" ? -2 : item.state === "unavailable" ? -1 : priority[item.kind];
	const ranked = items
	...
	return {
		items: selected,
		resources: [
			...resources.filter((resource) => visibleUris.has(resource.uri)),
...
function addDetailsResource(
	...
	input: {
		ref: string;
		label: string;
		jsonPath: "treeDiff" | "causal" | "relations";
		observed: number;
		total: number;
	},
): void {
...
function projectTreeDiff(
	observation: PageObservationV3,
	path: string,
	resources: ObservationResourceDescriptor[],
	items: ObservationFrontierItem[],
): PageObservationView["treeDiff"] {
	const treeDiff = observation.treeDiff;
	if (!treeDiff) return undefined;
	if (treeDiff.templates.length)
		addDetailsResource(observation, path, resources, items, {
			ref: "frontier:details:tree-diff",
			label: "Tree diff details",
			jsonPath: "treeDiff",
			observed: 0,
			total: treeDiff.templates.length,
		});
	return { summary: publicTreeDiffSummary(treeDiff.summary) };
}

export function publicCausal(
	causal: PageObservationV3["causal"],
	limit = Number.POSITIVE_INFINITY,
): PublicCausalSummary | undefined {
	...
	if (!("requests" in causal))
		return {
			unavailable: "Recent request activity was unavailable.",
			...(events?.length ? { events } : {}),
			...(causal.events && causal.events.length > (events?.length ?? 0)
				? { eventCount: causal.events.length }
				: {}),
		};
...
function projectCausal(
	...
	const observed =
		(projected && "requests" in projected ? projected.requests.length : 0) + (projected?.events?.length ?? 0);
	if (total > observed)
		addDetailsResource(observation, path, resources, items, {
			ref: "frontier:details:causal",
			label: "Complete causal activity",
			jsonPath: "causal",
			observed,
			total,
		});
...
export function publicRelations(
	relations: PageObservationV3["relations"],
	limit = Number.POSITIVE_INFINITY,
): PublicRelationSummary | undefined {
	if (!relations) return undefined;
	const highlights = relations.highlights
		.slice(0, limit)
		.map(({ type, sourceRef, targetRef }) => ({ type, sourceRef, targetRef }));
	const summary = Object.fromEntries(
		Object.entries(relations.summary).filter(
	...
	return {
		summary,
		highlights,
		...(relations.highlights.length > highlights.length ? { highlightCount: relations.highlights.length } : {}),
	};
}
...
function projectRelations(
	...
	if (relations.highlights.length > projected.highlights.length)
		addDetailsResource(observation, path, resources, items, {
			ref: "frontier:details:relations",
			label: "Complete relation highlights",
			jsonPath: "relations",
			observed: projected.highlights.length,
			total: relations.highlights.length,
		});
	return projected;
}
...
function projectCollections(
	...
	return selected.map((collection: CollectionSummary, index) => {
		...
		const controlRef =
			collection.paginationControl && typeof collection.paginationControl.ref === "string"
				? collection.paginationControl.ref
				: undefined;
		if (needsFrontier) {
			const label = collection.name || `Collection ${index + 1}`;
			const resource = hasOverflow
				? descriptor(observation, path, {
						name: label,
						ref,
						kind: "collection-window",
						label,
						jsonPath: `collections[${index}]`,
					})
				: undefined;
			...
			items.push({
				...
				kind: "collection-window",
				state:
					collection.completeness === "virtualized" ||
					collection.completeness === "paginated" ||
					collection.completeness === "lazy" ||
					collection.completeness === "viewport-window"
				...
				...(resource
					...
					: controlRef
						? {}
						: { unavailableReason: "No additional captured collection items are available." }),
			});
		}
		return {
			ref: collection.ref,
...
export function projectObservationResources(
	observation: PageObservationV3,
	path: string,
): { observation: PageObservationView; resources: ObservationResourceDescriptor[] } {
	const resources: ObservationResourceDescriptor[] = [];
	const items: ObservationFrontierItem[] = [];
	const deltaOnly =
		observation.delta === "session" ||
		observation.baselineSnapshotId !== undefined ||
		observation.diff !== undefined ||
		observation.treeDiff !== undefined;
	...
	if (observation.content?.complete === false)
		items.push({
			ref: "frontier:content:unavailable",
			kind: "content",
			state: "unavailable",
			label: "Uncaptured page content",
			unavailableReason: "capture reached the internal safety ceiling",
		});
	...
	const buildView = (
		frontier: ReturnType<typeof limitedFrontier>,
	...
	const fullActionSpace =
		observation.actionSpace && actionSpaceWindow(observation.actionSpace, observation.actionSpace.items.length);
	...
	if (fullActionSpace && Buffer.byteLength(JSON.stringify(projected), "utf8") > ROOT_RESULT_TARGET_BYTES) {
		...
		items.push({
			...
			label,
			observed: 0,
			total: fullActionSpace.coverage.captured,
			resourceUri: resource.uri,
		});
...
export function projectObservationOverflow(
	observation: PageObservationV3,
	path: string,
): { observation: PageObservationView; resources: ObservationResourceDescriptor[] } {
	...
	if (observation.actionSpace) {
		...
		items.unshift({
			...
			label,
			observed: 0,
			total: observation.actionSpace.coverage.captured,
			resourceUri: resource.uri,
		});
