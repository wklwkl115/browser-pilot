...
async function probeRecorderStates(options: ObserveProvidersOptions, timeoutMs: number, network: RecorderState | undefined, hook: RecorderState | undefined): Promise<{ network?: RecorderState; hook?: RecorderState }> {
	...
	try {
		...
		return { network: nextNetwork, hook: nextHook };
	} catch {
...
async function queryCausalNetworkDelta(
	...
	remainingMs: () => number,
): Promise<{ causal: CausalSummary; network?: RecorderState }> {
	...
	try {
		...
		options.signal?.throwIfAborted();
		return { causal: causalUnavailable("network recorder delta query failed"), network: knownNetwork };
	}
}

async function queryCausalHookEvents(
	options: ObserveProvidersOptions,
	knownHook: RecorderState | undefined,
	remainingMs: () => number,
): Promise<{ events?: ReturnType<typeof buildCausalEvents>; hook?: RecorderState }> {
...
async function runCausalProvider(options: ObserveProvidersOptions, item: CausalPlan, initialNetwork: RecorderState | undefined, initialHook: RecorderState | undefined): Promise<{ value?: CausalSummary; report: ProviderExecutionItem; network?: RecorderState; hook?: RecorderState }> {
	...
	const [networkResult, hookResult] = await Promise.all([
		queryCausalNetworkDelta(options, options.baseline?.networkSeq, knownNetwork, remainingMs),
		queryCausalHookEvents(options, knownHook, remainingMs),
	]);
	knownNetwork = networkResult.network;
	const causal = hookResult.events ? { ...networkResult.causal, ...hookResult.events } : networkResult.causal;
