...
export function mergeEntitiesByRef(...groups: unknown[][]): Entity[] {
	...
	for (const group of groups) {
		for (const item of group) {
			if (!isRecord(item) || typeof item.ref !== "string" || !isRecord(item.state) || seen.has(item.ref))
				continue;
			seen.add(item.ref);
...
export async function resolveBaselineEntities(
	server: BrowserCommandRuntimePort,
	snapshotId: string | undefined,
	signal?: AbortSignal,
): Promise<BaselineResolution | undefined> {
	if (!snapshotId) return undefined;
	const snapshot = server.getObservationSnapshot(snapshotId);
	if (!snapshot || snapshot.expired)
		throw new BrowserBridgeError(
			"INVALID_RULE",
			"browser_observe baseline snapshot is unavailable or expired",
			baselineRecovery({
				snapshotId,
	...
	if (!snapshot.saved?.path)
		throw new BrowserBridgeError(
			"INVALID_RULE",
			"browser_observe baseline snapshot has no saved artifact path",
			baselineRecovery({ snapshotId }),
		);
	let parsed: unknown;
	try {
		parsed = parseJsonOrThrow(
			await readFile(snapshot.saved.path, { encoding: "utf8", signal }),
			"browser_observe baseline snapshot artifact",
		);
	} catch (error) {
		signal?.throwIfAborted();
		throw new BrowserBridgeError(
			"INVALID_RULE",
			"browser_observe baseline snapshot artifact could not be read as JSON",
			baselineRecovery({ snapshotId, error: error instanceof Error ? error.message : String(error) }),
		);
	}
	if (!isPageObservationV3(parsed))
		throw new BrowserBridgeError(
			"INVALID_RULE",
			"browser_observe baseline snapshot artifact is not a canonical PageObservation",
			baselineRecovery({ snapshotId }),
		);
	const fromArtifact = parsed.entities ?? [];
	return {
		entities: fromArtifact,
		partialBaseline: false,
		networkSeq: snapshot.networkSeq,
		hookSeq: snapshot.hookSeq,
		snapshotId,
		pageIdentity: pageIdentityFromUnknown(snapshot) ?? pageIdentityFromUnknown(parsed),
	};
}
