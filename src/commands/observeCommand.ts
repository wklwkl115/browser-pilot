...
export function selectDiffBaselineSnapshot(
	...
	const effectiveTabId =
		resolveLocalTargetTabId(server, targetTabId(params), params.browserSessionId) ?? bridge.defaultTabId;
	const browserSessionId = bridge.browserSessionId;
	const pageIdentity = currentPageIdentity(server, {
	...
	return server
		...
		.filter(
			(snap) =>
				snap.browserSessionId === browserSessionId &&
				snap.tabId === effectiveTabId &&
				samePageIdentity(pageIdentityFromUnknown(snap), pageIdentity) &&
				snap.sourceMode === "scan" &&
				!snap.expired &&
				Boolean(snap.saved?.path),
		)
		.reduce(
			(latest, snap) => (latest === undefined || snap.capturedAt > latest.capturedAt ? snap : latest),
			undefined as ReturnType<BrowserCommandRuntimePort["listObservationSnapshots"]>[number] | undefined,
		)?.snapshotId;
}
		...
		parameters: strictCommandParameters({
			...
			visual: Type.Optional(
				Type.Enum(["auto", "always", "never"], {
					description: "Attach a coherent viewport screenshot automatically, always, or never.",
				}),
			),
			...sharedTabScopedToolParams(),
		}),
