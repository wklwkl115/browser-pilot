...
export function defineObserveCommand({ commands, ensureStarted }: CommandRegistrarContext) {
	defineBrowserCommand(commands, {
		...
		parameters: strictCommandParameters({
			...
			actionRef: Type.Optional(Type.String({ description: "scan mode only (R3.x P1): bp-ref:// of the control you just activated; attributes the baseline network-delta to it as `triggered` relations. Falls back to the focused control when omitted." })),
