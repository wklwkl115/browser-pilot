...
export async function withBrowserOperation<T>(
	...
	const abortFromCaller = () =>
		controller.abort(
			options.signal?.reason ??
				new BrowserBridgeError("BRIDGE_TIMEOUT", "Browser operation was cancelled", { aborted: true }),
		);
	const timer = setTimeout(
		() =>
			controller.abort(
				new BrowserBridgeError("BRIDGE_TIMEOUT", `Browser operation exceeded ${options.timeoutMs}ms`, {
	...
	try {
		return options.server.withTargetTransaction && options.tabId !== undefined
			? await options.server.withTargetTransaction(
					{
