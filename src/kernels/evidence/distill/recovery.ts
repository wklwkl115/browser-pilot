...
export function uniqueRecoveryActions(actions: Array<string | undefined | false>): string[] {
	return Array.from(
		new Set(actions.filter((item): item is string => typeof item === "string" && item.trim().length > 0)),
	);
}
...
function websocketRecoveryActions(code: string): string[] {
	...
	return uniqueRecoveryActions([
		code === "WEBSOCKET_INVALID_INPUT"
			? "use the WebSocket primitive's documented url, connection id, matcher, or steps fields"
			: undefined,
		["WEBSOCKET_SESSION_ALREADY_OPEN", "WEBSOCKET_SESSION_NOT_FOUND", "WEBSOCKET_SESSION_NOT_OPEN"].includes(code)
			? "use status or open with the intended WebSocket connection id"
			: undefined,
		[
			"WEBSOCKET_OPEN_FAILED",
			"WEBSOCKET_OPEN_TIMEOUT",
			...
			? "collect the WebSocket transcript, then retry once if the task still requires the connection; Browser Pilot owns the deadline"
			: undefined,
		code === "WEBSOCKET_INVALID_MATCHER"
			? "replace unsafe regex matching with contains or a bounded safe regex before retrying WebSocket wait"
			: undefined,
	]);
}

export function mergeRecoveries(
	primary: ErrorRecovery | undefined,
	secondary: ErrorRecovery | undefined,
...
export function recoveryForNormalized(
	code: string,
	details: Record<string, unknown>,
	taxonomy: RecoveryTaxonomyLike,
): ErrorRecovery | undefined {
	const actions = uniqueRecoveryActions([
		...abmlRecoveryActions(code, details),
		...websocketRecoveryActions(code),
		["NO_TAB", "TAB_NOT_FOUND", "INVALID_TAB_ID", "TAB_ID_REQUIRED"].includes(code)
			? "browser_tabs action=list"
			: undefined,
		["SELECTOR_NOT_FOUND", "INVALID_SELECTOR", "ELEMENT_NOT_FOUND"].includes(code)
			? "browser_observe to refresh the page model; use browser_command html.get for exact DOM evidence"
			: undefined,
		["BODY_UNAVAILABLE", "REQUEST_NOT_FOUND", "NETWORK_RECORDER_NOT_STARTED"].includes(code)
			? "use browser_command network.start on the intended tab, then network.list or network.body"
			: undefined,
		["NO_SESSION", "NOT_INSTALLED"].includes(code)
			? "use browser_command hook.install on the intended tab before collecting hook events"
			: undefined,
		[
			"INVALID_TIMEOUT",
			"TIMEOUT",
			...
			? "retry once with the current targetRef; Browser Pilot owns the execution deadline"
			: undefined,
		code === "TARGET_TRANSACTION_CONFLICT"
			? "finish the current target operation before starting work on another browser tab"
			: undefined,
		...
		code === "BROWSER_EXECUTION_ERROR"
			? "inspect the page error in this result; if selectors targeted stale DOM, refresh with browser_observe"
			: undefined,
	...
	return {
		summary: taxonomy.retryable
			? "Retry after correcting the target, selector, or evidence inputs."
			: "Inspect the suggested follow-up action before retrying.",
		nextActions: actions,
	};
}
