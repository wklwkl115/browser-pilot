...
export class SessionOperationRegistry {
	...
	begin(operation: Omit<SessionActiveOperationInfo, "operationId" | "startedAt" | "updatedAt"> & { operationId?: string }): SessionActiveOperationInfo {
		this.pruneStale();
		const now = this.now();
	...
	finish(operationId: string): SessionActiveOperationInfo | undefined {
		this.pruneStale();
		const current = this.operations.get(operationId);
		if (!current) return undefined;
	...
	snapshot(): SessionActiveOperationInfo[] {
		this.pruneStale();
