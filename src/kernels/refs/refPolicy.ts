...
export function defaultRefPolicyForKind(
	kind: BrowserPilotRefKind,
	options: { hasOwnerBinding?: boolean; sensitive?: boolean } = {},
...
export function classifyRefScope(
	...
	if (ref.owner.browserSessionId && ref.owner.browserSessionId !== context.browserSessionId) {
		if (!(ref.kind === "data-slice" && ref.policy.shareableAcrossSessions))
	...
	if (
		...
		!(ref.kind === "data-slice" && ref.policy.shareableAcrossSessions)
	) {
...
export function decideRefAccess(
	...
	if (options.resourceFound === false)
		return { ok: false, code: "HANDLE_NOT_FOUND", reason: "snapshot resource not found", sameSession };
	if (options.resourceExpired === true)
		return { ok: false, code: "HANDLE_EXPIRED", reason: "snapshot resource expired", sameSession };
	if (options.etagMatches === false)
		return { ok: false, code: "HANDLE_ETAG_MISMATCH", reason: "snapshot etag mismatch", sameSession };
	...
	if (ref.policy.redaction === "default") {
		...
		if (options.sensitive === false && sameSession && context.explicitSensitiveAccess)
			return { ok: true, mode: "raw", sameSession };
		return {
			ok: false,
			code: "PRIVACY_BLOCKED",
			reason: "raw access requires explicit safe same-session opt-in",
			sameSession,
		};
	}
	if (context.requestedRedaction === "default") return { ok: true, mode: "redacted", sameSession };
	if (!context.explicitSensitiveAccess)
		return {
			ok: false,
			code: "PRIVACY_BLOCKED",
			reason: "raw sensitive access requires explicit opt-in",
			sameSession,
		};
	if (!sameSession)
		return { ok: false, code: "PRIVACY_BLOCKED", reason: "raw sensitive access is same-session only", sameSession };
	return { ok: true, mode: "raw", sameSession };
}
