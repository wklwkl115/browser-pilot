...
function shouldRedactPayloadText(key: string, parentPayload: boolean): boolean {
	...
	return (
		parentPayload &&
		(normalized === "text" || normalized === "value" || normalized === "data" || normalized === "content")
	);
}
...
export function redactSensitiveText(text: string): string {
	...
	const redacted = redactUrlQueryValues(sheltered)
		...
		.replace(
			/(\b(?:auth|bearer|cookie|authorization|proxy-authorization|set-cookie|x-api-key|x-auth-token|x-csrf-token|x-xsrf-token|x-amz-security-token|x-aws-ec2-metadata-token)\s*:\s*)[^\r\n]*/gi,
			"$1[redacted]",
		)
		.replace(
		...
		.replace(
			/("(?:auth|bearer|cookie|cookies|authorization|proxy-authorization|set-cookie|x-api-key|x-auth-token|x-csrf-token|x-xsrf-token|x-amz-security-token|x-aws-ec2-metadata-token|token|access[_-]?token|refresh[_-]?token|id[_-]?token|session[_-]?token|secret|private[_-]?key|password|api[_-]?key|otp|totp|postData|payloadData|body)"\s*:\s*)"(?:\\.|[^"\\])*"/gi,
			'$1"[redacted]"',
		)
		.replace(
			/((?:auth|bearer|cookie|authorization|proxy-authorization|set-cookie|x-api-key|x-auth-token|x-csrf-token|x-xsrf-token|x-amz-security-token|x-aws-ec2-metadata-token|token|access[_-]?token|refresh[_-]?token|id[_-]?token|session[_-]?token|secret|private[_-]?key|password|api[_-]?key|otp|totp)\s*=\s*)[^;&\s,"'}]+/gi,
			"$1[redacted]",
		)
		.replace(/\bBearer\s+[A-Za-z0-9._~+/=-]+/gi, "Bearer [redacted]")
		.replace(/\bBasic\s+[A-Za-z0-9+/=]+/gi, "Basic [redacted]")
		.replace(
			/([?&](?:auth|authorization|bearer|token|access[_-]?token|refresh[_-]?token|id[_-]?token|session[_-]?token|secret|password|api[_-]?key|otp|totp)=)[^&#\s"'<>]+/gi,
			"$1[redacted]",
		)
		.replace(
		...
		if (parentBody) {
			try {
				return JSON.stringify(
					redactSensitiveValueInner(JSON.parse(value), options, new WeakSet<object>(), false, depth + 1),
				);
			} catch {
