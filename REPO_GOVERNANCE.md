...
- Use `mise run smoke-browser` after changing bridge/daemon/extension/live-browser behavior. It launches an installed Chrome/Edge/Chromium with the unpacked MV3 extension and verifies handshake, tabs, `browser-operation/v2` success/failure classification, full/delta observe, network capture, hook install/collect/uninstall, page-epoch re-anchoring, and extension reconnect; Windows CI runs the same acceptance gate.
