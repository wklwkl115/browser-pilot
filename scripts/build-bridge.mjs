...
const allowedDistJs = new Set(
	fingerprintInputs.filter((rel) => rel.includes("/dist/")).map((rel) => path.basename(rel)),
);
const entries = [
	{
		name: "service-worker",
...
const onePixelPngBase64 =
	"iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9s6Nwl8AAAAASUVORK5CYII=";
...
export async function computeBuildId(distDir) {
	...
	if (unexpectedDistJs.length)
		throw new Error(
			`Unexpected bridge dist JavaScript bundle(s) outside fingerprint inputs: ${unexpectedDistJs.join(", ")}`,
		);
...
async function main() {
	...
	await writeFile(
		path.join(defaultDistDir, ".npmignore"),
	...
	await writeFile(
		path.join(defaultDistDir, "build-manifest.json"),
		JSON.stringify(
			{
				generated: true,
				...
				exclusions: {
					...
					"dist/*.js.map":
						"Dev-only sourcemaps are not loaded by the browser and may contain checkout-local paths.",
	...
	if (!quiet)
		console.log(
			JSON.stringify(
				{
					ok: true,
					distDir: path.relative(root, defaultDistDir),
					buildId,
					entries: entries.map((entry) => entry.outfile),
				},
				null,
				2,
			),
