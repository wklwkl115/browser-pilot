...
const outputs = [
	...
	[path.join(root, "src", "types", "nativeProtocol.ts"), generateNodeProtocolTs],
	[path.join(root, "src", "commands", "nativeActionMetadata.ts"), generateNativeActionMetadataTs],
...
function generatedNativeActionMetadata() {
	...
	for (const [commandName, tool] of Object.entries(source.nativeActionTools || {})) {
		...
		// action tool — never hand-listed — so it stays correct after protocol sync.
...
function generateNativeProtocolDocs() {
	const readmeSnippet = "Run `npm run sync:protocol` after editing `src/bridge/protocol/native-command.schema.json`.";
	...
	return `# Native Protocol Contract (Generated)\n\n` +
		...
		`- Generated runtime/type outputs: \`src/bridge/extension/service_worker/protocol.ts\`, \`src/types/nativeProtocol.ts\`, \`src/commands/nativeActionMetadata.ts\`, \`src/types/nativeErrorCodes.ts\`.\n\n` +
		`## Native commands\n\n` +
