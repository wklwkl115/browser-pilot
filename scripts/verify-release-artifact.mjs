...
async function releaseIdentity() {
	...
	const extensionManifest = JSON.parse(
		await readFile(path.join(root, "bridge", "browser_pilot_bridge", "manifest.json"), "utf8"),
	);
	if (
		extensionManifest.version !== pkg.version ||
		extensionManifest.version_name !== `${pkg.version}-browser-pilot`
	) {
		throw new Error(
			`extension version mismatch: expected ${pkg.version}, received ${extensionManifest.version || "<missing>"}`,
		);
	}
	const tag = process.env.GITHUB_REF_NAME;
	const expectedTag = `v${pkg.version}`;
	if (tag !== expectedTag)
		throw new Error(`release tag mismatch: expected ${expectedTag}, received ${tag || "<missing>"}`);
...
async function verifyArtifact(directory, identity) {
	...
	if (tarballs[0].name !== expectedFilename)
		throw new Error(`release filename mismatch: expected ${expectedFilename}, received ${tarballs[0].name}`);
...
if (expectedSha256 && artifact?.sha256 !== expectedSha256)
	throw new Error(`release artifact SHA-256 mismatch: expected ${expectedSha256}, received ${artifact?.sha256}`);
...
process.stdout.write(
	`${JSON.stringify({ schema: "browser-pilot-release-artifact/v1", ...identity, ...(artifact ?? {}) })}\n`,
);
