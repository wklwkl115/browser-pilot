...
;(function PiBrowserHookDispatcher() {
  ...
  function install(optsInput?: unknown): HookResponse {
    ...
    hookXHR(); hookFetch(); hookWebSocket(); hookConsole(); piBrowserErrors(); hookDOM(); hookDomSinks(); hookCookies(); hookStorage(); hookCrypto();
    ...
    return { ok: true, data: {
      session_id, state, installed_at, targets: clone(targets), pi_browser_version: VERSION, dispatcher_version: VERSION,
  ...
  function status(optsInput?: unknown): HookResponse {
    ...
    return { ok: true, data: {
      ...
      targets: clone(targets), options: clone(options), pi_browser_version: VERSION, dispatcher_version: VERSION,
  ...
  window.addEventListener('message', (e: MessageEvent) => {
    ...
    window.postMessage({ __pi_browser_hook_response__: true, id, resp }, '*');
  });
  window.__PI_BROWSER_HOOKS__ = {
