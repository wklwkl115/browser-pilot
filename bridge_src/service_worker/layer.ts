...
async function cdpSend(tabId: number, method: string, params: JsonRecord = {}, timeoutMs?: number): Promise<JsonRecord> {
	...
	const response = normalizePersistentPiBrowserResponse(await cdp.send(tabId, method, params, { persistent: true, name: "layer_probe", timeoutMs }));
...
export async function probeLayerTree(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
	...
	const cdp = piBrowserPersistentCdp();
	if (!cdp?.send) return piBrowserError(PI_BROWSER_ERROR_CODES.INTERNAL_ERROR, "persistent CDP helper is not loaded", { tabId, cmd: "layer.probe" });

	const events: JsonRecord[] = [];
	const subscriptionId = subscribePiBrowserCdp(tabId, "LayerTree.layerTreeDidChange", (_source, _method, params) => {
	...
	try {
		...
		if (disableError) console.warn("[PI-BROWSER-LAYER] LayerTree.disable failed", disableError);
	}
}
