...
type WaitChild = PiBridgeCommand & { cmd?: string; type?: string; kind?: string };
type SettledWait = PromiseSettledResult<PiBridgeResponse>;
...
async function waitForAny(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
  ...
  const tasks = waits.map((w: WaitChild, i: number) => dispatchPiBrowserWait(tabId, { ...w, waitId: childWaitIds[i], wait_id: childWaitIds[i], abortController: controllers[i], timeoutMs: childTimeoutMs(w) }, w.cmd || w.type || w.kind || 'selector').then((result: PiBridgeResponse) => { if (result && result.ok) return { index:i, result }; throw result; }));
  ...
  try {
    ...
    const won = winner as { index: number; result: PiBridgeResponse };
    ...
    const firstTimeout = errors.find((x: JsonRecord) => x && (x.error_code === PI_BROWSER_ERROR_CODES.TIMEOUT || /timeout|timed out|parent timeout/i.test(String(x.error || x.message || ''))));
...
async function waitForAll(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
  const waits = (Array.isArray(msg.waits) ? msg.waits : Array.isArray(msg.conditions) ? msg.conditions : []) as WaitChild[];
  ...
  const tasks = waits.map((w: WaitChild, i: number) => dispatchPiBrowserWait(tabId, { ...w, abortController: controllers[i], timeoutMs: Math.min(normalizePiBrowserTimeoutMs(w || {}, parentTimeoutMs), Math.max(0, deadline - Date.now())) }, w.cmd || w.type || w.kind || 'selector')
    .then((result: PiBridgeResponse) => {
...
async function waitForComposite(tabId: number, msg: PiBridgeCommand, mode: string): Promise<PiBridgeResponse> { return mode === 'waitForAny' ? await waitForAny(tabId, msg) : await waitForAll(tabId, msg); }
function normalizePiBrowserWaitKind(kind: unknown, msg?: PiBridgeCommand): string {
...
async function dispatchPiBrowserWait(tabId: number, msg: PiBridgeCommand, kind: unknown): Promise<PiBridgeResponse> {
  ...
  if (k === 'navigateandwait' || k === 'navigate') return await navigateAndWait(tabId, msg) as PiBridgeResponse;
...
function cancelPiBrowserWait(tabId: number, msg: PiBridgeCommand) { return cancelWait(tabId, msg); }

function extractPiBrowserRuntimeValue(resp: PiBridgeResponse): JsonRecord | null {
...
async function cleanupPiBrowserPageListenersForTab(tabId: number, reason?: string, timeoutMs?: number): Promise<PiBridgeResponse> {
...
async function addEventListener(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
...
async function removeEventListener(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
...
async function getPerformanceEntries(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
  const input = msg || {};
...
async function diagnosePiBrowser(tabId: number, _msg: PiBridgeCommand): Promise<PiBridgeResponse> {
  ...
  try {
    ...
    })()` }).catch((e: unknown) => piBrowserError(PI_BROWSER_ERROR_CODES.NO_SESSION, waitErrorMessage(e), {}));
