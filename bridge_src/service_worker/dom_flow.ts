import { normalizePersistentPiBrowserResponse, piBrowserError, piBrowserEval, piBrowserPersistentCdp, PI_BROWSER_ERROR_CODES } from "./runtime";
...
export async function collectNodeListeners(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
	try {
...
export async function collectNodeSinkHints(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
	try {
		const selector = selectorText(msg.selector);
		const timeoutMs = asPositiveInt(msg.timeoutMs ?? msg.timeout_ms, 10_000, 1, 60_000);
...
export async function collectNodeListenerChain(tabId: number, msg: PiBridgeCommand): Promise<PiBridgeResponse> {
