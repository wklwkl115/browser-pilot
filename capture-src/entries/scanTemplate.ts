...
export function scanPage(config: any) {
	...
	function safeSemanticLabel(text, max = 160) {
		...
		if (/^\s*(?:[#.]?[a-z][\w-]*|[a-z][\w-]*(?:\[[^\]]+\])?)(?:\s*[>+~]\s*[#.\w\-:[\]="']+){1,}\s*$/i.test(value))
			return "";
		...
		const fieldWords =
			...
			).length + (value.match(/(?:输入|输出|缓存|价格|计费|模型|倍率|上下文|请求|额度|套餐)/g) || []).length;
		if (
			moneyTokens >= 2 ||
			(moneyTokens && (fieldWords >= 2 || separators >= 2)) ||
			(fieldWords >= 4 && separators >= 2)
		)
			return "";
		return value;
	}
	...
	function computedAccessibleName(el, max = 160) {
		...
		accessibleNameCount++;
		try {
			return safeSemanticLabel(provider(el, { computedStyleSupportsPseudoElements: false }), max);
		} catch (_) {
			return "";
		}
	}
	...
	function cleanClassValue(value) {
		return String(value || "")
			...
			.filter(
				(cls) =>
					cls &&
					!NOISE_CLASS_PATTERNS.some((pattern) => cls.toLowerCase().includes(String(pattern).toLowerCase())),
			)
			.join(" ");
	}
	...
	function matchesIgnoredSelector(el) {
		try {
			return IGNORE_SELECTORS.some((sel) => (el.matches && el.matches(sel)) || (el.closest && el.closest(sel)));
		} catch (_) {
			return false;
		}
	}
	function isIgnored(el) {
		...
		const ignored =
			...
			IGNORE_TAGS.has(el.tagName) ||
			matchesIgnoredSelector(el) ||
			(el.tagName === "INPUT" && String(el.type || "").toLowerCase() === "hidden") ||
			((el.tagName === "IFRAME" || el.tagName === "IMG") && isExtensionUrl(attrValue(el, "src")));
	...
	function isHidden(el) {
		...
		try {
			...
			hidden =
				...
				(Number(style.opacity) === 0 && !KEEP_EMPTY.has(el.tagName));
		} catch (_) {
			/* Style access failed; keep the visible fallback. */
		}
	...
	function nativeRoleOf(el) {
		...
		if (tag === "INPUT") {
			...
			if (["email", "tel", "text", "url"].includes(t))
				return el.hasAttribute && el.hasAttribute("list") ? "combobox" : "textbox";
			return "textbox";
		}
	...
	function actionNameOf(el) {
		...
		const cls = cleanClassValue(
			el.className && typeof el.className === "object" ? el.className.baseVal || "" : el.className || "",
		)
			.split(/\s+/)
			.find((part) => ACTIONABLE_RE.test(part));
	...
	function labelOf(el) {
		...
		if (tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT") {
			try {
				...
				/* Ignore inaccessible labels and use the value fallback. */
			}
			if ((tag === "INPUT" || tag === "TEXTAREA") && el.value) return clean(el.value, 160);
		}
		return (
			safeSemanticLabel(el.innerText || el.textContent || "", 160) ||
		...
		try {
			...
			for (let depth = 0; cur && depth < 3; depth++, cur = cur.parentElement) {
				const heading =
					cur.querySelector &&
					cur.querySelector(
						'h1,h2,h3,h4,h5,h6,[role="heading"],legend,caption,.sr-only,.visually-hidden,.screen-reader-text',
					);
	...
	function editable(el) {
		...
		if (tag === "INPUT")
			return !["hidden", "button", "submit", "reset", "checkbox", "radio", "file", "image"].includes(
	...
	function clickConfidence(el, style, handlers) {
		...
		if (["button", "link", "menuitem", "tab", "checkbox", "radio", "switch", "option"].includes(role))
			return "high";
		if (el.onclick || el.getAttribute("onclick") || handlers.some((name) => FRAMEWORK_ACTION_RE.test(name)))
			return "high";
		...
		const attrsText = [
			...
			el.getAttribute && el.getAttribute("data-e2e-state"),
		]
			.join(" ")
			.toLowerCase();
	...
	function cssEscape(value) {
		try {
			return CSS && CSS.escape ? CSS.escape(String(value)) : String(value).replace(/[^a-zA-Z0-9_-]/g, "\\$&");
		} catch (_) {
	...
	function selectorFor(el) {
		...
		if (el.id) {
			...
			try {
				if (document.getElementById(el.id) === el) {
					SELECTOR_CACHE.set(el, byId);
					return byId;
				}
			} catch (_) {
				/* Invalid ids fall through to a structural selector. */
			}
		}
		const parts = [];
		let cur = el;
		...
		while (cur && cur.nodeType === Node.ELEMENT_NODE && cur !== document.documentElement) {
			...
			if (cur !== document.body) {
				const cls = cleanClassValue(
					cur.className && typeof cur.className === "object"
	...
	function refTargets(el, attr, refElements) {
		...
		for (const id of ids.slice(0, 32)) {
			...
			out.push(sel);
			if (refElements && !refElements.has(sel))
				refElements.set(sel, {
					selector: sel,
					role: roleOf(target),
	...
	function hitTargetInfo(hit, owner) {
		...
		const inputLabel =
			differs && (tag === "input" || tag === "textarea" || tag === "select") && hit.getAttribute
				? clean(
						hit.getAttribute("aria-label") ||
							hit.getAttribute("placeholder") ||
		...
		return {
			tag,
			id: hit.id || "",
			class: cleanClassValue(
				hit.className && typeof hit.className === "object" ? hit.className.baseVal || "" : hit.className || "",
			).slice(0, 80),
			text: clean(hit.innerText || hit.textContent || "", 80),
	...
	function visibleInfo(el) {
		...
		if (inViewport && document.elementFromPoint) {
			...
			const centerOk =
				!centerHit ||
				centerHit === el ||
				el.contains(centerHit) ||
				(centerHit.contains && centerHit.contains(el));
			...
			if (centerOk) {
				...
				if (elIdx < 200) {
					...
					const cornerOk =
						!cornerHit ||
						cornerHit === el ||
						el.contains(cornerHit) ||
						(cornerHit.contains && cornerHit.contains(el));
					if (cornerOk) {
						hitOk = true;
						if (cornerHit) hitTarget = hitTargetInfo(cornerHit, el);
					} else {
						hitOk = false;
						if (cornerHit && !occluderSelector) occluderSelector = selectorFor(cornerHit);
					}
				} else {
					hitOk = false;
				}
		...
		const documentRect = {
			x: Math.round(r.x + (window.scrollX || 0)),
			y: Math.round(r.y + (window.scrollY || 0)),
			width: Math.round(r.width),
			height: Math.round(r.height),
		};
		const point = {
	...
	function scoreActionable(item) {
		...
		if (item.action)
			score += PRIMARY_INTENT_RE.test(item.action) ? 900 : HIGH_INTENT_RE.test(item.action) ? 420 : 160;
	...
	function edgeUtilityHint(visible, style) {
		...
		const small =
			Number(r.width || 0) > 0 &&
			Number(r.height || 0) > 0 &&
			Number(r.width || 0) <= 180 &&
			Number(r.height || 0) <= 180;
		const nearEdge =
			Number(r.x || 0) <= 32 ||
			Number(r.y || 0) <= 32 ||
			Number(r.x || 0) + Number(r.width || 0) >= vw - 32 ||
			Number(r.y || 0) + Number(r.height || 0) >= vh - 32;
	...
	function collectActionables(root, elements, itemScopes) {
		...
		for (const el of source) {
			...
			if (
				...
				(SKIP.has(el.tagName) && el.tagName !== "CANVAS") ||
				isIgnored(el) ||
				isHidden(el)
			)
				continue;
			...
			if (pseudoCheckCount < PSEUDO_CHECK_LIMIT) {
				...
				if (ps.before || ps.after) {
					if (!label)
						label = [ps.before, clean(el.innerText || el.textContent || "", 140), ps.after]
							.filter(Boolean)
							.join(" ")
							.trim();
			...
			const item = {
				...
				editable: isEditable,
				actionConfidence: isEditable ? "high" : confidence,
				visible: true,
				inViewport: visible.inViewport,
				disabled: !!el.disabled || el.getAttribute("aria-disabled") === "true",
				focused: document.activeElement === el,
				...(checkedState === undefined ? {} : { checked: checkedState }),
				...(selectedState === undefined ? {} : { selected: selectedState }),
				...(pressedAttr === "true" || pressedAttr === "false" ? { pressed: pressedAttr === "true" } : {}),
				...
				...((expandedAttr === "true" || expandedAttr === "false") && controlsSelectors.length
					? { expandedTargetSelectors: controlsSelectors }
					: {}),
				...(edgeHint.position ? { position: edgeHint.position } : {}),
				...(edgeHint.edgeUtility ? { edgeUtility: true } : {}),
				handlers: handlers.slice(0, 6),
				rect: visible.rect,
				documentRect: visible.documentRect,
				...(visible.inViewport ? { point: visible.point } : {}),
				hitOk: visible.hitOk,
				hitTarget: visible.hitTarget,
				...(scope ? { scope } : {}),
				...((el.tagName === "A" || el.tagName === "AREA") && el.href ? { href: String(el.href) } : {}),
				...((el.tagName === "A" || el.tagName === "AREA") && el.rel
					? { rel: String(el.rel).toLowerCase() }
					: {}),
				...(visible.hitOk === false && visible.occluderSelector
					? { occluderSelector: visible.occluderSelector }
					: {}),
			};
		...
		return out
			.sort((a, b) => b.priority - a.priority || a.rect.y - b.rect.y || a.rect.x - b.rect.x)
	...
	function collectListHints(root, elements) {
		...
		containers: for (const container of containers) {
			...
			for (const child of container.children) {
				if (++childScans > options.maxNodes) {
					truncated = true;
					break containers;
				}
				if (
					...
					continue;
				const cls = cleanClassValue(
					child.className && typeof child.className === "object"
			...
			for (const [key, items] of groups.entries()) {
				...
				const totalText = items.reduce(
					(sum, item) => sum + clean(item.innerText || item.textContent || "", 500).length,
					0,
				);
				...
				hints.push({
					...
					firstItemPreview: safePreviewOf(items[0]),
				});
	...
	function collectCanvasRegions(elements) {
		...
		for (const el of canvases) {
			...
			if (!visible.rendered) continue;
			const label = clean(
				el.getAttribute("aria-label") || el.getAttribute("title") || el.id || "canvas region",
				120,
			);
			out.push({
				index: out.length,
				...
				inViewport: visible.inViewport,
				hitOk: visible.hitOk,
				clickable: clickConfidence(el, styleOf(el), frameworkHandlers(el)) !== undefined,
			});
		}
		return out;
	}
	...
	function collectControlsPairs(elements) {
		...
		for (const el of elements) {
			...
			pairs.push({
				...
				...(ownsSelectors.length ? { ownsSelectors } : {}),
				...((expandedAttr === "true" || expandedAttr === "false") && ctlSelectors.length
					? { expandedTargetSelectors: ctlSelectors }
					: {}),
			});
		}
	...
	for (const el of scanElements.filter(
		(item) => item.matches && item.matches('canvas,video,embed,object,iframe,img:not([alt]),img[alt=""]'),
	)) {
		const r = el.getBoundingClientRect();
	...
	const unnamedActionableCount = actionables.filter(
		(item) => !clean(item.label || item.displayLabel || item.text || item.action || "", 120),
	).length;
	...
	const headings = scanElements
		.filter((node) => node.matches && node.matches('h1,h2,h3,h4,h5,h6,[role="heading"]'))
		.map((node) =>
	...
	const scanFingerprint = {
		...
		interactiveCount: scanElements.filter(
			(node) => node.matches && node.matches("a[href],button,input,textarea,select,[role='button'],[tabindex]"),
		).length,
		capturedAt: Date.now(),
	};
	return {
